...
| | |
|---|---|
| **만든다** | 문서·답변·코드에서 항목을 뽑고, 서로 어긋난 것은 사람이 결정합니다. 승인된 것만 남습니다. |
| **배포한다** | 같은 snapshot 은 언제나 같은 Pack 입니다. 모든 기기가 같은 버전·같은 해시를 받습니다. |
| **진행이 보인다** | 각자의 AI 가 작업 끝에 근거를 보고하고, 완료는 사람이 확인합니다. 행은 마일스톤입니다. |
...
|---|---|---|
| **개발자 A** | 최대 5회까지 지수 백오프로 재시도합니다. 고정 간격은 금지라고 되어 있습니다.<br><sub>쉬운 말로: 5번까지, 기다리는 시간을 점점 늘려 가며</sub> | `paylab-docs/goals.md §3.1`<br><sub>**팀 문서**를 읽었습니다</sub> |
| **개발자 B** | MAX_RETRY = 3 이고 간격은 500ms 고정입니다.<br><sub>쉬운 말로: 3번까지, 0.5초 간격으로 똑같이</sub> | `paylab-api/src/payment/retry.ts:11`<br><sub>**실제 코드**를 읽었습니다</sub> |

**둘 다 거짓말을 한 게 아닙니다.** 문서와 코드가 어긋나 있었고, 각자 자기가 읽은 것을 답했을 뿐입니다.
같은 팀, 같은 질문, 다른 답 — 그리고 **어긋났다는 사실 자체를 아무도 몰랐습니다.**
...
> 다시 시도하는 경우는 **응답이 안 올 때(타임아웃)**와 **상대 서버가 고장 났을 때(5xx)** 둘뿐이다.</sub>
...
**핵심은 이 표입니다.** 답만 주는 게 아니라 **"어느 문서 몇 절, 어느 코드 몇 번째 줄에서 나왔고,
누가 승인했는지"**가 항상 붙어 다닙니다. 근거를 못 대는 줄은 **애초에 배포되지 않습니다**(원칙 P7).
그래서 이제 A·B·C 모두 같은 답을 합니다.
...
|---|---|
| 팀·프로젝트 ID<br><sub>팀 이름과 프로젝트 이름</sub> | **저장소 코드 본문**<br><sub>소스 코드 그 자체</sub> |
| 구조화된 항목 (제목·규칙·마일스톤)<br><sub>정리된 규칙 문장</sub> | **환경변수 · secret**<br><sub>비밀번호·API 키 등 접속 정보</sub> |
| 팀장이 직접 등록한 문서 원문<br><sub>팀장이 손수 붙여 넣은 것만</sub> | 개인 CLAUDE.local.md<br><sub>개인이 따로 쓰는 메모</sub> |
| 경로 · 줄 번호 · 커밋 해시<br><sub>"몇 번째 줄"이라는 위치 표시만</sub> | Auto Memory<br><sub>AI 가 혼자 쌓아 둔 기억</sub> |
| 버전 · manifest 해시<br><sub>몇 판인지 · 내용이 같은지</sub> | Claude 대화 내용<br><sub>AI 와 주고받은 말 전부</sub> |
...
| 무엇 | 어디서 보나 |
|---|---|
| 서로 어긋난 것을 AI가 찾아 **카드로 물어보는** 화면 | `정리` |
