...
팀장은 브라우저에서 15분이면 됩니다. 개발자는 명령 한 줄이면 됩니다.
...
아래는 만들어낸 예시가 아니라, **샘플 팀에서 실제로 나온 답**입니다.
누구나 [데모](#직접-보기)에서 그대로 확인할 수 있습니다.

> **질문:** 결제가 실패하면 몇 번까지 다시 시도하나요?

| 😵 **전** — 각자의 AI | ✅ **후** — 팀이 승인한 것 하나 |
|---|---|
| **개발자 A의 AI**<br>"최대 5번까지, 간격을 점점 늘리며 재시도합니다."<br><sub>→ 팀 문서를 읽음</sub> | **모두의 AI가 같은 답**<br>"최대 5번까지 재시도한다. 간격은 0.5초 → 1초 → 2초 → 4초 → 8초로 늘린다. 시간 초과와 서버 오류일 때만 재시도한다." |
| **개발자 B의 AI**<br>"3번까지, 0.5초 간격으로 재시도합니다."<br><sub>→ 실제 코드를 읽음</sub> | **어디서 나온 답인지도 함께**<br><sub>팀 문서 §3.1 · 코드 11~14줄 · 팀장 승인 2026-09-01</sub> |
| ⚠️ 같은 팀, 같은 질문, **다른 답** | ✅ **A·B·C 모두 같은 답** |

**핵심은 오른쪽 아래 칸입니다.** 답만 주는 게 아니라 **"어느 문서 몇 쪽, 어느 코드 몇 번째 줄에서
나왔고, 누가 언제 승인했는지"**가 항상 따라붙습니다. 근거 없는 답은 나오지 않습니다.
...
|---|---|
| 팀·프로젝트 이름 | **소스 코드 내용** |
| 정리된 규칙 문장 (제목·내용·기한) | **비밀번호 · API 키 등 접속 정보** |
| 팀장이 직접 올린 문서 | 개인이 따로 쓰는 메모 |
| "몇 번째 줄" 같은 **위치 표시만** | AI와 나눈 **대화 내용** |
