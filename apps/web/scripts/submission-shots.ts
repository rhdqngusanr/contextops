...
// =====================================================================
//  제출 그림의 **원본 캡처** — production 을 **16:9** 로 찍는다 (2026-09-14)
//
//    pnpm --filter web exec tsx scripts/submission-shots.ts --url https://<production> [--only '1b-landing-compare,5-sync']
//
...
//    갤러리에 올리는 그림은 이 원본을 `submission-cards.ts` 가 잘라 제목을 얹은 것이다(전체 화면은 작은 칸에서 글자가 안 보였다).
//  ★ 들어가는 길은 심사위원과 같다 — `/demo` 로 게스트 세션을 받는다. 인증을 우회하는 문을 쓰지 않는다.
//  ⚠ `2-review-ai` 는 **진짜 AI 를 한 번 부른다** (게스트 체험 `POST /demo/ai-once` · 저장 안 됨 · IP 하루 상한 안 · 한 번에 약 $0.004).
//    결과 문장·초·값이 매번 달라지므로, 카드 제목 띠의 숫자(`submission-cards.ts`)와 맞는지 다시 찍은 뒤 확인한다 — 필요 없으면 `--only` 로 뺀다.
...
/** 찍는 원본의 이름 — `--only` 가 이 표에서 고른다. 게스트 세션이 필요한 것은 뒤의 넷이다. */
const SHOT_NAMES = ['1-landing', '1b-landing-compare', '2-review-ai', '3-pack-trace', '4-roadmap', '5-sync'] as const
type ShotName = (typeof SHOT_NAMES)[number]
const APP_SHOTS: readonly ShotName[] = ['2-review-ai', '3-pack-trace', '4-roadmap', '5-sync']
...
async function capture(page: Cdp, name: ShotName): Promise<void> {
...
async function main(): Promise<void> {
  const only = optionalArg('only')?.split(',').map((s) => s.trim())
  for (const name of only ?? []) {
    if (!(SHOT_NAMES as readonly string[]).includes(name)) throw new Error(`--only 는 ${SHOT_NAMES.join(',')} 중에서 고른다 — «${name}»`)
  }
  const want = (name: ShotName): boolean => only === undefined || only.includes(name)
  ...
  try {
    ...
    if (want('1-landing') || want('1b-landing-compare')) {
      //  ① 첫 화면 — 문제(같은 질문에 사람마다 다른 답)가 보이는 자리
      await open(page, '/', '.btn-primary')
      if (want('1-landing')) await capture(page, '1-landing')
      if (want('1b-landing-compare')) {
        //  ①b 첫 화면 아래 「같은 질문, 다른 답」 띠 — 문제를 한 장으로 보여 주는 자리라 화면 위쪽으로 끌어올려 찍는다.
        const compare = 'section[aria-labelledby="landing-compare"]'
        const scrolled = await page.evalJs<boolean>(`(() => {
          const s = document.querySelector(${JSON.stringify(compare)})
          if (!s) return false
          window.scrollTo(0, s.getBoundingClientRect().top + window.scrollY - 40)
          return true
        })()`)
        if (scrolled !== true) throw new Error('랜딩에 「같은 질문, 다른 답」 띠가 없다')
        await sleep(600)
        await settled(page, compare)
        await capture(page, '1b-landing-compare')
      }
    }

    if (APP_SHOTS.some(want)) {
