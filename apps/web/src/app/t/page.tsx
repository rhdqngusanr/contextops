...
import { TEAM_HOME_TEXT, TeamHome, type TeamHomeState } from '../../components/team-home'
...
function TeamsView() {
  ...
  async function submitInvite(): Promise<void> {
    if (!inviting) return
    //  🔴 쓰기 문 — 누르기 전에 서버와 같은 표를 읽는다 (게스트는 `/t` 에 올 일이 없지만 문은 하나다).
    ...
    try {
      ...
      setInviting({ ...inviting, busy: false, email: '', done: made.status === 'invited' ? `${made.name} 을(를) 초대했습니다. 로그인하면 팀원이 됩니다.` : `${made.name} 이(가) 팀원이 됐습니다.` })
    } catch (err) {
