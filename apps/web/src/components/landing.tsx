...
export const BEFORE_AFTER = {
  ...
  //  ⚠ 2026-09-13 — 「던졌습니다」(실제로 해 본 기록처럼 읽히는 과거형)를 장면 묘사로 바꿨다. 두 답은 픽스처의 문서·코드에
  //    적힌 값이고, 두 터미널에 실제로 물어 녹화한 기록은 아직 없다(`docs/PITCH.md` §3). 녹화가 생기면 과거형으로 되돌려도 된다.
  caption: '같은 질문을, 같은 팀 두 사람이 각자의 Claude Code 에 물어봅니다.',
...
/** C-2. 어떻게 동작하나요 — 3단계. ⚠ 썸네일은 없다 — 제품 캡처는 「3분이면 됩니다」 절(`TOUR`)이 싣는다. */
export const HOW_IT_WORKS = {
  ...
  steps: [
    {
      head: '가져와서 정리한다',
      //  ⚠ 「질문 10개에 답하면 AI 가 후보를 뽑는다」가 아니다 — 기본 질문의 답은 표가 정한 자리로 옮겨질 뿐 AI 를 안 거친다
      //    (`app/api/v1/projects/[id]/questions/route.ts` · FINDINGS 178). README · 제출서가 이 문장을 글자 그대로 옮긴다.
      body: '목표 문서·회의록을 붙여 넣으면 AI 가 규칙·마일스톤 후보를 뽑고 서로 어긋난 것을 찾아 질문으로 만듭니다. 문서가 없으면 질문 10개에 답해 시작합니다. 결정은 사람이 합니다.',
