...
describe('conflicts — 선택 4개가 상태를 가른다', () => {
  ...
  it('목록의 status 필터가 갈린다', async () => {
    const { owner, projectId } = await seed()
    const kept = await seedConflict(projectId)
    const closed = await seedConflict(projectId)
  ...
  it('답을 주면 질문이 닫히고, 초안을 같이 주면 그때만 항목이 생긴다', async () => {
    const { owner, projectId } = await seed()
    const plain = await seedConflict(projectId, 'open_question')
    const withDraft = await seedConflict(projectId, 'open_question')
  ...
  it('남의 프로젝트 질문이 섞이면 하나도 반영하지 않는다', async () => {
    const { owner, projectId } = await seed()
    const mine = await seedConflict(projectId, 'open_question')
    const res = await answerQuestions(req('POST', `/api/v1/projects/${projectId}/questions`, {
      auth: owner,
      body: {
        answers: [
