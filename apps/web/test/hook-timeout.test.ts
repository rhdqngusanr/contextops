...
import base, { HOOK_TIMEOUT_MS } from '../../../vitest.base'

// =====================================================================
//  🔴 훅 상한은 **한 곳**에 산다 — 루트 `vitest.base.ts` 의 `HOOK_TIMEOUT_MS` (FINDINGS 136)
//
...
//  ⚠ `it(…, 60_000)` 같은 **시험 본문**의 상한은 안 센다 — 그건 `testTimeout` 이고 다른 개념이다.
// =====================================================================
...
describe('🔴 훅 상한은 vitest.base.ts 한 곳에 산다 (FINDINGS 136)', () => {
  ...
  it('③ 워크스페이스의 어떤 시험 파일도 훅에 자기 상한을 주지 않는다', () => {
    ...
    //  걷기가 조용히 0 을 찾는 상태를 막는다 — PGlite 를 띄우는 파일이 실제로 목록에 있어야 한다.
    ...
    const scattered = files.flatMap(hookTimeoutsIn)
    expect(
      scattered,
