...
> 규정상 영상·슬라이드는 **필수가 아니다** — 실제 제출 폼(2026-09-14)은 일곱 칸이었고 영상·슬라이드 칸이 없었다 (`docs/SUBMISSION.md` 「제출 폼 원문」).
> 🔴 **2026-09-14 결정 — 영상(만들어 둔 세로 쇼츠 포함)은 올리지 않는다.** 이 문서는 예선 **온라인 투표 20%** 나 본선의 **발표 전달력**(TOP20 · 10/17 Demo Day)에
> 쓸 때를 위한 컷 표·대본·체크리스트의 기록이다. 쇼츠는 `tools/make-video.ps1` 로 다시 만들 수 있다.
...
>
> ⚠ **2026-09-13 에 이 화면 녹화를 버리고** 세로 쇼츠(1080×1920 · `tools/make-video.ps1` · `apps/web/scripts/demo-stills.ts`)로 바꿨고,
> 그 쇼츠도 2026-09-14 에 올리지 않기로 했다. 아래 「세 군데」 중 어디에도 클립은 올라가 있지 않다(README 머리 포함).
...
|---|---|---|---|---|---|
| 1 | 0–12 | 랜딩 `/` 상단 | 스크롤 없이 머리 문장 · **Before/After** 카드로 내려감 | 「같은 팀인데 팀원마다 AI 가 다른 답을 합니다. 재시도는 5회인지 3회인지 — 각자의 CLAUDE.md 가 다르기 때문입니다.」 | `apps/web/src/components/landing.tsx` `BEFORE_AFTER`(paylab 픽스처의 사실) |
...
⚠ 컷 3 은 **게스트로** 찍는다 — 「읽기 전용으로 둘러보는 중입니다」가 뜨는 것이 제품이 정직하다는 장면이다. 발행 성공 장면이 필요하면 컷 3 을 팀장 계정으로 한 번 더 찍어 붙인다(production 로그인 → 발행은 2026-09-15 에 밟았다).
...
| 2 | 같은 팀, 다른 답 | Before/After 카드 그대로 — 재시도 5회 vs 3회 | 랜딩 캡처 |
...
| 8 | 다음 | 초대 메일 발송(초대 자체는 있다) · SSO·감사 로그 · 다른 에이전트 타깃(AGENTS.md · Cursor 는 이미 거울 문서) · 지속 가설(코어 MIT · 호스팅 팀 구독) | 로드맵 화면 |
...
✅ **2026-09-15 한 바퀴** — production 에서 로그인(사람) → 팀 → 문서 → AI 정리 → 승인 → 발행이 약 11분(06:24~06:35 KST)이었다.
로그인 뒤 걸음은 Claude in Chrome 이 눌렀으므로 사람 손 스톱워치는 아니다 — 기록은 `docs/evidence/2026-09-13-final-audit/README.md`. 사람이 처음부터 재는 것은 🙋 아직이다.

1. production 에 팀장 계정으로 로그인 (GitHub 로그인은 2026-09-10 부터 켜져 있다).
...
- [ ] `verify:prod` 초록 · `/api/v1/health` 의 `ai:true` · 데모 리셋이 그날 새벽 3시쯤 돌았음 (`POST /api/v1/demo/session` 이 게스트 세션을 준다)
...
- [ ] 제출 폼에는 영상 칸이 없었다(2026-09-14) — 올린다면 투표·본선 안내에 링크로 걸고 `docs/SUBMISSION.md` 🙋 표를 고친다
