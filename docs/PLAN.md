...
- [x] **7.1 문서 구조화 · 7.2 충돌 탐지 · 예산 가드** — `f487d67` · `b1f7d3c` (⑧ 에서 재고 닫음)
      **완료 기준**: paylab 문서 → 항목 12 + 충돌 3 · **모든 AI 호출이 `withBudget()` 경유** (P3) ·
      ...
      ④ 🔴 **진짜 Gemini 로 쟀다** (81바퀴 · `pnpm --filter web p3:measure` ·
      `docs/evidence/2026-09-06-p3-gemini/`): 첫 실행은 두 문서 다 `AI_OUTPUT_INVALID` —
      기본 thinking 이 출력 상한 8,000 을 먹어 JSON 이 잘렸다 (FINDINGS **141** ✅ ·
      `GEMINI_THINKING_LEVEL='low'`). 고친 뒤: goals.md **항목 18 · 열린 질문 4** ✅ ·
      `withBudget` 장부 3행 = 왕복 3 ✅ · offset **27/27 범위 안** — 그러나 **가리키는 문장이
      틀린 것이 많다**(모델이 글자를 못 센다 · FINDINGS **142** 구멍 · P7) · 충돌 **2/3** —
      셋째(환불 SLA)는 old-roadmap 쪽 항목이 안 뽑혀서다 (FINDINGS **143** 격차).
      ⑤ ✅ **142 를 닫았다** (82바퀴 · `3b6ebef`): 모델은 `quote` 를 내고 서버가 `indexOf` 로
      offset 을 계산한다 — 진짜 Gemini 인용 **21/21** 이 제목과 같은 문장 (probe.txt §5).
      같은 실행에서 충돌 **1/3** — goals 의 재시도 후보가 old-roadmap 과 같은 slug 라 accept 에서
      거절돼 탐지 재료에서 빠졌다 (FINDINGS **145** 구멍). 항목 12 · 질문 4 · 장부 3행.
      **행은 열어 둔다** — 143(프롬프트 · 환불 줄) · 145(기존 id 를 `uniqueId` 에) · 144 를 닫고
      `p3:measure` 가 충돌 3/3 일 때 체크한다.
      ⑥ ✅ **143 을 닫았다** (83바퀴 · `5be2611`): SYSTEM 에 「규칙 목록은 줄마다 항목 · 부정형도
      규칙」 두 줄 — 진짜 Gemini 로 old-roadmap 항목 **6**(policy 3 · 환불 줄이 두 번 다 policy).
      같은 코드로 두 번: 2회차 충돌 **3/3**(셋째 = 24h vs 기한 없음) · 1회차 **0/3**(후보 0 —
      후보가 모델이 고른 scope 에 달려 있다 · FINDINGS **146** 구멍). accept 거절 1 은 두 번 다(145).
      **행은 열어 둔다** — 145 · 146 을 닫고 `p3:measure` 가 **두 번 연속** 3/3 일 때 체크한다.
      ⑦ ✅ **145 를 닫았다** (84바퀴 · `9a5da46`): 러너가 기존 항목 id 를 `structureDocument({takenIds})` 로
      넘겨 같은 slug 후보를 `_2` 로 가른다. 재려니 goals.md 가 셋 다 `AI_OUTPUT_INVALID` — 인용이 줄을
      넘으면 `indexOf` 가 못 찾았다 (FINDINGS **147** 구멍 · 같은 바퀴에 닫음 · 공백을 접어 찾는다).
      그 뒤 진짜 Gemini 같은 코드로 두 번: goals 항목 **24 · 18** · 인용 **28/28 · 22/22** · accept 거절
      **0 · 0** · 충돌 **4/3 · 4/3**(의도된 셋 + 5회 vs 3회) · 장부 3행. **두 번 연속 3 이상**은 봤지만
      146(후보의 scope)이 열려 있어 **행은 아직 열어 둔다** — 146 을 닫고 다시 두 번이면 체크한다.
      ⑧ ✅ **146 을 닫고 두 번 연속 쟀다 — 행을 닫는다** (85바퀴 · `f487d67` · `b1f7d3c` ·
      `docs/evidence/2026-09-07-p3-gemini/probe.txt` 85바퀴 절): 후보를 scope 로 거르지 않는다(같은 type 이면
      후보 · scope 는 프롬프트 줄로). 146 만 고친 코드 세 번 중 셋째가 goals 에서 죽어(모델이 인용에서 `**` 를
      뺐다 · FINDINGS **148** 구멍 · 같은 바퀴에 닫음) 146+148 코드로 다시 두 번: goals 항목 **17 · 25** · 질문 4 · 4 ·
      인용 **21/21 · 29/29**(전부 원문) · 재시도 0 · 0 · accept 거절 0 · 0 · 후보 **6/6 · 6/6** · 충돌 **4 · 5**
      (의도된 셋이 두 번 다 · 전부 contradiction high · 질문형) · 장부 3행 = 왕복 3. run1 은 goals 의 roadmap·domain
      넷이 `project` 가 아닌데도 후보 6 — 146 이 실제로 갈린 실행. ⚠ 완료 기준 밖에 남은 것: **144**(실패 경로의
      이름 · finishReason·429) · **149**(첫 응답 id 패턴 위반 → 재시도 → 항목 11) — 대장에 남긴다. 화면 3 은
      브라우저로 안 봤다(P4 둘째 행의 눈 판정).
...
- [x] **AGENTS/cursor 타깃 · Pack zip · 터미널 재생 컴포넌트** — `8faad6a` `2a1db06` `8c53a89`
      ...
      **눈으로 봤다** — `docs/evidence/2026-09-07-landing-shots/`: 1440×900 의 **접히기 전**에 캡처
      ...
      낱말도 주소도 `PROJECT_SCREENS` 에서 온다).
      🔴 **넷을 동시에 잰다**: ① **빈 창** — 새 브라우저 컨텍스트(시크릿과 같은 것)라
      앞 캡처가 받아 둔 게스트 세션이 안 따라온다 (`localStorage` **0칸**)
      ② **링크만으로** — `Page.navigate` 는 첫 `/` **한 번뿐**이고 나머지는 다 눌렀다
      (횟수를 세서 잠근다 · 막힌 걸음을 주소로 건너뛰면 게이트가 없어진다)
      ③ **진짜로 그려졌다** — 껍데기가 아니라 skeleton 0 까지 ④ **3분 안**(`GATE3_BUDGET_MS`).
      **잰 것**: 총 **4.8초 / 180초** (랜딩→게스트 1.2s · Context 0.7 · 제안 0.7 · Roadmap 0.7 ·
      Sync 0.7) · navigate **1회** · 관통 1199 → **1233검사** · 시험 +10(`test/e2e-gate3.test.ts`).
      **눈으로 봤다** — `docs/evidence/2026-09-07-gate3/` (빈 창이 실제로 밟은 다섯 장 + `gate3.json`).
      ⚠ **재는 곳은 `next dev` 다** — 캡처가 화면을 다 데운 **뒤에** 밟는다(안 데우면 3분의 대부분이
