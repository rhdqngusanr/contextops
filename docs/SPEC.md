...
- 출력: 충돌 카드(판정 없는 질문) + 실제로 쓴 모델·토큰·값·걸린 시간. 질문 문장 안에 모델이 적은 항목 id 는 내기 전에 그 항목의 「제목」으로 바꾼다(`nameItemsInQuestion` · 긴 id 먼저 · 표에 없는 id 는 그대로 — 기록 카드가 적어 둔 것과 같은 예외 · 2026-09-13 production 실측에서 모델이 id 를 문장에 적었다). 🔴 **저장하지 않는다** — 방문자가 누를 때마다 샘플 팀의 정리 화면이 늘면 다음 사람의 데모가 망가진다. 화면은 정리 화면 머리의 체험 칸이다(`components/demo-ai-once.tsx` · 게스트에게만).
