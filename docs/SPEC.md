...
**슬로건:** 팀의 기억과 AI의 기억을 한 방향으로. (2026-09-10 · 정본 `apps/web/src/lib/web/site.ts`)
...
| GET /projects/{id}/jobs | member | ?feature&status&limit&offset → {jobs[] (**`shape:'summary'`** — `result` 없음 · `progress`·`updated_at`·`stalled`·`requeues` 는 있다), limit, offset} — **최신순.** job id 는 `POST /documents`·`batch-draft` 의 응답에만 있어서, 이 문이 없으면 화면 3 이 새로고침 한 번에 도는 job 을 잃는다 (FINDINGS 58). `?feature=structure&limit=1` 이 「마지막 구조화 job」이다 |
| GET /projects/{id}/jobs/{jobId} | member | → {**shape:'full'**, id, feature, status, progress, input, result, error_code, **requeues**, started_at, finished_at, updated_at, **stalled**} — 화면 3 의 polling (§9). 도는 동안 갈리는 값은 `status`·**`progress {done,total,unit}`**·`updated_at` 이다 (§2). 🔴 **`stalled` 는 서버가 내는 판정**이다 — 「끝나지 않았는데 `updated_at` 이 그 기능의 `stallAfterSec` 을 넘겼다」. 잣대가 서버 전용 표(`AI_JOB_RUNNERS`)에 있고 화면의 시계는 서버와 어긋나므로 재는 쪽이 시각 둘을 다 가진 서버다. 근거(`updated_at`)를 판정 옆에 같이 낸다. 남의 프로젝트 job 은 없는 job 과 같은 404 다 |
| POST /projects/{id}/jobs/{jobId}/items | member | {item_ids[]} → 201 **`ContextItemsBatchDraftResult`** = {accepted[{index,id}], rejected[{index,issues}], **`job_id`**} — 🔴 **§7.1 이 낸 항목 후보가 항목이 되는 유일한 문이다** (§7.1 · FINDINGS 84). 🔴 **받아들인 항목이 있으면 §7.2 탐지 job 을 시작한다** (`batch-draft` 와 같은 줄 · 없으면 `job_id: null` · FINDINGS 174) — 이 문이 탐지를 안 부르던 동안 웹에서 문서만 올리는 팀은 AI 가 찾은 충돌 카드를 한 장도 못 봤다. 고른 id 만 `status:'draft'` · `origin:'doc'` 로 들어간다. 본문은 요청이 아니라 **job 의 `result` 에서** 온다 — 요청이 본문을 실으면 화면이 모델 출력을 고쳐 되보낼 수 있고 그 항목의 근거는 여전히 원문 구간을 가리킨다 (P7). 구조화 job 이 아니거나 아직 안 끝났으면 `VALIDATION_FAILED`, 남의 프로젝트 job 은 없는 job 과 같은 404 다. ⚠ 항목별로 갈라 받는다 (`batch-draft` 와 같은 이유) — 후보 하나가 계약과 안 맞는다고 나머지를 못 받으면 그 문서는 통째로 막힌다 |
| POST /projects/{id}/jobs/{jobId}/retry | member | → job(**`shape:'full'`** · `status:'queued'`) — 🔴 **막힌 job 을 다시 굴리는 유일한 문이다** (FINDINGS 59 · 154). 이 문이 없을 때 예산 초과·빈도 초과로 죽은 job 에 사람이 할 수 있는 일은 **문서를 다시 올리는 것**뿐이었고, 그러면 같은 문서가 두 벌 생겨 §7.2 가 서로를 `duplicate` 로 잡는다. 🔴 **아무 실패나 다시 굴리지 않는다 — 되는 코드의 정본은 `ERROR_STATUS[code].retryable`**(`packages/schema` · 지금 `true` 인 넷은 `BUDGET_EXCEEDED`·`RATE_LIMITED`·`INTERNAL`·`AI_OUTPUT_INVALID`)이고 **화면 3 의 [다시 시도] 버튼이 같은 표를 읽는다**(`canRetryJob`) — 조건을 화면이 다시 적으면 느슨한 쪽이 이겨서 그린 버튼이 400 을 받는다 (`PROPOSAL_DECISIONS`·`ACTOR_RULES` 와 같은 모양). `AI_OUTPUT_INVALID` 는 2026-09-09 에 `true` 로 뒤집었다 (INBOX G8) — 실측에서 같은 문서의 약 1/3 이 비결정으로 이 코드로 끝났고 다음 판은 지났다. 대신 **같은 행은 `MAX_JOB_REQUEUES`(2)번까지만 되돌린다** — `ai_jobs.requeues` 를 `requeue` 의 조건부 UPDATE 가 세고, `jobRetryMode` 가 화면·서버 양쪽에서 같은 상한을 읽으며, 상한에 닿은 화면은 버튼 대신 「문서를 더 짧게 나눠 새로 올려보세요」를 말한다 (P3). 🔴 **갈래는 상태가 정한다 — `AI_JOB_RETRY_RULES`**(`packages/schema` · FINDINGS 154): `failed`+되는 코드 → `requeue`(같은 행을 `queued` 로) · **`running`+`stalled` → `fresh`**(그 행을 `failed`+`INTERNAL` 로 닫고 **같은 `input` 으로 job 을 하나 더** 만든다 · 응답은 그 **새 행**이다) · `queued`·`succeeded` → 다시 굴릴 수 없다(400). ★ 왜 `running` 을 되돌리지 않나 — `runJob()` 은 `queued` 만 집으므로 되돌리면 아직 살아 있을지 모르는 러너와 **둘이 같은 job 을 굴려** LLM 왕복이 두 배가 된다 (P3). 새 행을 만들면 늘은 러너가 살아 있어도 **자기 행에만 쓴다.** ⚠ 멈추지 않은 `running` 은 400 이다 — 도는 중인 일을 사람이 죽이면 안 된다. ⚠ 멈춘 행의 `progress` 는 남긴다(「4조각 중 1에서 멈췄습니다」가 그 행의 참말이다) · 새 행은 자기 진행률을 처음부터 센다. ⚠ 되돌릴 때 `started_at`·`finished_at`·`result`·`error_code` 를 **전부 비운다**(`AI_JOB_STATUS_RULES` 의 CHECK 이 강제한다) — `progress` 도 같이 비운다: 남기면 아직 아무것도 안 한 job 에 **지난 판**의 막대가 그려진다. ⚠ 되돌리는 UPDATE 에 `status='failed'` 조건이 달려 있어 **둘이 동시에 눌러도 한 번만 굴러간다**(`runJob()` 의 집기와 같은 모양). 예산은 여기서 미리 세지 않는다 — 다시 굴린 job 도 `withBudget()` 을 지나므로 아직 안 풀렸으면 같은 코드로 다시 실패한다. 남의 job 은 없는 job 과 같은 404 다 |
...
| POST /projects/{id}/ask | member | {question} → {answer, cited_item_ids[]} (§7.3, 예산 가드) — ⛔ **아직 없다** (2026-09-12 확인). 라우트가 0곳이고 `withBudget('ask')` 호출처도 0곳이다. 안 도는 기능이라는 사실의 정본은 `lib/ai/features.ts` 의 `AI_FEATURES_NOT_WIRED` 이고 `test/ai-features-wired.test.ts` 가 양쪽으로 잠근다 — 이 문을 만들면 그 줄을 지워야 초록이다 |
| POST /demo/session | **공개** | → `{access_token, expires_at, entry_path}` — 🔴 **인증 없이 부르는 유일한 쓰기 문**이다. 데모 팀·데모 프로젝트·게스트 `users` 행·그 소속이 **넷 다** 있을 때만 200 이고, 하나라도 없으면 `NOT_FOUND` 다 (「일단 토큰은 주고 화면에서 404 를 보게」 하면 심사위원이 보는 것은 빈 화면이고 원인은 화면에 안 적힌다). 행을 **만들지 않는다** — 게스트도 팀도 시드가 만든다. 응답에 이메일·사람 이름이 없다. 🔴 **빈도 제한은 IP 당 10분에 200회다** (2026-09-12 에 20 으로 걸었고 2026-09-13 에 올렸다 — 한 NAT 뒤의 사람들이 같이 막혔다 · `lib/api/rate-limit.ts` 의 `HTTP_RATE_LIMITS`). 예전엔 없었고 그 근거가 「LLM 을 안 부르고 행을 안 만든다」였는데 **그 셈이 틀렸다 — 돈이 드는 것은 LLM 만이 아니다**: 자격증명 없이 부를 수 있는 문이 무제한이면 Vercel 호출 수와 Supabase 연결이 그대로 올라가고, 심사 기간에 링크가 죽는 것은 규정상 제외 사유다 |
| POST /demo/ai-once | **공개** | `{preset}` → `{preset, conflicts:[{kind, severity, question, other}], compared, model, input_tokens, output_tokens, cost_usd, duration_ms}` (§7.4 · 2026-09-13) — 🔴 **인증 없이 부르는 둘째 쓰기 문**이다(`ctx.actor()` 를 안 부른다 — `/demo/session` 과 같은 모양). `preset` 은 **고른 메모의 이름**뿐이고(정본 `apps/web/src/lib/demo/ai-presets.ts` · 계약 `DemoAiOnce` 는 `.strict()` 라 본문을 같이 실으면 400) 표에 없으면 400, 데모가 안 심겼으면 404. `other` 는 부딪힌 적용 중 규칙의 `{id, title}` 이거나 null. **아무것도 저장하지 않는다** — 남는 것은 `ai_usage` 한 줄(IP 는 sha256)과 빈도 표뿐이다. 상한 둘: 사람은 라우트 감싸기가 IP 당 하루 10회(`HTTP_RATE_LIMITS`), 돈은 `withBudget('demo')` 가 샘플 팀 전체 하루 20회(`AI_FEATURE_LIMITS.demo`) · 월 천장 그대로 |
...
| GET /health | 공개 | {ok, db, **ai**, version} — `ai` 는 이 배포가 서버측 AI 를 부를 수 있나(키 + 표 안의 모델 · INBOX G9). `ok` 에는 안 섞는다 — AI 없는 배포도 읽기·발행·sync 는 되므로 200 이고, `verify:prod` 가 `ai` 를 따로 잰다 |

에러 코드: `UNAUTHORIZED`, `FORBIDDEN`, `NOT_FOUND`, `VALIDATION_FAILED`, `STALE_BASE`, `REVISION_CONFLICT`, `BUDGET_EXCEEDED`, `RATE_LIMITED`, `COMPILE_FAILED`, `INTERNAL`, `AI_OUTPUT_INVALID`, `AI_NOT_CONFIGURED`.
...
- 🔴 **질문 문장 안에 모델이 적은 항목 id 는 저장하기 전에 그 항목의 「제목」으로 바꾼다** (`nameItemsInQuestion` · `askModel` 안 · FINDINGS 175). 짝을 가리키는 칸(`a_item_id`·`b_item_id`)은 그대로 id 다 — 바뀌는 것은 사람이 읽는 문장뿐이고 역추적(P7)은 칸이 잇는다. 긴 id 먼저 · 따옴표째 · 프롬프트에 없던 id 는 그대로다. ★ 왜 — 2026-09-15 production 정리 화면의 「AI 가 올린 질문」에 `item_refund_within_24_hours` 가 그대로 떴다. 같은 변환이 §7.4 체험 라우트에만 있었다. ⚠ 바꾼 문장은 500자(`Question`)를 넘을 수 있다 — 그 상한은 모델 출력의 계약이고 저장 칸(`text`)과 화면에는 없다.
...
- 🔴 **부르는 자리는 `ai_jobs` 의 `conflict` job 하나다** (§2 · `AI_JOB_RUNNERS.conflict`). 항목이 들어오는 두 문 — 플러그인의 `batch-draft` 와 화면 3 의 후보 받기(`POST /projects/{id}/jobs/{jobId}/items`) — 이 **받아들인 항목이 있을 때만** 만든다 (FINDINGS 174 — 둘째 문이 빠져 있던 동안 웹에서 문서만 올리는 팀에는 탐지가 한 번도 안 돌았다). 낸 것을 행으로 옮기는 것도 그 러너다 — `detectConflicts()` 자신은 DB 에 쓰지 않는다.
...
### 7.4 데모 "AI 한 번" `detectDemoConflicts(preset)` — 2026-09-13
- 입력: 고른 메모 한 줄(`apps/web/src/lib/demo/ai-presets.ts` 의 표 · 자유 입력 없음)을 <b>「바뀐 항목」</b>으로 삼고, 샘플 팀의 같은 type **적용 중** 항목(최대 40)을 기존 항목으로 싣는다. 🔴 **§7.2 와 같은 프롬프트 · 같은 검증**(`askModel` 하나 · 실리지 않은 id 는 재시도)이고 다른 것은 예산 줄(`withBudget('demo')`)뿐이다 — 데모가 제품과 다른 AI 를 보여 주지 않게.
- 출력: 충돌 카드(판정 없는 질문) + 실제로 쓴 모델·토큰·값·걸린 시간. 질문 문장 안에 모델이 적은 항목 id 는 §7.2 와 **같은 자리**(`askModel` 의 `nameItemsInQuestion`)에서 그 항목의 「제목」으로 바뀐다(긴 id 먼저 · 표에 없는 id 는 그대로 — 기록 카드가 적어 둔 것과 같은 예외 · 2026-09-13 production 실측에서 모델이 id 를 문장에 적었다 · 이 문에만 있던 동안 저장되는 카드는 id 를 담았다 — FINDINGS 175). 🔴 **저장하지 않는다** — 방문자가 누를 때마다 샘플 팀의 정리 화면이 늘면 다음 사람의 데모가 망가진다. 화면은 정리 화면 머리의 체험 칸이다(`components/demo-ai-once.tsx` · 게스트에게만).
- 상한: 한 IP 하루 10회(라우트 감싸기) · 샘플 팀 전체 하루 20회(`AI_FEATURE_LIMITS.demo` · 탐지 한 번 ≈ $0.01) · 월 천장. ⚠ 원래 이 절은 「픽스처 항목으로 카드 3장 · 24h 캐시 · 게스트 IP당 일 5회」였다 — 캐시한 결과는 또 하나의 기록물이라 「AI 가 도는 장면」이 못 되고, IP당 5회는 한 사무실(NAT) 뒤의 심사위원을 같이 막아서 바꿨다.
...
- `stop.mjs`: `git diff --name-only HEAD` + untracked → manifest.milestones.paths와 glob 대조 → 해당 마일스톤에 `in_progress` 이벤트(source:hook, evidence는 path만) — 단 이번 세션에 progress 보고가 이미 있으면 생략(로컬 `cache/progress-<session>.json`으로 판단). **보낸 뒤에는 훅 자신도 같은 표시 파일을 남긴다.** 정책·아키텍처 관련 경로(`migrations/**`, `infra/**`, `*.config.*`, rules에 scoped된 paths)가 바뀌면 `pending-proposal.json`에 {changed_paths, hint} 저장. LLM 호출 없음.
  🔴 **Stop 은 세션 끝이 아니라 Claude 가 한 턴을 마칠 때마다 돈다** (INBOX G7 · 2026-09-09). 표시 파일이 없으면 턴마다 이벤트가 하나씩 쌓여 근거 수를 부풀린다 — 그래서 한 세션에 한 번만 보내고, 두 번째 턴부터는 표시를 보고 물러선다(`test/hooks.test.ts` 「두 턴 → 보고 1건」). 표시는 agent 가 `/contextops:progress` 로 보고할 때도 같은 이름으로 남는다(CLI 의 `--session` · env `CLAUDE_CODE_SESSION_ID` — Claude Code 의 Bash 환경이 주는 값이고 훅 stdin 의 `session_id` 와 같다 · 2026-09-10 실측). 기다리는 시간의 합(git 2회 + 네트워크 1회)은 hooks.json 의 timeout 보다 짧아야 하고 같은 시험이 합을 잰다.
  ⚠ **이 쓰기가 P6의 예외가 아니라 P6의 정의다** — `pending-proposal.json`과 `cache/progress-<session>.json`은 `.contextops/`의 ignore 목록 안이라 git이 그 변화를 보지 못한다(우리가 `.contextops/.gitignore`를 만든다). 훅은 사용자의 파일을 한 바이트도 바꾸지 않는다. 쓸 수 있는 경로는 `hooks/hooks.json`의 `_writes`에 선언한 둘뿐이고, 선언 밖으로 나가면 `test/hooks.test.ts`가 빨개진다.
...
- 게스트는 데모 팀의 **member** 로 앉는다. 그래서 owner 전용 화면 요소(로드맵 「완료 확인」)는 버튼 대신 「…완료 확인은 팀장만 할 수 있습니다.」 한 줄로 그려진다 (`components/roadmap.tsx` · 화면의 낱말은 「owner」가 아니라 「팀장」이다). 게스트는 같은 자리에서 「팀장만」 대신 **읽기 전용 이유**를 본다 — 쓰기 문(`writeDoor()`)이 닫혔으면 그 이유가 등급 문장보다 먼저다 (`confirmBlockedText` · FINDINGS 180 · 2026-09-15 고침 · Context 드로어의 `editCaption()` 과 같은 틀).
