...
_마지막 갱신: 2026-09-04 · 루프 32바퀴 · `1aebc22`_
...
🔴 **맨 위는 `80` 이다 — 이번 바퀴에 나온 [고장]이고, 대기 중에 유일한 고장이다.**
심각도 순서는 언제나 **고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3).
(**79** 는 `1aebc22` · **76** 은 `a39419c` · **74** 는 `094102b` · **71** 은 `a201a51`.)

**80 — 승인된 항목이 0건이어도 발행이 201 이다** (고장). 나오는 Pack 은
`.claude/rules/workflow.md` 한 장뿐인데 **공식 버전이 그리로 옮겨 가고** 기기들이
그것을 `applied` 로 받아 간다. 사람은 자기가 답한 열 문장이 배포됐다고 믿는다.

**시작하기 전에 아는 것 (80):**

- 🔴 **막으려던 가드가 이미 있다. 조건이 틀렸을 뿐이다.**
  `packages/compiler/src/compile.ts:57` 의 `EMPTY_SNAPSHOT` 은 「Pack 파일이 0개」를
  재는데, `assemble.ts:55` 가 `always` 문서(`workflow.md`)를 **먼저** 넣어서 0 이 될 수
  없다. 재야 하는 것은 파일 수가 아니라 **항목 수**다.
- 🔴 **갈래는 둘이고 둘 중 하나만 골라라.**
  ① 컴파일러가 막는다 → 발행이 500 `COMPILE_FAILED`. ⚠ 5xx 는 「서버가 터졌다」로
     읽힌다 — 사람 잘못을 5xx 로 내지 마라.
  ② **발행 라우트가 막는다** (이쪽이 맞아 보인다) → `active` 항목 0개면 400
     `VALIDATION_FAILED`. 문구는 「승인된 항목이 없습니다 — Context 에서 초안을 승인한
     뒤 발행하세요」. 화면 5 에 **이제 그 문이 있다** (79 가 만들었다).
- ⚠ **어느 쪽이든 `EMPTY_SNAPSHOT` 을 살리거나 지워라.** 지금은 소비처 0곳·시험 0건인
  죽은 코드다 (FINDINGS 81 ④).
- ⚠ **게이트를 「400 이 난다」로 끝내지 마라.** 「승인 0건으로 만든 버전이 **공식이 되지
  않는다**」까지 재라 — 막는 자리가 라우트 한 곳이 아닐 수 있다.
- ⚠ **이번 바퀴의 시험 하나가 그 201 을 지금 사실로 적고 있다** —
  `api-seed-questions.test.ts` 「승인하기 **전에는** 내 답이 Pack 에 하나도 없다」.
  80 을 고치면 그 `expect(res.status).toBe(201)` 이 먼저 빨개진다. **그게 맞다** —
  그 자리가 「무엇을 바꾸기로 했나」를 다음 사람에게 보여 주는 자리다.
