...
_마지막 갱신: 2026-09-04 · 루프 39바퀴 · `800372f`_
...
🔴 **맨 위는 `92` 다 — 이번 바퀴에 [구멍]이 하나 나왔다.** 심각도 순서는 언제나
**고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3). 그 다음은 93 · 87 · 91 (셋 다 격차).
(**90** 은 `800372f` · **89** 는 `5858fbb` · **88** 은 `6f06881` ·
**86** 은 `c6905df`+`b1d2190` · **84**·**31** 은 `c57b3fb` · **82**·**65** 는 `5fe0068` ·
**80** 은 `f9962cb` · **79** 는 `1aebc22`.)

**92 — `tsc` 가 `apps/web/scripts/` 를 한 번도 안 본다** (구멍).
`apps/web/tsconfig.json` 의 `include` 가 `["*.ts", "src", "test", …]` 인데 `"*.ts"` 는
**맨 위 한 층**이다. `seed.ts`·`walkthrough-publish.ts`·`dev-server.ts`·`dump-*.ts` 가
전부 검사 밖이다 — **관통을 만드는 코드가 typecheck 층에 없다.**

**시작하기 전에 아는 것 (92):**

- 🔴 **이미 재 놨다. 다시 재지 마라.** `cd apps/web; npx tsc --noEmit --listFiles |
  grep -c "scripts/"` → **0**. `include` 에 `"scripts"` 한 줄을 넣고 돌린 결과는
  **에러 3개뿐이고 전부 미사용 변수(TS6133)**다:
  `dump-resolve-effect.ts(74,10) 'loser'` · `walkthrough-publish.ts(19,58) 'sessionJwt'` ·
  `walkthrough-publish.ts(128,15) 'db'`. **타입 에러는 0개다** — 켜는 값이 싸다.
- ⚠ `include` 는 `tsc` 와 **next 가 같이 읽는다.** `next build` 가 아파하면
  `tsconfig.scripts.json` 을 따로 두고 `ci.ps1` 의 `typecheck` 층이 두 프로젝트를
  다 돌게 하는 것이 다음 갈래다.
- ⚠ `packages/*` · `plugin` 의 `include` 도 같이 봐라 — 같은 모양이면 거기도 빠져 있다.
