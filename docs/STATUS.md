...
_마지막 갱신: 2026-09-06 · 루프 58바퀴 · (이 커밋)_

---

## 지금 어디인가

**이번 바퀴는 `docs/PLAN.md` **P4 둘째 행의 둘째 조각**을 만들었다 — **게스트 데모 테넌트**다.**
...
(54~57바퀴와 같은 이유다. 그 줄의 모양을 넓히는 것이 FINDINGS **109** 다).

> **다음 바퀴의 일 = `docs/PLAN.md` P4 **둘째 행의 마지막 조각** — 「랜딩 v1」**
> 게스트 데모는 됐다. **랜딩(`/`)의 [샘플 팀으로 둘러보기] 버튼이 이제 갈 곳이 있다** —
> `/demo` 다. 정본은 `docs/DESIGN_BRIEF.md` §4 「화면 1」 (헤드라인 · Before/After 두 열 ·
> 설치 4줄 · 신뢰 경계 표) 과 SPEC §9 표의 1행.
> ⚠ **Before/After 의 예시 문구는 목업이다** — 픽스처 정본은 paylab 이다 (§10.1 ·
> DESIGN_BRIEF §4 화면 1 의 주의 두 줄). **paylab 데이터로 채워라.**
> ⚠ 랜딩은 **정적**이다 (SPEC §9 표의 「상태」 칸). 세션을 읽지 마라 — 읽는 순간
> 랜딩이 로그인 상태에 따라 두 모양이 되고, 시크릿 창에서 보는 첫 화면이 갈린다.
> ⚠ 「2분 영상 보기」 버튼은 **영상이 없으면 만들지 마라** (P6 의 일이다).
> 누르면 아무 일도 안 하는 버튼을 두지 않는 것이 이 저장소의 규칙이다.
> ⚠ 그 행을 닫으려면 **GATE 3 을 사람이 시크릿 창에서 한 번 밟아야 한다** —
> 이 환경에 브라우저가 없다 (「눈 판정 대기」). `pnpm --filter web demo:db` 가
> 그 환경을 띄운다 (PGlite + 데모 테넌트 + `/demo`).

- PLAN 의 `- [ ]` 중 **위의 둘은 사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키).
- 대장의 대기(121 · 120 · 119 · 118 · 117 · 116 · 115 · 114 · 112 · 111 · 108 · 25 · 33 …)는
