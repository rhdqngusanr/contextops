...
_마지막 갱신: 2026-09-06 · 루프 54바퀴 · `130b5f2`_

---

## 지금 어디인가

**이번 바퀴는 제품을 새로 만들지 않고 쟀다 — 「화면에서 항목을 새로 만들 수 없다」
(FINDINGS **35**)를 재고 닫았고, 같이 재던 PLAN P3 둘째 행(웹 화면 3·4)도 닫았다.**

🔴 **왜 재는 것이 한 바퀴의 일인가.** 53바퀴가 대장의 낡은 상태 둘(25·35)을 찾아
`107` 로 적으면서 「**재지 않고 닫지 마라**」를 같이 적었다 — 56 이 그렇게 틀린 사실을
대장에 남긴 적이 있다. 35 의 닫는 조건(「화면 3·4 가 생기면 닫힌다」)은 곧 PLAN P3
둘째 행의 완료 기준(「문서 없이 질문만으로 v1.0 발행 가능」)이라, **한 번 재면 둘이
같이 결정된다.**

🔴 **잰 결과 — 화면이 항목을 만드는 문은 셋이고, 셋 다 Pack 까지 간다.**

| 문 | 무엇을 누르면 | 그 줄의 근거 (P7) |
|---|---|---|
| A | 씨앗 질문에 답한다 (문서 0건) | `manual:<씨앗 질문 문장>` |
| B | 열린 질문에 **자리를 골라** 답한다 (`save_as`) | `manual:<그 질문>` — 원문 구간을 안 물려받는다 |
| C | 구조화 후보를 받아들인다 (`jobs/{id}/items`) | `doc:{version}#0-40` |

🔴 **문 쪽 시험들은 「행이 생기는 데까지」만 쟀다.** 사람이 원하는 것은 행이 아니라
**배포된 Pack 의 한 줄**이고 그 사이에는 승인·컴파일·역추적이 있다 — 문 하나가 Pack
앞에서 끊겨도 문 쪽 시험은 초록이다. 그래서 **셋을 한 프로젝트에서 섞어 지나 발행까지
가는 자리**를 새로 뒀다 (`apps/web/test/web-item-doors.test.ts`).

**잰 것:**

| | 전 | 후 |
|---|---|---|
| 화면만으로 만들 수 있는 항목 | (35 를 쓸 때) **0개** | 문 셋 · 한 프로젝트에서 **3개** |
| 문 → Pack 까지 재는 시험 | 문 A 만 (`api-seed-questions`) | **셋 다** (`web-item-doors` 4개) |
| 문 A 로 만든 v1.0 (⑧ 이후 첫 재측정) | 2026-09-04 산출물 | **본문 한 글자도 안 다름** |
| PLAN P3 둘째 행 | `- [ ]` (⑩까지 열려 있었다) | **`- [x]`** |
| 관통 검사 | 666 | **670** |
| CI | — | principles OK 9 · typecheck · test · build · walkthrough **670** · docs → GREEN |

**빨개지는 것을 봤다** — 라우트의 `if (!answer.save_as) continue` 를 `continue` 로
바꾸니 새 파일 **4개가 전부** 빨개진다 (문 B 가 항목을 안 낳으면 그 뒤가 다 무너진다).
되돌렸다.

**눈으로 읽었다** — `docs/evidence/2026-09-06-item-doors/doors-pack.txt`.
세 줄이 `## Mission` · `## Policies (must follow)` · `## Constraints` 로 갈려 나가고
태그가 셋 다 다르다. `questions-only-rerun.txt` 는 2026-09-04 것과 **본문이 같다**
(다른 것은 `snapshot_hash` 뿐이고, 그건 `project_id` 가 지문에 들어가서다 —
`snapshotHash()`. 고장이 아니다).

🔴 **재다가 하나 나왔다 (FINDINGS 108).** `CONFLICT_KIND_RULES` 의 `answerSlot` 을
`open_question` 에서 `'none'` 으로 뒤집으니 **화면 시험 7개는 빨개지는데 API 시험은
하나도 안 빨개진다** — 라우트가 `slot === 'seeded'` 인지만 보고 나머지를 한 갈래로
묶는다. 즉 세 값 중 `none` 은 서버에서 아무것도 안 바꾼다. ⚠ **지금은 닿을 수 없다**
(답할 수 있는 종류가 `seeded`·`ask` 둘뿐) — 그래서 고장이 아니라 구멍이다.

**2-B 54바퀴 라운드는 이 재기 자체였다** — `answerSlot` 3종을 뒤집어 「값을 바꾸면
결과가 달라지나」를 물었고, 셋 중 하나가 서버에서 죽어 있는 것을 찾았다 (108).
...
`loop/PROMPT.md` ④3 은 **PLAN 이 FINDINGS 보다 위**이고, 이번 바퀴에 P3 둘째 행이
닫히면서 루프가 움직일 수 있는 맨 위 행이 바뀌었다:

> **다음 바퀴의 일 = `docs/PLAN.md` P4 첫 행 — 「웹 화면 6·8 (Proposal · Roadmap · Realtime)」**
> 완료 기준은 그 행에 적혀 있다. SPEC §9 화면 6·8 과 `docs/DESIGN_BRIEF.md` 를 먼저 읽어라.

- PLAN 의 `- [ ]` 중 **위의 둘은 사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키).
- 대장의 대기(108 · 25 · 33 · 34 · 36 …)는 **PLAN 을 막지 않는다** — 적어 두고,
  그 항목의 주인이 될 PLAN 행을 할 때 같이 닫는다 (④3 ②).
- ⚠ 「다음은 PLAN 행이다」를 이 줄의 **모양으로는 말할 수 없다** — 그것을 FINDINGS
  **109** 로 적었다 (게이트의 정규식이 `FINDINGS` 를 글자로 박고 있다).
