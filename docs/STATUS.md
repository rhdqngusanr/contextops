...
_마지막 갱신: 2026-09-06 · 루프 51바퀴 · `69c9a80`_

---

## 지금 어디인가

**이번 바퀴는 질문에 답해서 만든 항목이 그 질문과 이어지지 않던 구멍을 닫았다 —
...
🔴 **대장의 ①을 그대로 하지 않았다 — 그게 이번 바퀴의 판단이다.**
...
🔴 **고치는 동안 더 큰 구멍이 나왔다 (FINDINGS 105).** 이번에 근거를 채워 준 그 길로
...
`SourceRef` 는 **쓰고 되읽는 왕복**(`srcTag` → `srcKindOf`)까지 잠근다. 이번 바퀴의
...
**다음 바퀴의 일 — FINDINGS 105** · [구멍] · 「열린 질문에 답하면 답만 남고 항목이 안 생긴다」
...
🔴 **왜 87(격차)이 아니라 105 인가 — `loop/PROMPT.md` ④3 이 바뀌었다** (`778c461`).
**PLAN 이 FINDINGS 보다 위**이고, `구멍`·`격차` 는 PLAN 을 막지 않는다. 그래서 고르는
기준은 「대장 맨 위」가 아니라 **「PLAN 의 다음 행을 여는 것」**이다.
- PLAN 의 `- [ ]` 맨 위 둘은 **사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키) —
  루프가 할 몫은 이미 끝났다 (`389c7f2` · P3 ①②③).
- 그래서 루프가 움직일 수 있는 맨 위 행은 **「웹 화면 3·4」**이고, 그 행이 주인인 것이
  56(닫음) · **105**(구멍) · 59 · 63(격차)다. 심각도 순서로 **105 가 다음**이다.
- 87 은 화면 5·7 이 주인이라 그 행을 열지 않는다. 값은 그대로 남아 있다.

⚠ **105 를 고르기 전에 SPEC §5 를 먼저 읽어라.** 지금 SPEC 은 「초안이 안 오면 답만
기록하고 질문을 닫는다」라고 **적어 두고 있다** — 즉 이 구멍은 **의도된 것으로 적혀 있다.**
고칠 방향 셋(화면이 타입을 고른다 · 서버가 옮긴다 · 못 한다고 말한다) 중 무엇을 골라도
**SPEC 을 같이 고쳐야 한다.** 셋의 값과 대가는 105 에 적어 뒀다.
