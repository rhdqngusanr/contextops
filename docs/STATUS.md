...
**다음 바퀴의 일**: 대기는 넷이다. 심각도 순서는 **고장 → 구멍 → 격차**이므로
맨 위는 **102**([고장] · 위) 하나다. 그 다음이 [격차] 셋 — **99**(아키텍처 블록이 같은
...
🔴 **맨 위는 `97` 이다 — 대기 중인 [고장]은 없고 [구멍]은 97 하나다.** 심각도 순서는 언제나
**고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3). 그 다음은 **96**·**98**·87·91 (전부 격차).
(**94** 는 `8b96ef5` · **95** 는 `843bfd1` · **93** 은 `ed30a89` · **92** 는 `95c525e` ·
...
이번 바퀴에 밟았다). `.ci/walkthrough-pack/.claude/rules/domain-refund.md` 는
「`# 도메인`」 → 「`## 이 도메인의 규칙`」 → 규칙 한 줄뿐이고 **「refund」라는 낱말이
파일 안에 없다.** 아는 길은 파일 이름뿐이라 발췌·인용하는 순간 사라진다.

**시작하기 전에 아는 것 (97):**

- 🔴 **재료는 이미 채워져 있다.** `DocVars.title` 이 `partition.ts:83`(`byScope`)에서
  도메인 이름으로 채워지는데 `templates/index.ts` 의 `DOCS.domain.head` 가 **안 쓴다.**
- 🔴 **머리말에 무조건 찍는 것으로 고치지 마라.** `domain` ItemType 항목이 있는 파일
  (`domain-payment.md`)은 절이 이미 `## payment` 를 찍는다 — 이름이 두 줄 겹친다.
  그 경고가 `DOCS.domain.head` 주석에 적혀 있고, **그 판단은 항목이 있을 때만 맞다.**
  값싼 갈래는 머리말을 `# 도메인 — {title}` 로 하는 것이다 — `scoped` 가 이미
  `# 경로 규칙 — {title}` 이라 **문서들의 모양이 오히려 같아진다.**
- ⚠ **템플릿을 고치면 `TEMPLATE_VERSION` 을 올리고** golden 의 expected 를 갱신한 이유를
  커밋 메시지에 적어라 (`loop/PROMPT.md` ⑤). 말없이 덮으면 게이트가 게이트가 아니게 된다.
- ⚠ 이 항목은 **P7 위반이 아니다** — 태그는 멀쩡하다. 사람이 읽는 종이에서 「무엇에
  관한 규칙인지」가 빠진 것이다. 그러니 태그 쪽을 건드리지 마라.

**96 — 관통 스크립트 셋이 `check()` 를 각자 복사해서 들고 있다** (격차).
`walkthrough-publish.ts:45` · `walkthrough-payload.ts:42` · `walkthrough-sync.ts:34`.
⚠ 세 파일은 **패키지가 둘**이라(`apps/web` · `plugin/contextops`) 공용 자리를 만드는 것이
의존 방향에 개발 도구를 얹는 일이 된다 — **값이 그만한지 먼저 재라.** 값싼 갈래는
`checks` 배열의 **모양만** `packages/schema` 의 타입 하나로 못 박는 것이다.

**98 — 아키텍처 다섯 줄이 §7 그림의 흐름 순서를 잃고 이름 순으로 나온다** (격차 ·
이번 바퀴에 밟았다). 다섯이 전부 `priority: 60` 이라 제목 코드포인트 순으로 떨어진다.
⚠ **정렬(`compiler/src/sort.ts`)을 고치지 마라 — 그건 P4 의 심장이고 지금 맞다.**
순서를 말하는 자리는 `priority` 인데, 그 값이 「중요도」인지 「읽는 순서」인지를
**SPEC §3 에서 먼저 읽어라.** 뜻이 아니면 **닫지 말고 「그 값은 그런 뜻이 아니다」로 남겨라.**
