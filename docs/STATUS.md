...
_마지막 갱신: 2026-09-07 · 루프 101바퀴 · 코드 `8daf8a8` + `1839969`(FINDINGS 160 ✅ — 좁은 폭에서 껍데기가 내비를 접는다 · 시험 +4) · 눈 판정 근거 6장 + 진짜 브라우저에서 잰 폭 · 문서는 그 다음 커밋_

---

## 지금 어디인가
**이번 바퀴(101)는 FINDINGS 160 — 격차(앱 껍데기가 375px 에서 내비를 안 접어 본문이 76px 로 눌린다)를 닫았다** (`8daf8a8` + `1839969`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **여덟 바퀴째 같다**).
...
> 155 를 ✅ 로 닫고(커밋 해시는 그 세션의 것) 바로 153 으로 간다 ② 아직 빨가면 **고치지 말고**
> 155 에 한 줄만 더하고 153 을 한다. ⛔ **README·랜딩을 이 루프가 고치지 마라** — 문장의 정본은
> 랜딩 상수라 둘을 같이 고쳐야 닫히고, 사람이 그 세션에 지시하고 있다. 이번이 **열 바퀴째**다.
> ⚠ 155 가 빨간 동안은 **관통도 막힌다**(`api` 단계 = `pnpm --filter web test`). CI 의 `test` 층
> 결과로 대신 읽고, 「관통 못 돌림」을 STATUS 에 적어라.

> **왜 153 인가** — 격차 차례의 다음은 **131**(랜딩 첫 화면)인데 ⛔ **랜딩은 지금 남의 세션 몫**이다(155).
> 그래서 순서(고장 → 구멍 → 격차)의 **구멍**으로 간다: 156 은 배포 뒤에 재고 · 140 은 🙋 새 PC · 152 는
> 「지금은 행동할 수 없다」고 스스로 적어 뒀다 → 남는 것이 **153**이고, 그 항목은 이미
> 「**두 번 났으니 이제 관찰이 아니라 할 일**」이라고 적혀 있다.
>
> **153 을 하는 법** — ① ⛔ 상한을 그 시험 파일에 손으로 적지 마라. 136 이 정한 자리 **`vitest.base.ts`**
> 에 `testTimeout` 을 두고 **왜 그 수인지 한 줄**을 같이 적는다(`hookTimeout` 이 이미 거기 있다)
> ② **올리기 전에 재라** — 그 시험이 무엇을 기다리는지(자식 프로세스 기동인가 파일 I/O 인가)를 재고,
> 잰 수의 몇 배로 잡았는지 적어라. 재지 않고 올리면 진짜로 느려진 날에 아무도 모른다
> ③ 재는 법: `pnpm --filter contextops exec vitest run test/hooks.test.ts --reporter=verbose` 를 여러 번 돌려
> 그 두 시험(`연결 안 된 저장소에서는 아무 일도 하지 않는다` · `세션 id 를 모르면 보내지 않는다`)의 ms 를 모은다.
