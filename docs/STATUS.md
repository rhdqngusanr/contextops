...
_마지막 갱신: 2026-09-05 · 루프 47바퀴 · `03fb4f1`_

---

## 지금 어디인가

**이번 바퀴는 `STATUS.md` 가 자기와 어긋나던 것을 고쳤다 — 다음 할 일을 말하는 자리를
하나로 만들고, 그것을 세는 게이트를 세웠다** (FINDINGS **102** · `06dc1c8` + `03fb4f1`).

🔴 **무엇이 문제였나.** 이 파일은 「다음 바퀴의 유일한 기억」인데 **기억이 두 갈래**였다.
머리는 「99 가 맨 위」라고 했고, 733줄의 `## 다음 바퀴가 할 일` 절은 「🔴 **맨 위는 `97`
이다**」로 시작해 「시작하기 전에 아는 것 (97)」을 20여 줄 이어 갔다. **97 은 44바퀴에
닫혔다**(`7ba2feb`). `loop/PROMPT.md` ②표가 이 파일을 「**전체**」 읽으라고 지목하므로,
아래까지 읽는 바퀴는 **닫힌 항목을 다시 연다.** 두 바퀴가 그 절을 지나쳤다.

🔴 **왜 그 자리가 썩었나 — 손이 안 닿는 자리라서다.** 199,431바이트·2,250줄이었고 그중
900여 줄이 10~40바퀴 기록이었다. **같은 역사를 두 형식으로 두 곳에** 들고 있었고
(`### 지난 바퀴 (N)` 와 `**N바퀴 · `), `## 잰 것` 절이 **두 번** 있었다.
★ 크기가 고장이 아니라 **어긋남**이 고장이다. 크기는 그 어긋남이 생긴 **이유**다.

**잰 것:**

| | 전 | 후 |
|---|---|---|
| 다음 할 일을 말하는 자리 | **2곳** (머리 · 733줄 절) — 서로 다른 항목을 가리켰다 | **1곳** (머리 한 줄) |
| 그 자리가 가리키는 항목의 상태 | 아래쪽은 **닫힘**(97 · 두 바퀴 전) | **대기**임을 게이트가 센다 |
| `## ` 절 제목 | 7개인데 `## 잰 것` 이 **두 번** | **전부 다름** (게이트가 센다) |
| STATUS 크기 | 2,250줄 · 199,431바이트 | **1,062줄 · 82,238바이트** |
| 지난 바퀴 기록이 사는 곳 | STATUS 안에 **두 형식으로** 21블록 | STATUS 는 **최근 5바퀴** · 나머지는 `docs/history/cycles.md` |
| 이 고장을 세는 게이트 | **0개** | **7가지** (`tools/status-shape.mjs`) |
| CI | — | principles OK 9 · typecheck 9초 · test 67초 · build 18초 · walkthrough **645** · **docs OK** → GREEN |

**고친 방법은 「규칙을 더한다」가 아니라 「자리를 없앤다」였다.** 항목 자신이 그렇게 적어
뒀고, 그게 맞았다 — 절 이름이 「다음 바퀴가 할 일」인 한 그 절은 계속 지목을 끌어들인다.
그래서 이름을 `## 앞 바퀴들이 남긴 것 — 다음 사람이 알아야 하는 것` 으로 바꿨다.
⚠ **안에 있던 지식은 한 줄도 안 지웠다.** 지운 것은 97·96·98(셋 다 닫힘) 지목 38줄뿐이다.
그 절이 썩은 것은 **지식이 아니라 지목**이었다 — 둘을 같이 버리면 다음 사람이 다시 밟는다.

**게이트** (`tools/status-shape.mjs` · `pnpm docs:check` · `ci.ps1` 의 `docs` 층 ·
`.github/workflows/ci.yml`). 일곱을 센다: ① 지목하는 줄이 **하나**인가 ② 그 번호가
FINDINGS 에서 **대기**인가 ③ `## ` 절 제목이 서로 다른가 ④ 절 제목에 「다음 바퀴」가
없는가 ⑤ `### 지난 바퀴 (N)` 이 `STATUS_MAX_PAST_CYCLES`(5) 안인가 ⑥ 같은 바퀴 기록이
STATUS 와 history **양쪽에** 있지 않은가(옮겼나 베꼈나) ⑦ 정본 모양을 **흉내 내는** 줄이 없는가.
🔴 **일곱이 빨개지는 것을 전부 봤다** — 102→97 로 되돌리니 상태 문자열까지 찍으며 FAIL,
둘째 자리를 만드니 두 줄 번호를 짚으며 FAIL, 절 이름을 되돌리니 줄 번호로 FAIL,
`## 눈 판정 대기`를 하나 더 만드니 FAIL, 40바퀴를 STATUS 로 되돌리니 상한 초과+양쪽 2건,
history 에 41바퀴를 베끼니 겹침으로 FAIL. 전부 되돌렸다.
★ **이 층만 「앞 층이 빨갛다」로 안 건너뛰고 제일 뒤에서 항상 돈다** — 층 순서는 비용
순서이고 그 뜻은 「앞이 빨가면 뒤는 **무의미**하다」인데, 문서가 어긋난 것과 타입이 갈린
것은 서로를 무의미하게 만들지 않는다.

**눈 판정 — 그리고 거기서 구멍이 하나 더 나왔다** (`03fb4f1`).
고친 뒤 STATUS 를 **다음 바퀴처럼 훑어 읽었다.** 지난 바퀴 기록 블록 셋(45·44·43)이 각자
`**다음 바퀴의 일**: FINDINGS 97 …` 같은 줄을 **그때의 지목**으로 들고 있었고, 셋이
가리키는 98·97·94 는 **전부 닫힌 항목**이다. 정본 모양(`— FINDINGS N`)이 아니라 `**:` 라서
게이트에는 안 걸리는데 **훑는 눈에는 똑같이 보인다.** 지나간 지목은 이제 다른 낱말
(「그 바퀴가 다음으로 지목한 것」)로 적고, 게이트 ⑦ 이 흉내 내는 줄을 잡는다.
★ **「자리를 하나로 만들었다」와 「그 자리처럼 보이는 것이 하나다」는 다른 질문이다.**
그리고 이 구멍은 **시험이 아니라 눈 판정에서** 나왔다.

**2-B 47바퀴 라운드** (`102-B`): 지금까지 안 돈 **「값이 숫자인 순위·순서 표」** 둘을 골랐다.
- **`ROLE_RANK` 2종은 ①② 다 살아 있다** — 뒤집으니 웹 시험 **106개**가 빨개졌다. 되돌렸다.
- 🔴 **`SCOPE_ORDER` 3종은 「살아 있는데 아무도 안 잠갔다」** — 정반대로 뒤집어도 컴파일러
  시험 **147개가 전부 초록**이다(golden 3케이스 포함). 그런데 죽은 게 아니다: 조건을 만들면
  (`roadmap` 둘 · 같은 priority · scope 만 다르게) `CLAUDE.md` 안의 **순서가 실제로 갈린다.**
  **P4 의 심장에 손을 대도 CI 가 초록**이라는 뜻이다. **103** 으로 냈다.
- ★ 이 라운드가 남긴 것 — **2-B 의 답은 「살아 있다/죽었다」 둘이 아니라 셋이다.**
  셋째가 **「살아 있는데 안 잠겼다」**이고, 죽은 코드보다 위험하다. 죽은 것은 고쳐도 아무
  일이 안 나지만, **안 잠긴 산 것은 고치면 조용히 산출물이 바뀌고 CI 는 초록**이다.
  ②단계에서 「시험이 빨개졌나」와 「산출물이 갈렸나」를 **따로** 물어라 — 이번에 둘이 갈렸다.

**다음 바퀴의 일 — FINDINGS 103** · [구멍] · 「`SCOPE_ORDER` 3종을 잠그는 시험이 0개다」
...
대기는 넷이다. 심각도 순서는 **고장 → 구멍 → 격차**이므로 [구멍]인 **103** 이 맨 위다.
그 다음이 [격차] 셋 — **99**(아키텍처 블록이 같은 문장을 연달아 두 번) ·
**100**(`## Goals` 한 줄도 같은 문장을 두 번) · **101**(`지표:` 의 낱말이 근거 원문에 없다).
