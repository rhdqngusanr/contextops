...
_마지막 갱신: 2026-09-06 · 루프 56바퀴 · `e1e79d5`_

---

## 지금 어디인가

**이번 바퀴는 `docs/PLAN.md` **P4 첫 행을 닫았다** — 남아 있던 절반, 웹 **화면 6
(Proposals)** 목록과 상세다.** 55바퀴가 화면 8(Roadmap)을 만들었고, 이 바퀴가
`…/proposals` · `…/proposals/[id]` 를 만들면서 그 행의 두 화면이 다 생겼다.

🔴 **잰 것 — 제안이 화면에서 승인·거절되고, 그 판단의 근거가 화면에 있다.**

| | 전 | 후 |
|---|---|---|
| `GET /projects/{id}/proposals` 를 읽는 화면 | **0곳** | `…/proposals` (좌측 내비에 탭 한 줄) |
| 제안 한 장을 여는 문 | **없었다** | `GET /proposals/{id}` (+ `targets`) |
| `POST /proposals/{id}/approve\|reject` 를 부르는 제품 코드 | **0곳** | 화면 6 상세의 [모두 승인]·[거절] |
| before/after Diff | 없음 | `lib/web/diff.ts` (LCS · 순수 함수) |
| 사유 없는 거절 | **200** | **400** (`noteRequired` · 서버와 화면이 같은 표) |
| 웹 시험 | 430 | **467** |
| 관통 검사 | 704 | **741** |
| CI | — | principles OK 9 · typecheck · test · build · walkthrough **741** · docs → GREEN |

🔴 **문을 하나 더 만들었다 — 그게 이번 바퀴의 결정이다.** 55바퀴가 남긴 질문이
「`GET /proposals/{id}` 를 만들지, 목록으로 버틸지」였다. **만들었다.** 목록이 `items` 까지
통째로 내므로 상세를 목록으로 그릴 수는 있지만 목록은 `?limit=50` 이라 **51번째 제안의
상세를 영원히 못 연다** — 주소가 가리키는 것이 목록의 어느 쪽에 있느냐로 정해지면
그건 주소가 아니다.

🔴 **그 문이 `targets` 를 같이 낸다 — diff 의 「before」다.** before 는 `target_item_id` 가
가리키는 **지금 항목의 본문**인데, 화면이 그것을 항목 목록에서 찾으면 같은 상한(50)에
다시 걸린다. 서버는 그 제안이 가리키는 id 만 골라 읽으므로 상한이 없다.
⚠ **없는 대상은 안 싣는다** — 빈 before 로 그리면 `update` 가 `add` 처럼(없던 줄이 통째로
추가된 것처럼) 보인다. 화면은 「대상 항목을 찾을 수 없습니다」라고 말하고, 그건 발행이
그 제안에서 롤백되는 것과 **같은 사실**이다.

🔴 **`PROPOSAL_DECISIONS` 가 `packages/schema` 로 올라갔다 (둘째 사용자).** 화면이
「지금 이 제안에 어떤 버튼을 그릴 수 있나」를 물으면서 서버(`decide()`)와 **같은 판단**을
하게 됐다 — 화면이 조건을 다시 적으면 둘이 갈리고 **느슨한 쪽이 이긴다**(화면이 그린
버튼이 400 을 받는다). 상태 5종 × 등급 2종 **열 갈래**를 시험이 표와 대조한다.
`PROPOSAL_STATUSES` 도 같이 올라갔다 (화면이 상태마다 다른 칩을 그린다).

🔴 **거절은 사유가 필수가 됐다** (DESIGN_BRIEF §4 화면 6 「거절(사유 필수)」).
표에 `noteRequired` 한 칸을 만들고 **화면과 서버가 같이 읽는다** — 화면만 막으면
CLI 로 사유 없는 거절이 들어오고, 그때 제안을 쓴 사람은 무엇을 고쳐야 하는지 알 자리가
없다 (제안은 되돌아오지 않고 새로 쓴다). 기존 시험 두 개가 사유를 싣도록 바뀌었다.

**표 넷이 늘었고 넷 다 시험이 잠근다** — `PROPOSAL_STATUS_CHIP`(5종 · `approved` 와
`published` 가 **다른 말**이어야 한다: 승인은 약속이고 배포는 발행 뒤다) ·
`PROPOSAL_OPERATION_CHIP`(3종) · `DECIDED_TEXT`(3종) · `DIFF_MISSING_TEXT`(2종).

**눈으로 읽었다** — `docs/evidence/2026-09-06-proposals/proposals.txt` (모양 25개 · 뽑는 자는
`apps/web/scripts/dump-proposals.tsx`). 🔴 **덤프가 시험이 못 잡은 것 둘을 잡았다**:
① 화면이 `approved` 같은 **영어 enum 값을 사람에게 그대로** 보여 줬다 (→ `DECIDED_TEXT`
— 「무엇이 되었나」가 아니라 「이제 무슨 일이 일어나나」를 적는다) ② `add` 항목 카드에
**역추적 태그가 없었다** — 대상이 없다고 id 가 없는 게 아니다 (`draft.id` 가 그 항목이
받을 id 다 · P7).

**빨개지는 것을 봤다** — 「대상이 없다」와 「바꿀 본문이 없다」를 한 문장으로 합치면
시험이 빨개진다. 둘은 **사람이 할 일이 다르다** (앞엣것은 발행이 막히는 일이다).

🔴 **재다가 넷 나왔다** — **113** 제안 목록에 「작성자」를 그릴 수 없다(uuid→이름을 내는
문이 0곳 · 주인은 화면 9) · **114** 항목별 [승인]/[거절] 을 담을 자리가 서버에 없다
(DESIGN_BRIEF 가 코드보다 넓다 · **결정이 먼저 필요하다**) · **112** 목록 거르개가 없다
(라우트가 `?status` 를 안 받는다 · DB 인덱스는 이미 있다) · **115** CLI 가 찍는 제안
주소(`/p/{project_id}/proposals/{id}`)가 **앱에 없는 주소**라 눌러도 404 다.
...
(54·55바퀴와 같은 이유다. 그 줄의 모양을 넓히는 것이 FINDINGS **109** 다).

> **다음 바퀴의 일 = `docs/PLAN.md` P4 **둘째 행** — 「웹 화면 9 · 게스트 데모 · 랜딩 v1」**
> 그 행의 완료 기준은 🔴 **GATE 3**(시크릿 창에서 링크만으로 3분 체험)이고, 한 바퀴에
> 다 못 한다. **화면 9(`…/sync`)부터** 해라 — `GET /projects/{id}/sync-status` 는 이미 있고
> 플러그인이 `POST /sync-reports` 로 보고도 하는데 **읽는 화면이 0곳**이라 `SYNC_CHIP`
> 5종이 아무 데도 안 그려진다 (FINDINGS **110**). SPEC §9 화면 9 · DESIGN_BRIEF §4 「화면 9」.
> ⚠ **먼저 정할 것**: 그 표의 「팀원」 칸도 **이름을 내는 문이 없다** (FINDINGS **113** —
> 화면 6 이 「작성자」 칸을 못 만든 것과 **같은 구멍**이다). 두 화면이 같은 문을 기다리므로
> 이 바퀴에 `author`/`user` 이름 한 칸을 낼지 **정하고 시작해라.**
> ⚠ 질의창(§7.3)은 절삭 순서 1번이라 **뒤로 미룰 수 있다** — 표가 먼저다.

- PLAN 의 `- [ ]` 중 **위의 둘은 사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키).
- 대장의 대기(115 · 114 · 113 · 112 · 111 · 110 · 108 · 25 · 33 …)는 **PLAN 을 막지 않는다** —
  적어 두고, 그 항목의 주인이 될 PLAN 행을 할 때 같이 닫는다 (④3 ②).
