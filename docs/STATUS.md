...
_마지막 갱신: 2026-09-07 · 루프 108바퀴 · 코드 `f095e66`(FINDINGS 161 절반 — 캡처를 **픽셀로** 견줘 안 달라졌으면 안 옮긴다 · FINDINGS 25 ✅ 는 재고 닫았다) · **CI GREEN 전 층** · 관통 1252검사 · 9단계 SKIP 0 · 문서는 그 다음 커밋_
...
> 🔴 **왜 다시 161 인가 — 절반이 남았고, 남은 절반이 진짜 뿌리다.** PLAN 의 `- [ ]` 셋은 여전히
> 전부 사람을 기다린다(아래 「막힌 것」). 대장의 대기 중 루프가 혼자 열 수 있는 것은 161 뿐이다
> (156 은 배포 뒤 · 152 는 「⛔ 지금은 행동할 수 없다」 · 140 은 🙋 새 PC · 117 은 잘랐다 · 33·24 는 「지금 열지 마라」).
>
> **남은 절반은 무엇인가** — **데모 씨앗이 재현 가능하지 않다.** 둘이 같이 와야 한다:
> ⓐ **고정 시계** — `seedDemo(now)` 는 이미 인자를 받는다. 관통이 상수를 주면 발행 시각이 안 흔들린다.
> ⓑ **결정론적 문서 버전 id** — 지금은 DB 기본값 uuid 라 심을 때마다 새로 나고, 그 uuid 가
> **Pack 의 모든 줄 태그**에 실려서 `manifest_hash` 를 흔든다.
> ⚠ ⓑ 를 하려고 `POST /documents` 에 「id 를 받는 문」을 열지 마라 — **데모 사정으로 제품 계약을 넓히는 것**이다.
> 씨앗이 라우트로 만든다는 원칙(`lib/demo/seed.ts` 머리 주석)과 어떻게 같이 설지를 **먼저 정하고** 손대라.
> ⚠ 한쪽만 하면 두 장은 계속 달라진다 — **어중간하게 두지 마라.**
> ★ 값 — 「같은 씨앗 → 같은 `manifest_hash`」가 서면 P4 주장을 **제품 수준에서** 한 번 더 보이고,
> 그때부터 워킹트리가 관통마다 안 더러워진다.

- PLAN 의 `- [ ]` 중 남은 것 **셋**: P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(**161** · 156 · 152 · 140 · 117 · 24 · …) — **고장 0** · 이번 바퀴가 새로 찾은 고장도 **0** 이다(관통 GREEN · 9단계). 25 는 닫혔다.
