...
_마지막 갱신: 2026-09-04 · 루프 40바퀴 · `95c525e`_
...
**이번 바퀴는 Pack 의 역추적 태그가 「어디를 가리키는지」를 손으로 적는 것에서
...
🔴 **맨 위는 `93` 이다 — 대기 중인 [고장]도 [구멍]도 지금은 없다.** 심각도 순서는 언제나
**고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3). 그 다음은 **94** · 87 · 91 (전부 격차).
(**92** 는 `95c525e` · **90** 은 `800372f` · **89** 는 `5858fbb` · **88** 은 `6f06881` ·
**86** 은 `c6905df`+`b1d2190` · **84**·**31** 은 `c57b3fb` · **82**·**65** 는 `5fe0068` ·
**80** 은 `f9962cb` · **79** 는 `1aebc22`.)

🔴 **93 과 94 는 같은 고장의 두 축이다 — 한 게이트로 같이 닫아라.**
89(`enforcement`)·93(`SourceRef`·`scope.kind`)·94(`ItemType`)가 전부 **「표는 살아
있는데 데모가 한두 갈래만 보여 준다」**이다. 89 가 `PACK_ENFORCEMENT_MIN` 상수 하나를
만들었는데, **여기서 축마다 상수를 하나씩 더 만들면 넷이 된다.** 「데모가 이 표의 몇
갈래를 보여 주나」를 **표 하나**(`Record<축이름, {값들, 최소치, 왜}>`)로 모으고 관통이
그 표를 **읽기만** 하게 해라 (`CLAUDE.md` 「확장은 표에 한 줄」).
⚠ **최소치를 「전 갈래」로 잡지 마라.** 원문이 없는 갈래를 채우려면 픽스처에 문장을
지어내게 되고 그 순간 P7 이 깨진다 (89 가 `permission`·`none` 을 비워 둔 이유).
표의 각 줄에 **「지금 몇 갈래이고 왜 그 수인가」**를 적어라.

**94 — 데모 Pack 이 `ItemType` 10종 중 다섯만 보여 준다** (격차 · 이번 바퀴 2-B 에서 나왔다).
서는 것은 `mission`·`goal`·`roadmap`·`policy`(3개)·`constraint` 다섯.
안 나오는 것은 `architecture`·`domain`·`adr`·`workflow`·`open_question` 다섯.

**시작하기 전에 아는 것 (94):**

- 🔴 **셋은 원문이 `fixtures/paylab-docs/goals.md` 에 이미 있고 아무도 안 읽는다 —
  지어낼 필요가 없다.** §5 「아직 정하지 못한 것」(115–122줄 · 미결 4건 = `open_question`) ·
  §6 「용어」(128–136줄 · PSP·승인·매입·종결·원장 = `domain`) ·
  §7 「아키텍처 한 장」(138–150줄 = `architecture`). 씨앗이 이 세 절을 통째로 건너뛴다.
- 🔴 **§7 은 93 의 `path` scope 근거와 같은 문단이다** — 93 과 94 를 같이 하면
  픽스처를 한 번만 만진다.
- ⚠ `adr` 과 `workflow` 는 픽스처에 원문이 **없다.** 문서를 먼저 늘려야 하고,
  안 늘릴 거면 최소치에서 빼고 **왜 뺐는지**를 표에 적어라.
- ⚠ **세다가 틀리기 쉬운 자리 둘.** `.claude/rules/domain-refund.md` 는
  ...
  `workflow` 타입 항목이 만든 것이 아니다 (열어 보면 `ctx:` 태그가 한 줄도 없다).

**93 — 데모 Pack 이 근거 4종 중 둘 · scope 3종 중 둘만 보여 준다** (격차 ·
**89 와 같은 모양이고 89 는 표 하나만 고쳤다**).
`.ci/walkthrough-pack/` 의 태그는 `doc` 14개 · `proposal` 1개뿐이고
`repo`(코드가 근거)·`manual`(사람이 정리한 근거)은 데모에 없다. `scope.kind` 도
`project`·`domain` 뿐이라 **`.claude/rules/scoped-*.md` 라는 Pack 파일 갈래가 통째로 없다.**

**시작하기 전에 아는 것 (93):**

- 🔴 **셋 다 진짜 근거가 있다 — 지어낼 필요가 없다.** ① `path` scope: goals.md §3.5
  「웹훅은 서명 검증 후에만 처리한다」가 `src/webhook/` 에 걸린다 (§7 아키텍처 그림이
  경로를 적어 둔다). ② `repo` 근거: `fixtures/paylab-api` 의 `src/payment/retry.ts` 가
  `MAX_RETRY=3` 고정 간격이다 — 「문서는 5회 백오프인데 코드는 3회 고정」이 **이 데모의
  충돌 서사 그 자체**다. `item_policy_retry` 에 근거를 한 칸 더하면 태그가 `doc:…,repo:…`
  로 나온다. ③ `manual` 근거: **충돌을 정리하면** 붙는다 (`conflicts/{id}/resolve`).
- 🔴 **셋 중 제일 큰 것은 ③이다.** 관통이 지금 씨앗 → **바로** 발행으로 간다. 발표
  타임라인(SPEC §10.5)의 「1:10 충돌 카드 3장 → 2:00 정리·발행 v1.0」에서 **가운데 한 칸을
  건너뛴다.** 그 칸을 관통에 넣으면 `manual` 근거는 저절로 따라온다.
- ⚠ **게이트를 표마다 복사하지 마라.** 89 가 `PACK_ENFORCEMENT_MIN` 하나를 만들었다.
  「데모가 이 표의 몇 갈래를 보여 주나」를 **표 하나로** 모아라 — 안 그러면 표가 늘 때마다
  관통에 검사 한 벌씩 붙는다 (`CLAUDE.md` 「확장은 표에 한 줄」).
- ⚠ **`confidence` 는 여기 없다.** 91-B 가 이미 보고 「고칠 것이 아니다」로 닫았다 —
  씨앗은 사람이 문서를 보고 손으로 옮긴 것이라 `high` 가 사실이고, `medium`·`low` 는
  AI 구조화(§7.1)가 내는 값이다. 같은 것을 또 캐지 마라.
