...
_마지막 갱신: 2026-09-06 · 루프 55바퀴 · `7398e44`_

---

## 지금 어디인가

**이번 바퀴는 `docs/PLAN.md` **P4 첫 행**의 절반을 만들었다 — 웹 **화면 8(Roadmap)** 과
그 행이 말하는 「Realtime」이다.** 54바퀴가 P3 둘째 행을 닫으면서 루프가 움직일 수 있는
맨 위 행이 P4 첫 행으로 바뀌었고, 그 행의 완료 기준이 곧 이 화면이다:
「agent progress → Roadmap 갱신 · **Roadmap 기본 행이 마일스톤**(P5)」.

🔴 **잰 것 — 기기가 보고하면 그 수가 화면에서 갈린다.**

| | 전 | 후 |
|---|---|---|
| `GET …/roadmap` 을 읽는 화면 | **0곳** | `…/roadmap` (좌측 내비에 탭 한 줄) |
| `POST /progress/{id}/confirm` 을 부르는 제품 코드 | **0곳** | 화면 8 의 [완료 확인] |
| 「Realtime」 | 없음 | **폴링 10초** (`ROADMAP_POLL_MS` · SPEC §14 절삭 8) |
| `SYNC_CHIP` 을 그리는 화면 | 0곳 | 0곳 (**FINDINGS 110** 으로 적었다) |
| 웹 시험 | 396 | **430** |
| 관통 검사 | 670 | **704** |
| CI | — | principles OK 9 · typecheck · test · build · walkthrough **704** · docs → GREEN |

🔴 **[완료 확인] 버튼은 만들 수가 없었다 — 그게 이번 바퀴에 새로 안 것이다.**
`POST /progress/{id}/confirm` 은 **보고 하나의 id** 로 부르는데, 그 id 를 내는 문이
하나도 없었다. 보고를 만든 것은 **기기**이고 그 응답은 사람의 브라우저에 안 온다 —
즉 화면은 확정할 대상을 **가리킬 수가 없었다.** 그래서 라우트가 마일스톤마다
**`confirmable`**(아직 확정 안 된 제일 최근 `done_candidate` **하나**)을 낸다.
⚠ 목록으로 안 낸다 — 여럿을 내면 「어느 것을 확정하나」를 화면이 고르게 되고 그 규칙이
서버와 갈린다. ★ 이게 「agent 는 스스로 완료를 선언하지 못한다」의 **화면 쪽 절반**이다.

🔴 **`PROGRESS_STATUSES` 의 `none` 이 처음으로 무언가를 바꾼다** (④2-B). 「이 작업은
어느 마일스톤도 아니다」 보고는 `PROGRESS_EFFECT.none === undefined` 라 접히지도 않고
마일스톤 행에도 안 붙어서 **어디에도 안 보였다.** 화면 8 하단의 「로드맵 외 작업 N」이
그 자리다. 가르는 규칙은 **하나**다 — 「지금 Manifest 의 마일스톤 id 가 아닌 것」.
`'none'` 은 마일스톤 id 가 될 수 없어 같은 규칙에 걸리고, **지난 Pack 에만 있던
마일스톤**도 같이 걸린다 (갈래를 둘로 만들면 뒤엣것이 조용히 샌다).

**표 둘이 늘었고 둘 다 시험이 잠근다** — `MILESTONE_CHIP`(4종) · `PROGRESS_SOURCE_LABEL`(3종).
🔴 `MILESTONE_CHIP` 은 라벨뿐 아니라 **tone 까지** 잠갔다: `done_candidate` 가 `ok` 로
보이면 확정이 끝난 것처럼 읽히고, 그 순간 이 제품의 약속 하나가 화면에서 사라진다.

**두 개가 「둘째 사용자가 왔다」로 올라갔다** — `MILESTONE_STATUSES` 는
`lib/api/progress.ts` → `packages/schema` (화면이 `Record<MilestoneStatus,…>` 를 갖게 됐다) ·
진행 막대는 `job-progress.module.css` → `globals.css` 의 `.bar`/`.bar-fill`.
후자는 **그 파일의 주석이 예고한 그대로다**: 「화면 8 이 같은 모양을 쓰게 되면 그때 올린다」.

**빨개지는 것을 봤다** — ① 라우트에서 `confirmedAt === null` 을 빼면 확정한 뒤에도
버튼이 남고 API 시험이 빨개진다 ② 화면에서 충돌 수를 행마다 더하면 「3」이 「6」이 되고
요약 시험이 빨개진다. 둘 다 되돌렸다.

**눈으로 읽었다** — `docs/evidence/2026-09-06-roadmap/roadmap.txt` (20모양 · 뽑는 자는
`apps/web/scripts/dump-roadmap.tsx`). 🔴 **덤프가 시험이 못 잡는 거짓 문장 하나를
잡았다**: 마일스톤이 0개인데 타일이 「전부 한 번은 보고됐습니다」라고 말했다 — 아무것도
없는데 전부 됐다고 하는 문장이다. 고치고 그 자리를 시험으로 잠갔다.

🔴 **재다가 둘 나왔다.** ① **Manifest 마일스톤에 `due` 가 없다** (FINDINGS **111**) —
`RoadmapData.due` 는 계약에 있고 Pack **본문**에도 나가는데 `milestonesOf()` 가 Manifest 로
안 나른다. 화면 8 은 마일스톤을 Manifest 에서 읽으므로 DESIGN_BRIEF 가 요구하는
`due 09-20` 을 **그릴 근거가 없다.** ⚠ 지어내지 않았다. ② **화면 9 가 없어서
`SYNC_CHIP` 5종을 그리는 곳이 0곳이다** (FINDINGS **110** — 주인은 P4 둘째 행).

**2-B 55바퀴 라운드는 `PROGRESS_STATUSES` 4종과 `PROGRESS_SOURCES` 3종이었다** —
`none` 이 소비처를 얻었고(위), `source` 3종은 근거 드로어가 처음 읽는다.
...
(54바퀴와 같은 이유다. 그 줄의 모양을 넓히는 것이 FINDINGS **109** 다).

> **다음 바퀴의 일 = `docs/PLAN.md` P4 **첫 행의 나머지 절반** — 「웹 화면 6 (Proposals)」**
> 목록(`GET /projects/{id}/proposals`)과 상세(`…/proposals/[id]` · before/after Diff)다.
> SPEC §9 화면 6 · `docs/DESIGN_BRIEF.md` §4 「화면 6」 을 먼저 읽어라.
> ⚠ **먼저 확인할 것**: `GET /proposals/{id}`(한 장) 라우트가 **없다** — 목록 라우트가
> `items` 까지 통째로 내므로 상세를 그것으로 그릴 수는 있지만, 그러면 51번째 제안의
> 상세는 영원히 못 연다 (`ListQuery` 기본 limit 50). 문을 하나 더 만들지, 목록으로
> 버틸지를 **정하고 시작해라.** 그리고 diff 의 「before」는 `target_item_id` → 현재 항목
> 본문이라 `fetchItems` 를 같이 읽어야 한다.

- PLAN 의 `- [ ]` 중 **위의 둘은 사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키).
- 대장의 대기(111 · 110 · 108 · 25 · 33 …)는 **PLAN 을 막지 않는다** — 적어 두고,
  그 항목의 주인이 될 PLAN 행을 할 때 같이 닫는다 (④3 ②).
