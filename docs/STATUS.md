...
_마지막 갱신: 2026-09-04 · 루프 36바퀴 · `b1d2190`_

---

## 지금 어디인가

**이번 바퀴는 「고르라고 내놓고 근거는 안 준 줄」에 근거를 붙였다 — 후보 한 줄이 이제
무엇이 될 문장인지와 어디서 온 문장인지를 같이 말한다** (FINDINGS **86** ·
`c6905df` + `b1d2190`).
...
🔴 **맨 위는 `88` 이다 — 대기 중에 고장도 구멍도 없고, 88 과 87 둘 다 [격차]다.**
심각도 순서는 언제나 **고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3).
(**86** 은 `c6905df`+`b1d2190` · **84**·**31** 은 `c57b3fb` · **82**·**65** 는 `5fe0068` ·
**80** 은 `f9962cb` · **79** 는 `1aebc22`.)

**88 — `ink-4`(비활성 색)로 그린 문장이 다섯 자리에 있다** (격차).
토큰 표에서 `ink-4` 의 용도는 「비활성」 한 낱말인데, 화면 다섯 자리가 그 색으로
**읽어야 하는 것**을 그린다. 대비 1.6:1 — 발표 영상·인쇄된 심사 자료에서는 글자가
아예 없는 것과 같다.

**시작하기 전에 아는 것 (88):**

- 🔴 **제일 아픈 둘은 근거 쪽이다.** ①`evidence.tsx:73` 「코드 본문은 서버에 없습니다」는
  **P1 을 설명하는 문장**(심사 첫 질문의 답)이고, ②`packs/[semver]/page.tsx:187` 의
  Pack 줄 번호는 **P7 을 눈으로 재는 값**이다 — 역추적 태그가 몇 줄째인지 그걸로 센다.
- ⚠ **`aria-hidden` 인 `◌` 와 빈 칸 `—` 여섯 자리는 장식이 맞다.** 같이 고치지 마라 —
  「전부 바꿨다」로 세면 진짜 문제 다섯이 그 안에 묻힌다.
- 🔴 **게이트를 같이 올려라.** `design-tokens.test.ts` 는 지금 「임의 색 리터럴이 없나」만
  센다. 「비활성 색으로 글자를 그리지 않았나」는 아무도 안 센다 — 그래서 다섯 자리가
  조용히 쌓였다. 같은 지적이 두 번 나오면 게이트다 (`CLAUDE.md`).
- ⚠ 토큰을 **더하는** 쪽을 고르면 `DESIGN_BRIEF` §3 표에 **먼저** 넣고 써라 —
  `design-tokens.test.ts` 가 `:root` 와 그 표를 대조한다 (`bad-bg` 가 그렇게 들어왔다).
