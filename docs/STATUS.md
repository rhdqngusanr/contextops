...
_마지막 갱신: 2026-09-04 · 루프 25바퀴 · `a24120e`_

---

## 지금 어디인가

**이번 바퀴는 「문서가 없어도 됩니다」를 참말로 만들었다** (FINDINGS **67 ③** ·
...
🔴 **FINDINGS 68.** (67 은 ③이 닫혔고 ①(zip)만 남았다 — `9f481a0`+`a24120e`.)

**68 이 먼저인 이유** — P7 이 반만 이어져 있다. 제안이 만든 개정은 `origin:'proposal'`
을 찍는데 `source_refs` 에는 **어느 제안인지가 없다.** 골든은 `src:proposal:…` 를 갖고
있어서 형식은 그것을 약속하는데 진짜 발행은 그 모양을 못 만든다.
고치는 자리는 `lib/api/publish.ts` 의 `insertRevision()` **한 곳**이고 호출부가 이미
제안 id 를 들고 있다 (`publish.ts:83`·`172`). ⚠ 잠글 것은 「붙었다」가 아니라
**「제안으로 들어온 항목의 Pack 줄에 `src:proposal:` 이 있다」**다.
⚠ 같은 자리에 `manual` 을 같이 만들지 마라 — **그건 이번 바퀴에 생겼다** (씨앗 질문의
답이 만드는 항목이 `{kind:'manual', note:질문}` 을 단다). 두 종류가 이제 다 살아 있다.

**그 다음 후보 둘** — 둘 다 「P3 둘째 행」이 주인이다:
- **화면 4(정리)** — 볼 것이 이제 둘이다 (충돌 카드 · 씨앗 질문 10장). 이 행의 완료
  기준(「질문만으로 v1.0 발행」)을 실제로 **재는** 유일한 길이기도 하다.
- **FINDINGS 67 ①(zip)** — SPEC §11 의 경로·개수·용량 상한이 먼저다 (26 과 한 묶음).
