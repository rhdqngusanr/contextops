...
_마지막 갱신: 2026-09-04 · 루프 34바퀴 · `5fe0068`_

---

## 지금 어디인가

**이번 바퀴는 「고르는 칸은 있는데 고른 값이 아무것도 안 바꾸던 것」을 살렸다 — 문서
종류가 이제 §7.1 프롬프트를 바꾼다** (FINDINGS **82**·**65** · `5fe0068`).

🔴 **무엇이 문제였나.** 화면 3 의 업로드 폼이 「목표·정책·로드맵·ADR·메모·위키」 여섯 중
하나를 고르게 하고, 그 값은 DB 에 저장되고 응답으로도 나갔다. 그런데 **읽는 자리가
0곳**이었다 — §7.1 프롬프트에 `kind` 가 없어서 같은 문서를 「정책」으로 올리든 「메모」로
올리든 구조화 결과도, 나오는 항목도, Pack 도 **완전히 같았다.** 사람은 분류한 줄 알고
골랐다. 표(`SOURCE_DOCUMENT_KIND_LABEL`) 자체는 `web-tables.test.ts` 가 잠그고 있어서
죽지 않았다 — 죽은 것은 **값의 효과**였다 (2-B ②단계 불합격).

**바꾼 것 셋:**
1. **`lib/ai/structure.ts` 에 표 하나** — `SOURCE_DOCUMENT_KIND_BRIEF`
   (`Record<SourceDocumentKind, string>`). chunk 프롬프트 **머리 한 줄**이 그 표를 읽어서
   나간다 (`이 문서는 …다`). 본문은 여전히 `<untrusted>` 블록 안에만 있다 (§11).
2. **`lib/ai/job.ts` 의 러너가 배선한다** — join 한 `source_documents.kind` 를 읽어
   넘긴다. `kind` 는 버전이 아니라 **문서**의 칸이라 개정마다 갈리지 않는다.
   ⚠ **`StructureInput.kind` 에 기본값을 두지 않았다.** 기본값을 주면 부르는 자리가
   잊어도 조용히 돌고, 그게 이 값이 죽어 있던 방식 그대로다 — 지금은 타입이 막는다.
3. **화면 라벨과 일부러 다른 표다.** `SOURCE_DOCUMENT_KIND_LABEL`(chips)은 사람이 고를
   때 읽는 낱말이고, 모델이 읽는 문장은 새 표다. 합치면 화면 문구를 다듬는 바퀴가
   §7.1 결과를 조용히 바꾼다. 라벨 표에 붙어 있던 「아직 아무것도 바꾸지 않는다」
   주석(FINDINGS 65)을 지금 사실로 고쳤다.

**게이트는 「소비처가 생겼다」에서 멈추지 않았다** (2-B ②단계) —
① `ai-structure.test.ts`: 여섯 값을 각각 돌려 프롬프트 머리말을 모으고, 각 값의 문장이
실려 있고 **여섯이 서로 다르다**를 잰다 (하나라도 겹치면 골라도 같은 값이다). 시간을
한 시간씩 밀어 §7.5 「시간당 5회」를 피한다 — 재는 것은 빈도가 아니라 프롬프트다.
② `ai-job.test.ts`: 「메모」로 **라우트를 거쳐** 올린 문서로 job 을 굴려, LLM 이 받은 user
프롬프트에 notes 문장이 있고 **goal 문장이 없다**를 잰다 (DB → 프롬프트 배선 · 기본값이
조용히 실리지 않는다). 시험: 웹 **342 → 347**. CI 5층 전부 초록.
SPEC §7.1 에 이 줄을 넣었다 — 코드가 SPEC 을 어긴 게 아니라 **둘 다 비어 있었다.**

**눈으로 읽은 것** — `docs/evidence/2026-09-04-doc-kind/prompt-heads.txt`
(여섯 종류로 올린 같은 문서의 **실제 user 프롬프트**). 읽고 판정한 것: 첫 줄이 여섯 번
다 다르고, 그 줄은 `<untrusted>` **밖**에 있고(본문이 지시로 읽히는 자리가 안 늘었다),
그 뒤 세 줄(조각 번호·제목 계보·offset 상한)은 그대로다 — 바뀐 것은 **한 줄뿐**이다.

**2-B 한 라운드도 돌렸다** (FINDINGS **83-B**) — 이번엔 둘 다 살아 있었다:
- **`AI_JOB_STATUS_RULES` 4종** ✅ — 네 줄에서 CHECK 제약 4개가 나오고(`db/schema.ts:617`)
  `finished` 축이 「멈춘 job 인가」를 가른다. 시험이 칸을 **뒤집어** 거부되는지 잰다.
- **`TeamRole` 2종** ✅ — 같은 body·같은 프로젝트로 member 는 403 · owner 는 201
  (`api-auth.test.ts:128`).

**다음 바퀴의 맨 위** — 🔴 **남은 것 중 「고장」은 없다.** 구멍이 둘(**56** 질문↔항목이
안 이어진다 · **36** 토큰 발급 화면이 없다)이고 나머지는 격차다. 심각도 순서(고장 →
구멍 → 격차)대로면 **56** 이다. ⚠ FINDINGS 파일의 맨 위 대기 항목은 **69**(격차 · sync
`manual` 을 찍는 코드가 0곳)라 파일 순서와 심각도 순서가 갈린다 — 순서대로 골라라.

---

**이번 바퀴는 있는데 한 번도 안 걸리던 가드를 살렸다 — 빈 Pack 은 이제 발행되지
않는다** (FINDINGS **80** · `f9962cb`).

🔴 **무엇이 문제였나.** 승인된 항목이 0건이어도 발행이 **201** 이었다. 씨앗 질문 10장에
답하면 항목이 `draft` 로 생기는데 발행은 `active` 만 담는다 — 사람은 [발행하기] 를 누르고
「v1.0.0 을 발행했습니다」를 보지만, 나온 Pack 은 **제품이 넣는 고정 텍스트**
(`.claude/rules/workflow.md`) 한 장뿐이고 팀 규칙은 한 줄도 없다. 그 빈 버전이 공식이
되고 기기들이 `applied` 로 받아 간다.

🔴 **가드는 있었는데 재는 값이 틀렸다.** `EMPTY_SNAPSHOT` 의 조건이
`files.length === 0` 인데 `assemble.ts:55` 가 `always` 문서를 **먼저** 만들어서 파일 수가
0 이 될 수 없다. 소비처 0곳 · 시험 0건 — 정의만 있고 아무 일도 안 하는 코드였다.

**바꾼 것 셋:**
1. **`packages/compiler/src/compile.ts` — 「빈 Pack」을 파일 수가 아니라 근거 수로 잰다**:
   `files.every(f => f.source_item_ids.length === 0)`. ⚠ **항목 수를 세지 않은 이유**가
   이 바퀴의 판단이다 — 라우트가 「active 0개」를 세면 **「항목은 있는데 전부 제외된」
   경우를 놓친다** (초안·검토·폐기·`open_question`). 재야 하는 것은 입력이 아니라
   **나온 Pack 에 팀의 것이 한 줄이라도 있는가**이고, 그 답이 `source_item_ids` 다 (P7).
2. **`apps/web/src/lib/api/publish.ts` — `COMPILE_ERROR_FAULT` 표 하나**가 「컴파일 실패
   중 사람 잘못인 것」을 정한다 (`Record<CompileErrorCode, {code,message}|null>`).
   `EMPTY_SNAPSHOT` → **400 `VALIDATION_FAILED`** 「승인된 항목이 하나도 없습니다 —
   Context 에서 초안을 승인한 뒤에 발행하세요」. 나머지 둘은 그대로 500 `COMPILE_FAILED`.
   사람 잘못을 5xx 로 내면 화면은 「다시 해 보세요」밖에 못 고른다. 넷째 컴파일 코드가
   생기면 그 표에서 **타입 검사가 막는다** (compiler 가 `CompileErrorCode` 를 내보낸다).
3. **판정은 한 곳이다** — 라우트가 항목을 다시 세면 「무엇이 빈 Pack 인가」가 두 곳이
   되고 조용히 갈라진다. 컴파일러가 판정하고 라우트는 **무엇을 보여 줄지만** 정한다.

**게이트는 「400 이 난다」에서 멈추지 않았다.** 끝난 뒤 DB 로 잰다 — `context_versions`
0행 · `pack_files` 0행 · `official_version_id` 가 여전히 null · 버전 목록이 비어 있음.
그리고 「**하나만 승인하면 201** 이고 그 항목이 Pack 에 있다」로 가드가 너무 많이 막지
않는 것까지 잰다. 컴파일러 쪽은 `always.test.ts` 에 넣었다 — `always` 문서가 이 가드를
죽인 원인이라 둘을 같은 파일에 둔다.
⚠ 201 을 **사실로 잠그고 있던** `api-seed-questions.test.ts` 의 시험을 둘로 나눴다:
「승인 0건이면 막힌다」와 「승인한 것만 나간다(나머지 아홉 장은 한 줄도 없다)」.
초안이 새지 않는 것을 계속 재면서 새 가드도 잰다.
시험: 컴파일러 **136 → 140** · 웹 **338 → 342**. CI 5층 전부 초록.

**눈으로 읽은 것** — `docs/evidence/2026-09-04-empty-publish/publish-guard.txt`.
세 경우(초안만 · 빈 snapshot · 한 장 승인)를 찍었고 ③이 함정을 그대로 보여 준다:
`workflow.md` 는 **근거 0개**로 나가 있고, Pack 이 「비지 않았다」고 말하게 하던 것이
바로 그 한 장이었다. ③의 마지막 줄이 P7 이다 —
`답 1 번입니다. <!-- ctx:item_seed_mission rev:1 conf:high src:manual:이 팀은 무엇을 만드나요? -->`

**2-B 한 라운드도 돌렸다** (FINDINGS **81-B**) — 이번엔 둘 다 걸렸다:
- 🔴 **`origin` 4종 중 `doc` 이 쓰이는 자리가 0곳이다** (**31** 그대로 · 이번에 **결과**를
  짚었다). 문서에서 온 항목이 `doc` 으로 들어가는 길이 **없다** — `lib/ai/job.ts:165` 가
  「항목 초안은 행으로 만들지 않는다」고 적고 실제로 안 만든다. 읽는 쪽은 살아 있어서
  (`lib/ai/conflict.ts` 가 `origin` 을 프롬프트에 싣는다) **`doc_vs_code` 충돌이
  실데이터로는 절대 안 나온다.**
- 🔴 **새 구멍 82: `source_documents.kind` 6종은 골라도 아무것도 안 달라진다.** 업로드
  폼이 여섯 중 하나를 고르게 하고 DB 에 저장까지 하는데 **읽는 자리가 0곳**이다 —
  §7.1 프롬프트에 `kind` 가 없고, 되돌려 보여 주는 화면도 없다. 표(`..._LABEL`) 자체는
  `web-tables.test.ts` 가 잠그고 있어서 죽지 않았다 — 죽은 것은 **값의 효과**다.
...
시험 합계 **772** (schema 117 · compiler **140** · plugin 174 · web **342**).
🔴 **DB 는 안 바뀌었다 — `0006` 그대로다.** 이번 바퀴는 표(`RESOLUTION_ITEM_OUTCOME`)와
라우트만 건드렸고 새 컬럼도 새 enum 값도 없다. 표 18개 · 인덱스 8개.

## 다음 바퀴가 할 일

🔴 **맨 위는 `82` 다 — 이번 바퀴에 나온 [구멍]이고, 대기 중에 고장이 하나도 없다.**
심각도 순서는 언제나 **고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3).
(**80** 은 `f9962cb` · **79** 는 `1aebc22` · **76** 은 `a39419c` · **74** 는 `094102b`.)

**82 — `source_documents.kind` 6종은 골라도 아무것도 안 달라진다** (구멍).
화면 3 의 업로드 폼이 「목표·정책·로드맵·ADR·노트·위키」 중 하나를 고르게 하고 그 값을
DB 에 넣는데, **읽는 자리가 0곳이다.** 같은 문서를 「정책」으로 올리든 「회의록」으로
올리든 구조화 결과도 항목도 Pack 도 완전히 같다. 사람은 분류한 줄 알고 고른다.

**시작하기 전에 아는 것 (82):**

- 🔴 **쓰는 자리는 하나뿐이다** — `app/api/v1/projects/{id}/documents/route.ts:38`.
  `grep -rn "sourceDocuments" apps/web/src` 의 나머지는 `lib/ai/job.ts:135` 의
  **소유 확인 join** 뿐이고 `kind` 를 안 본다.
- 🔴 **§7.1 프롬프트(`lib/ai/structure.ts`)에 `kind` 가 없다.** 문서 종류가 모델에
  전달되지 않는다. 되돌려 보여 주는 화면도 없다 — `SOURCE_DOCUMENT_KIND_LABEL` 의
  유일한 소비처가 업로드 폼의 `<select>` 다 (`import/page.tsx:145`).
- ⚠ **표 자체는 죽지 않았다** (`web-tables.test.ts:249` 가 여섯 줄이 서로 다른 글자를
  내는 것을 잰다). 죽은 것은 **값의 효과**다 — 2-B ②단계 불합격이다.
- 🔴 **갈래는 둘이고 하나만 골라라.**
  ① **살린다** — §7.1 프롬프트에 문서 종류를 한 줄 싣는다. 게이트는 「`kind` 만 바꾸면
     프롬프트가 달라진다」로 잠근다 — **픽스처 결과로 재면 LLM 없이 잰다.**
  ② **지운다** — `SOURCE_DOCUMENT_KINDS`·pgEnum·폼의 `<select>` 를 같이 지운다.
     ⚠ pgEnum 은 마이그레이션이 붙고 **직렬화된 값**이다. 지우기 전에 정말 다시 안
     필요한지 정해라.
- ⚠ SPEC 도 「프롬프트가 `kind` 를 쓴다」고 적지 않았다 — **코드가 SPEC 을 어긴 게
  아니라 둘 다 비어 있다.** 살리기로 하면 SPEC §7.1 도 같이 고쳐라.

**그 다음은 `31` 이다 (`origin` 4종 중 `doc` 이 0곳):** 이번 2-B 가 결과까지 짚었다 —
문서에서 온 항목이 `doc` 으로 들어가는 길이 없어서 **`doc_vs_code` 충돌이 실데이터로는
절대 안 나온다.** 읽는 쪽(`lib/ai/conflict.ts`)은 멀쩡히 살아 있다. 82 와 **같은 뿌리**다
(둘 다 「문서에서 온 것」이라는 개념이 반만 배선돼 있다) — 82 를 고칠 때 같이 볼 만하다.
