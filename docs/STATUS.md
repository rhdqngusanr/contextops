...
_마지막 갱신: 2026-09-15 · 사람 세션(루프 밖 · 루프는 2026-09-07 부터 `loop/STOP`) · production <https://contextops-rosy.vercel.app> 은 2026-09-10 부터 떠 있다 (`verify:prod` 44/0 → 47/0 · Cron 매일) · 대회는 9/14 제출 · 9/15 참가 접수 확인 · 마지막 push `fa303f5`(FINDINGS 176 · 177) — 요약은 「지금 어디인가」 맨 위 「2026-09-15 현재」, 사람 몫의 정본은 `docs/evidence/2026-09-13-final-audit/README.md`. 세션별 기록은 아래 절들(09-13 · 09-12 · 09-10 · 09-09 · 09-07)이고, 09-14~15 의 일은 커밋 · `docs/SUBMISSION.md` · FINDINGS 174~177 에 있다._
...
| 제출서 · README · 랜딩 — AI 툴 칸에 Codex CLI · 커밋 수(실측 352) · 「못 찾으면 버립니다」→ 다시 묻는다 · 월 $10 상한 · 「실제로 나온 답」/「던졌습니다」→ 사실대로(녹화 없음) | `readme.test` 제출 폼 상한 ~~(한 줄 60 · 문제 290 · AI 활용 446 · AI 툴 191)~~ → 2026-09-14 실제 폼으로 바뀌었다: 일곱 칸 · 문제 ≤ 100자 · AI 활용 ≤ 500자 (`0eec8e5` · `95d135a` · `docs/SUBMISSION.md` 「제출 폼 원문」) |

🙋 **사람 몫 — 마감 순** (방법은 근거 문서 표): ① ✅ **참가 접수 완료** (2026-09-15 사용자 확인 · 과제는 9/14 에 제출 · 폼 캡처 ✅ 9/14 · 9/20 23:59:59 전까지 고칠 수 있다 — 고치면 다시 [과제 제출하기]) ② ✅ 이 일곱 커밋 push·배포 확인(`248d4ce` · 뒤의 push 에서 `verify:prod` 47/0 — 아래 「push · 배포 · 실측」) ③ ✅ watch-prod 수동 dispatch 로그에 `GET / → 200`(아래 「감시」 행) · 🙋 틀린 `PROD_ORIGIN` 으로 실패 메일 받아 보기는 아직 ④ **플러그인 Skill 실기**(setup → sync → progress · 로드는 고쳤지만 세션 안 실행은 아무도 안 봤다) ⑤ ◐ production 로그인 → 발행 — 2026-09-15 06:24~06:35 KST 한 바퀴(로그인은 사용자 · 나머지는 Claude in Chrome · 팀 `mukteam` 에 v1.0.0 발행까지 · 근거 문서 표) · 사람 손 스톱워치(「15분」)는 아직 ⑥ ✅ Vercel `DATABASE_URL` 은 Transaction pooler **6543** (2026-09-14 사용자 확인) · Gemini 는 **이미 유료(Tier 1)** 였다 — 같은 날 사용자 확인 → `GEMINI_DATA_TIER='paid'` · production `/privacy` 한·영 확인(`41de61b`) ⑦ 쇼츠 mp4 는 올리지 않기로 했다(사용자 결정 · 규정상 필수 아님) · ✅ GitHub 저장소 소개(README 머리 + 제출 한 줄)·topics 12개 — 2026-09-14 `gh api` ⑧ 9/18 저녁 동결 · 9/19 `verify:prod` 한 번 (제출은 ✅ 9/14 에 했다 · `docs/DEPLOY.md` 「심사 기간 런북」). 정리: 이 세션의 격리 시험이 `%USERPROFILE%\.claude\.claude.json` 을 하나 만들었다 — 기본 실행은 안 읽는 파일이라 지워도 된다.
...
## 🧑 사람 세션 (2026-09-12 · 루프 밖) — **공개 전 보안 점검 · 영어 모드**

> 사용자 지시: 「레딧 고수들한테 배우고 싶어서 글을 쓰고 싶은데 영어 모드도 가능할까 ·
> 레딧에 물었을 때 해킹당해서 요금폭탄 맞을 만한 것들 검토도 해줘, 그런 거 다 고치게」 →
> 「우리 페이지 싹 다 영어 모드도 추가해줘」.
> 순서는 사용자가 골랐다: **보안 → 영어 모드 → 레딧 글** (영어 범위 「전부」 · 전환은 토글 + 브라우저 감지).

### 보안 점검 — 잰 것

| 항목 | 실측 |
|---|---|
| 커밋된 secret | **0건.** 344 커밋 전체를 키 모양(`sk-`·`AIza`·JWT)으로 훑음 |
| Supabase 직접 접근 | 18표 RLS ON · 정책 0 = deny-all. anon 키로 못 읽는다 |
| Cron | `CRON_SECRET` 없으면 **401로 닫힌다** · `timingSafeEqual` |
| LLM 예산 | advisory lock 안 예약 선기록 · 일 $3 · 월 $10 · `AI_DISABLED` |
| 계정 상한 | 팀 3 · 프로젝트 5 |
| 플러그인 송신 | 파일 **경로 목록만** (본문 없음) |

🔴 **고친 것 셋** (`a11310e` 뒤 두 커밋):
- **R1 빈도 제한이 아예 없었다.** 세는 자리가 `withBudget()` 하나뿐이라 **LLM 을 부르는 요청만**
  셌고, 나머지 라우트는 전부 무제한 — 그중 `POST /demo/session` 은 **자격증명도 요구하지 않는다.**
  ★ 막으려는 것은 지갑이 아니라 **가용성**이다: 심사 기간(9/21~10/5)에 링크가 죽으면 규정상 제외 사유고,
  월 $10 천장은 Vercel 호출 수·Supabase 연결을 막지 못한다. → 라우트 감싸기 한 자리에서 IP 축으로 센다
  (표 `lib/api/rate-limit.ts` · 장부 `rate_hits` · 기본 600/분 · `/demo/session` 은 10분 20회 · IP 는 sha256).
  ⚠ **밟은 함정**: 처음에 `checkRateLimit(getDb(), …)` 로 **값을** 넘겨서, DB 없는 배포에서 fail-open
  **바깥**이 터졌다 → 「DB 없어도 401」·「DB 없으면 503」 두 시험이 500 을 받았다. 여는 **함수**를 넘겨 고쳤고 그 갈래를 시험이 잠근다.
- **R2 zip 을 304 판정 전에 조립**했다. 「파일 수십 개가 DB 왕복 하나보다 싸다」가 근거였는데 304 인 요청의
  옳은 값은 왕복 하나가 아니라 **0** 이고 그 문은 게스트 토큰으로도 열린다 → `ctx.cached` 의 body 가 async 를
  받게 해서 **종류 전체**를 닫았다.
- **R3 `AI_FEATURE_LIMITS` 4종 중 `ask`·`demo` 가 죽은 줄**이었다 (`withBudget` 호출처 0곳 · SPEC §5 는 그 두
  엔드포인트를 있는 것처럼 적고 있었다). enum 이라 지우지 않고 `AI_FEATURES_NOT_WIRED` 로 적고 **양방향** 게이트.

### 영어 모드 — 잰 것

| 무엇 | 잰 것 |
|---|---|
| 범위 판정 | 화면 문구 **1,275개 / 107파일** (주석 제외 · 스크립트로 셈). 이 중 LLM 프롬프트·씨앗 질문은 번역 금지 |
| 기반 | `lib/i18n/` 4파일 — 표 하나(`LOCALES`) · `Localized<T>=Record<Locale,T>`(언어 추가 시 **타입 검사가 막는다**) · 서버 판정 1곳 · 클라이언트 context |
| 순서 | 쿠키(사람이 고름) → `Accept-Language` → 한국어. ①이 ②보다 센 이유는 토글이 다음 페이지에서 풀리면 안 되기 때문 |
| 영어화 완료 | 랜딩 표 13개 전체 · 3분 코스(`DEMO_TOUR_EN`) · `<head>`·OG·`<html lang>` · 게스트 배너 |
| 게이트 | `web-landing-en.test` 12 — 키·배열 길이 일치 · **en 쪽 한글 0**(예외는 `EN_KEEPS_KOREAN` 에 이유와 함께 4줄) · 그 목록도 낡지 않음 · 설치 명령의 실행부 동일 |
...
올리기 전 점검표. ⚠ ~~**링크는 위 보안 수정이 배포된 뒤에** 붙인다.~~ → ✅ 보안 수정은 2026-09-12 저녁에 배포됐다(위 「배포 (2026-09-12 저녁)」 표 · `verify:prod` 47/0) — 링크를 붙여도 된다.

`tools/ci.ps1`: 2026-09-12 16:45 | principles OK | typecheck OK | test OK | build OK | walkthrough OK (1558) | docs OK => GREEN
...
🔴 **열다섯째 판 — 다른 화면들도 같은 식으로 · 사이드 내비** (사용자: 「다른 화면들도 같은 식으로 다듬어줘」 · 「사이드도 별로야」 · 2026-09-11 새벽).
**사실 한 줄**(`components/fact-line.tsx` — 네 화면의 문장이 한 파일, 낱말은 칩 표, 0 은 안 적음, 숫자 굵게): Sync 「기기 14대 중 9대가 최신 판을 받았습니다. 옛 버전 2대 · 손으로 고침 1대 · 수동 적용 1대 · 보고 없음 1대.」 · Roadmap 「마일스톤 3개 — 시작 전 1 · 진행 중 1 · 완료 확인 대기 1. 완료 조건 9개 중 3개에 근거가 붙었습니다.」 · 제안 「제안 5개 — … 팀장이 볼 것은 승인 대기 1개입니다.」 · Context 「항목 30개 — 적용 중 27 · 초안 3. 발행에 들어가는 것은 「적용 중」 27개입니다.」 `test/web-fact-line.test.ts` 가 넷을 센다.
**볼 것이 있는 행**은 왼쪽 주황 괘선(`tr[data-tone=warn]` — 옛 판·손으로 고침·승인 대기·검토 중, 칩의 tone 이 정한다). 마일스톤 카드는 완료 확인 대기 주황 / 완료 초록, **제목이 id 보다 크다**(`.row-name`), 완료 조건의 ✓/○ → 색점 + 「근거 있음/없음」.
**제안 상세**: 연산별 머리 문장(「…항목을 이렇게 바꾸자는 제안입니다」 · `PROPOSAL_ITEM_SENTENCE`) + **「지금 규칙 / 바꾸자는 규칙」 두 판**(충돌 카드와 같은 모양 · 한 줄 요약 `itemGist` 로 규칙 문장의 변화를 보여 준다 — 본문만 견주던 줄 diff 는 「+0 −0 본문이 그대로다」라 바뀐 것을 못 보여 줬다) · 줄 diff 는 접어 둠. Pack 상세 머리에 세 칸 설명 한 줄.
**사이드 내비**: 사용자가 데스크톱 앱의 사이드바를 보여 주며 「이런 식으로 색깔이 다르면」 — 내비는 옅은 회색 면(`surface-hi`), 본문은 흰 종이로 **면이 갈리고**, 지금 화면 칸만 흰 카드 + 왼쪽 검정 괘선(`data-current`). 머리는 프로젝트 이름(표제체) + 팀 슬러그(작게). 라벨은 표제체 600, 설명은 한 줄(≤ 12자 — 「문서를 넣습니다」「어긋난 것을 정합니다」「승인된 규칙 목록」…, 넘치면 …), 팔레트(⌘K)는 목록 아래로. 링크 글자는 라벨뿐 — GATE 3 그대로.
...
🔴 **열여덟째 판 — Context 를 문서로 · 화면 전부를 처음 보는 비개발자의 눈으로** (사용자: 「Context 화면도 문서처럼 읽기 쉽게 다듬어주고 / 전체적으로 모든 기능들이 일반 사용자·비개발자·처음 보는 사람들이 봐도 안 이상하도록, 바로 이해되도록 코드들을 구현해줘」 · 2026-09-11).
**어떻게 찾았나**: 화면 열한 묶음(랜딩 · 로그인/내 팀/만들기 · 껍데기 · 가져오기 · 정리 · Context · 제안 · Pack · Roadmap · Sync · 근거/칩 조각)에 **감사 에이전트 하나씩**을 「3분 안에 이해해야 하는 비개발자 심사위원」으로 붙여 120건을 모으고, 묶음마다 **반박 에이전트**가 코드·캡처로 대조해 「개발자만 거슬린다 · 지어냈다 · 이미 고쳤다 · 원칙을 어긴다」를 버려 **106건**이 남았다. 그 106건을 공용 조각(칩·근거·상태 표)은 손으로, 화면 아홉은 구현 에이전트 아홉이 **파일을 나눠 갖고 동시에** 고쳤다(공용 파일은 아무도 안 만진다 · 시험은 자기 것만 · git 금지).
**Context 를 문서로** (화면 5): 기본이 **문서로 보기** — 절(종류)마다 머리(사람 말 이름 + 「n개 · 초안 1」)와 「이 절이 무엇인가」 한 줄, 항목마다 블록(제목 → 이름표 한 줄 → 설명 → 근거 수·확신 · 범위 · 꼬리표). 절 차례는 Pack 의 CLAUDE.md 와 같다(`lib/web/context-doc.ts` 의 `CONTEXT_DOC_ORDER`·`CONTEXT_SECTION_LEAD` 표 둘 · 화면은 읽기만). **「적용 중」이 아닌 항목에만** 상태 칩과 색 괘선 — 문서의 기본은 승인된 것이고 예외만 눈에 띈다. [표로 보기]가 옛 표(열은 「종류 | 제목 | 범위 | 상태 | 근거 확신 | 근거 수 | 갱신」 · 「개정」 열은 꼬리표가 이미 말해 뺐다). 거르개의 선택지가 사람 말이고 「scope」는 「범위」. `test/web-context-doc.test.ts` 가 「차례가 10종 전부 · 빈 절 없음 · 예외만 표시 · 기호 없음」을 센다.
**기호를 전부 걷었다**: ✓ ✕ ⚠ ◌ › ○ ▸ 가 서던 40여 자리를 조각 하나 `Note({tone})`(색점 + 글자 · `chips.tsx`)로 — 성공은 ok 점, 봐야 할 것은 warn 점, 실패만 빨강 글자. 팔레트의 고른 줄은 잉크 막대, 랜딩 표 머리·축소판 로드맵은 색점 칩. 터미널 녹화의 `✓ 파일` 은 CLI 가 실제로 찍는 기록이라 남긴다.
**뜻이 없던 자리에 뜻을** (공용 표): 칩 툴팁이 영어 값(`draft`·`update`·`contradiction`·`source_document`)을 그대로 내던 것을 사람 말 표(`CONFIDENCE_MEANING`·`PROPOSAL_STATUS_MEANING`·`PROPOSAL_OPERATION_MEANING`·`SRC_MEANING` · 충돌 종류는 계약의 `hint`)로 — `test/web-tables.test.ts` 가 표마다 「키 = enum · 값이 서로 다름」을 센다. 꼬리표 칩(`CtxTag`)에 「이 항목의 꼬리표입니다 · 개정 n = n번째 고친 판」 툴팁, 8자 코드 앞에 「승인본」, 근거 한 줄은 한글 문장을 본문체로(`SRC_FONT`) · 「문서판 끝8자」(고정 씨앗의 앞 8자는 전부 0 이라 「#00000000」이 빈 자리로 읽혔다) · 「직접 적은 근거」 · 글자 범위 안 접힘 · 머리(「근거」)를 조각이 스스로 든다 · 「로컬에서 열어 보세요」→「개발자가 자기 컴퓨터에서 엽니다」. 사실 한 줄(Sync)은 「최신 판」 대신 칩 이름 「적용됨」을 센다(표와 9 ≠ 11 로 어긋났었다). 「수동 적용」→「zip 으로 받음」, 「승인됨 · 발행 대기」→「승인됨 (발행 대기)」(구분자 「·」와 겹쳤다), 「미션」→「사명」, 「결정 (ADR)」→「기술 결정 기록」, semver 등급은 「작은 고침 / 규칙 추가·변경 / 틀이 바뀜」, `FORBIDDEN` 은 「팀장만」, 배너는 「"Paylab (샘플 팀)"을 둘러보는 중 · 매일 03:00(한국 시간)에 처음 상태로」, 코스 알약은 흰 카드(검정 알약은 화면당 액션 하나)에 설명 한 줄, 팔레트는 「Ctrl+K」와 설명 힌트, 없는 주소는 한국어 `not-found.tsx`.
**화면마다 무엇이 바뀌었나**: **가져오기** — 세 카드가 폭에 따라 자리를 바꾸던 `flex-wrap` 을 격자(`.import-grid`)로 고정해 「붙여넣기 | 결과」가 나란히 서고 질문이 아래 전체 폭이다(빈 문구의 「왼쪽에」가 늘 틀렸다). 씨앗이 **끝난 정리 job 하나**를 남겨 게스트도 진행 막대·「항목 후보 25개」·고르는 목록을 본다(그 목록은 제 안에서 구른다 — 캡처로 보니 25장이 오른쪽 칸을 4,200px 로 늘려 왼쪽이 통째로 비어 있었다 · `--pick-list-h`) — `test/demo-guest.test.ts` 가 「가짜 대기는 없고, 보여 줄 결과는 있다」를 한 쌍으로 잠근다. 「조각/걸음/일/굴리다」 같은 개발자 은유를 걷고, 잠긴 버튼이 왜 안 눌리는지 말한다. 「기록만 남는다」는 「답만 저장됩니다」로(저장하지 않는다고 해 놓고 [저장하기] 를 누르게 했다) — 화면 4 의 같은 고르개도 같은 낱말이다. **정리** — 게스트에게도 결정 버튼 넷과 답 칸이 **잠긴 채 보이고** 그 밑에 읽기 전용 안내가 온다(배너가 「결정 버튼은 사람 몫」이라 말하는데 화면에는 버튼이 없었다). 항목 목록을 못 읽으면 [다시 시도] 가 붙는다. **제안** — 마일스톤이 「PL-M3」 대신 제목으로 서고(찾는 법의 정본은 `lib/web/milestone-titles.ts` 하나 · 화면 8 도 같은 것을 읽는다), 씨앗의 근거가 이유를 되풀이하는 「사람이 적음」이 아니라 **실제 문서판·코드 경로**를 가리킨다(`demo-guest` 가 잰다). 「기준 버전 v1.0.0 f7e58156」은 「이 판을 보고 만든 제안입니다」로. **Roadmap** — 「열린 충돌 12」가 「정리 화면에서 정할 것」이 되고 그 화면으로 가는 문이 붙었다. 늦은 보고는 타일과 **한 글자도 같은** 「21일 이상 보고 없음」 색점 칩이라 어느 행이 그 1인지 눈으로 찾는다(「오래됨」은 이미 충돌 종류의 이름이었다). ▸ 삼각형은 「접기 / 완료 조건 보기」 낱말로, 「근거 2 / 3」은 「완료 조건 3개 중 근거 있음 2」로. **Pack Explorer** — 머리가 「지금 팀의 공식 판입니다」인지 「옛 판입니다 — 지금 공식은 v1.1.0」인지 말하고(옛 판을 열어도 똑같아 보였다), 파일 트리가 폴더/이름 두 줄이며 차례의 정본이 `lib/web/pack-files.ts` 다(CLAUDE.md 가 맨 아래 있었다). `sha256 … 6310B` 는 「확인값 … 6.2 KB」, 빠진 항목은 id 대신 제목·종류로, 첫 진입에 첫 항목 문단이 골라져 오른쪽에 출처가 이미 서 있다.
`tools/ci.ps1`: 2026-09-11 06:12 | principles OK | typecheck OK | test OK | build OK | walkthrough OK | docs OK => GREEN
...
1. ~~**참가 접수** — 9/18(금) 23:59:59 마감 · 접수 뒤 제출 폼의 칸·글자 수·썸네일 규격을 캡처해 `docs/SUBMISSION.md` 「제출 폼 원문」의 상한을 실제 값으로.~~ → ✅ 2026-09-14 실제 폼(일곱 칸)으로 제출 · 캡처 `docs/evidence/2026-09-14-submission-form/` · 2026-09-15 접수 확인.
2. **로그인 실측** (DEPLOY ⑥-b · 10분) — 시크릿 창에서 https://contextops-rosy.vercel.app/login → [GitHub로 계속] → `/t` → 팀 만들기 → Network 의 `POST /api/v1/teams` 가 201 인지 캡처. 콜백에서 튕기면 화면의 원인 코드를 Claude 에게. → ◐ 2026-09-15 production 에서 한 바퀴 돌았다(로그인 → 팀 `mukteam` → 문서 → AI 정리 → 승인 → v1.0.0 발행 · `docs/evidence/2026-09-13-final-audit/README.md` · `docs/DEPLOY.md` 머리의 「충족」 줄) — 201 캡처 파일은 저장소에 없다.
3. ~~**GitHub 저장소 Variables 에 `PROD_ORIGIN`** = 위 URL~~ → 필요 없어졌다(`a078f96` · 변수가 없으면 제출서의 production URL 로 잰다 · 수동 dispatch 로그 `GET / → 200`) — `watch-prod.yml` 이 30분마다 두드린다. 🙋 남은 것: 일부러 틀린 값으로 dispatch 해 실패 메일이 오는지 확인 뒤 되돌린다.
4. ~~**Gemini Tier 1 결정** — 빌링 연결 뒤 키 교체(Vercel env 갱신 + Redeploy) · `apps/web/src/lib/web/privacy.ts` 의 `GEMINI_DATA_TIER` 를 `'paid'` 로~~ → ✅ 2026-09-14 사용자 확인 — 키는 이미 유료(Tier 1)였다 · `GEMINI_DATA_TIER='paid'`(`privacy.ts:25` · `41de61b`). 🙋 남은 것: Google Cloud 결제 예산 알림(지금 런북은 $10 — `docs/DEPLOY.md` 「심사 기간 런북」 · 이 줄의 $20 은 그 전 값).
...
7. ~~**9/17 녹화** (`docs/PITCH.md` §1·§5)~~ → 영상은 안 올린다(2026-09-14 사용자 결정) · **9/18 저녁** release 동결 · Preview 끄기 · 루프 STOP 유지 · ~~**9/19 제출**~~ → 제출은 ✅ 9/14 에 했고 9/19 는 `verify:prod` 한 번(`docs/DEPLOY.md` 「심사 기간 런북」) · 루프 재개 여부는 사람이 정한다.
...
## 🧑 같은 사람 세션 2부 — **대장을 25건 닫았다** (대기 33 → 8)
...
**남은 8건은 전부 「지금 하면 안 되는」 것**이다 (2026-09-15 정정 — 「7건」이라 적었지만 아래 목록은 여덟이다) — 24·33(P3·P4 행이 주인) · 40(50파일 Pack 이
...
> **2026-09-15 현재** (사람 세션이 붙임 · 아래 117바퀴 보고는 2026-09-07 그대로다)
> - **production** <https://contextops-rosy.vercel.app> — 2026-09-10 부터 떠 있다 · `verify:prod` 44/0 → 47/0 · Cron 매일(데모 리셋 03:00 · health 06:00 KST) · watch-prod 30분 예약.
> - **대회** — 2026-09-14 실제 폼 일곱 칸으로 제출 · 09-15 참가 접수 확인 · 9/20 23:59:59 까지 고칠 수 있다(고치면 다시 [과제 제출하기]) · 영상은 안 올린다(사용자 결정).
> - **루프** — 2026-09-07 부터 멈춰 있다(`loop/STOP`). 그 뒤는 전부 사람 세션이고 마지막 push 는 `fa303f5`.
> - **대장 171~177** — 172 stop 훅 git 한도 · 173 플러그인 로드 실패 · 174 웹 가져오기의 충돌 탐지·이전 문서 후보 · 175 저장되는 충돌 카드 질문의 id → 제목 · 176 팀·프로젝트 만들기 안내 · 177 안내 문장의 모양 — 전부 ✅ · 171(로드맵 셋째 타일)은 문구로만 덮고 열림.
> - **🙋 남은 사람 몫** — 새 PC 에서 플러그인 설치·Skill 실기(setup → sync → progress) · 알림 둘(Google Cloud 예산 · watch-prod 실패 메일) · 9/18 저녁 동결(`release` · Preview 끄기) · 9/19 `verify:prod`. PLAN 의 `- [ ]` 는 이 중 🙋 셋(새 PC · 동결 · 9/19)뿐이다.
> - **대장** — 대기 아홉(24 · 29 · 33 · 40 · 45 · 53 · 117 · 152 · 156) · 고장 0. 대기는 아니지만 안 끝난 것: 168 절반 · 171 열림 · 72 ①② · 26·67 ① zip.
> - **아래 한 줄이 45 인 이유** — 09-07 2부가 대기 여덟 중 「지금 하면 안 된다」가 안 붙은 것이 45 하나라 골랐다(위 2부 절 끝). 그 뒤 24·33·53·156 이 기다리던 것(P3·P4 행 · API 키 · 배포)은 채워졌지만(2026-09-15 상태 줄 갱신) 루프가 멈춰 아무도 다시 고르지 않았다. 45 는 여전히 대기라 게이트는 초록이다 — 루프를 다시 켜면 `loop/PROMPT.md` ④3 순서로 다시 골라라.
...
> ⚠ **2026-09-15 대조 — 아래 두 줄(168 을 고른 이유)은 낡았다.** 117바퀴가 쓴 뒤 09-07 2부가 170 을 닫고(`5d024ed`) 168 의 절반을 닫고(`e800e59` · 남은 것은 `valid_from`·`valid_until` · 상태는 대기가 아니다) 위 한 줄을 45 로 바꿨다. 45 인 이유는 이 절 맨 위.
...
> PLAN 의 `- [ ]` 셋은 전부 🙋 사람 몫이다 — 2026-09-15 기준으로 새 PC 실기 · 9/18 동결 · 9/19 `verify:prod` (PLAN P5).

- PLAN 의 `- [ ]` 중 남은 것 **셋**: ~~P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋)~~ → 2026-09-15 대조: P5 의 🙋 셋(새 PC 에서 플러그인 설치·Skill 실기 · 9/18 동결 · 9/19 `verify:prod`). Vercel 은 P5 둘째 행이었고 `375be95`(2026-09-10)로 닫혔고, P6 두 행은 제출(9/14)·영상 안 올림(9/14 결정)으로 닫았다.
- 대장의 대기(~~**168** · 170 · 156 · 152 · 140 · 117 · 33 · 24 · …~~ → 2026-09-15 대조 아홉: 24 · 29 · 33 · 40 · 45 · 53 · 117 · 152 · 156 — 170·140 은 `5d024ed` ✅ · 168 은 절반 ✅ 이라 대기가 아니다) — **고장 0** · 이번 바퀴(117)가 새로 찾은 것은 **하나(170)**. 관통은 GREEN(9단계 · 1275검사)이다.
...
**69 — sync 상태 `manual` 을 찍는 코드가 0곳이다** (격차). 다섯 중 하나가 죽어 있다. → ✅ `9dd032b` 로 닫혔다 (2026-09-08 · 아래는 그 전의 기록).
...
**76 — 「A로 합침」이 합치지 않는다** (고장). → ✅ `a39419c` 로 닫혔다 (①을 골랐다 · 아래는 그 전의 기록). 중복 카드의 버튼은 「합침」이라고 묻는데
...
**74 — 화면 4 가 「A가 맞음」의 결과를 안 알린다** (격차). → ✅ `094102b` 로 닫혔다 (아래는 그 전의 기록). 🔴 **이번 바퀴에 버튼의 뜻이
...
~~⚠ **Anthropic API 키도 사람이 준다.**~~ → ✅ 2026-09-06 사람이 **Gemini** 키를 꽂았다 (공급자를 Gemini 로 바꿨다 · `836a0a9` · 아래 「막힌 것」 표).

✅ **2026-09-15 대조 — 아래 두 문단의 번호는 24·29·33·53 과 zip(26·67 ①) 말고 전부 닫혔다**: 14·17·21·22·23·32 `f736884` · 44·50·57·61·63 `f781ae9` · 46 `5d024ed` · 55 `39c9b04` · 30 `7ba2feb` · 25 `a201a51` · 35 `130b5f2` · 36 `478ba42` · 56 `69c9a80` · 59 `175d339` · 69 `9dd032b`. 공급자는 Gemini 이고 `callClaude()` 는 `callModel()` 이 됐다(`836a0a9`). 24·29·33 의 주인이던 P3 둘째 행은 닫혔지만 셋은 여전히 열려 있고, 53 은 키가 왔지만 대기다 (대장 상태 줄).
...
(**67 ①** 과 같은 자리다 · 2026-09-15 에도 그대로). ✅ **31·65 는 닫혔다** (`c57b3fb`·`5fe0068`).
...
~~🔴 **Supabase 위에서 화면을 연 적이 없다** (71바퀴 · `adac632`).~~ → ✅ **production(Supabase) 위에서 열었다** — 2026-09-10 첫 리셋 200 · 7.6초(`CRON_SECRET` 자물쇠 뒤) · `verify:prod` 44/0 · GATE 3 캡처 5장(`docs/evidence/2026-09-10-production/`) · 2026-09-15 실제 GitHub 로그인 → 팀 → 발행 한 바퀴(`docs/evidence/2026-09-13-final-audit/README.md`). 그때의 기록: 마이그레이션만 적용했고 표는 비어 있다. 못 잰 것: ① `.env.local` 그대로 `next dev` 를 띄우고
...
~~🔴 **게스트 데모 — 고친 뒤 브라우저로 안 봤다** (67바퀴 · FINDINGS 127).~~ → ✅ **봤다** — 관통의 GATE 3 가 빈 창에서 `/` → Context · 제안 · Roadmap · Sync 를 눌러 매 바퀴 밟고(`bd6ddf3` · `docs/evidence/2026-09-07-gate3/`) production 에서도 지났다(2026-09-10 `verify:prod` · GATE 3 캡처 5장). 그때의 기록: → **70바퀴가 부분 봤다** (`docs/evidence/2026-09-06-focus-visible/probe.txt`):
...
~~🔴 **데모 리셋 — 배포에서 돌린 적이 없다** (63바퀴 · 🙋 Vercel 연결 뒤).~~ → ✅ **돌렸다** — 2026-09-10 production 첫 리셋 200 · 7.6초(300초 상한의 1/40 · 항목 30 · 기기 14 를 심었다 — `fixtures/` 를 찾았다 · `docs/evidence/2026-09-10-production/README.md`) · 그 뒤로 Cron 이 매일 03:00 KST 에 돈다 · Cron 로그의 200 자체는 저장소에 기록이 없다. 그때의 기록: PGlite 위에서는 읽었다
...
~~🔴 **화면 1 랜딩 — 브라우저로 본 적이 없다** (59바퀴 · 랜딩 v1).~~ → ✅ **봤다** — 1440×900·375px 캡처(`docs/evidence/2026-09-07-landing-shots/`) · 2026-09-10 리디자인 때 1440·375 로 다시(`docs/evidence/2026-09-10-redesign/`) · [샘플 팀으로 둘러보기] → `/demo` → 코스는 GATE 3 가 매 바퀴 밟는다(`bd6ddf3` · production 2026-09-10). 그때의 기록: 덤프는 읽었다
...
없어서 여전히 SKIP 이다.~~ → ✅ 2026-09-15 대조: 103바퀴(`f29133f`)부터 `apps/web/e2e`(CDP 헤드리스 Chrome · 의존성 0)가 있고 관통 `shots` 단계가 돈다 · 105바퀴 GATE 3(`bd6ddf3`) · 2026-09-10 e2e 61/0 이 `import` 를 포함한 일곱 화면을 찍었다(위 09-10 절 「앱 화면」 행). 그때의 눈 판정은 **두 층으로 갈렸다**:
...
| ~~Anthropic API 키~~ → **Gemini 키** | ✅ **2026-09-06 사람이 꽂았다** (`GEMINI_API_KEY`·`GEMINI_MODEL=gemini-3.5-flash` · 79바퀴가 `836a0a9` 로 갈아끼웠다 · 진짜 호출 통과) | ~~🙋 남은 것 하나: **`gemini-3.5-flash`·`3.6-flash` 의 정가**를 `apps/web/src/lib/ai/features.ts` `AI_MODELS` 에 — 지금은 2.5 flash 공개가(0.30/2.50 USD/M)가 임시로 있다. 틀리면 하루 예산(`AI_DAILY_BUDGET_USD=3`)의 셈이 틀린다.~~ → ✅ 2026-09-10 공개 정가 페이지에서 읽어 고쳤다(`ddd1329` · INBOX G10) — 3.5 Flash 1.50 / 9.00 · 3.6 Flash 0.75 / 3.75 USD/M(2026-12-31 까지 도입가 · `features.ts:175-177`) · Vercel 에도 키가 있다(production health `ai:true`) |
| ~~**참가 접수**(원티드 · 9/18 23:59:59 마감 · 제출과 별개) + 제출 폼의 칸·글자 수·썸네일 규격 캡처~~ | ✅ **2026-09-15 사용자 확인** (과제 제출 9/14 · 폼 캡처 9/14) | 9/20 23:59:59 전까지 고칠 수 있다 — 고치면 다시 [과제 제출하기] |
...
| ~~Vercel 프로젝트 연결 · env · 첫 배포 · 첫 리셋 · verify:prod~~ | ✅ **2026-09-10** — <https://contextops-rosy.vercel.app> · 마이그레이션 0009 까지(Claude) · 첫 리셋 7.6초 · verify:prod 44/0 · 근거 `docs/evidence/2026-09-10-production/` | ~~🙋 남은 것: 로그인 실측(⑥-b) · GitHub 변수 `PROD_ORIGIN`~~ → 로그인은 2026-09-15 production 에서 한 바퀴 돌았다(로그인 → 팀 → 발행 · `docs/evidence/2026-09-13-final-audit/README.md` · 사람 손 스톱워치는 아직) · 변수는 필요 없어졌다(`a078f96` · 변수가 없으면 제출서 URL 로 잰다 · 수동 dispatch `GET / → 200`) · 🙋 남은 것: watch-prod 실패 메일 시험 한 번 |
| ~~Gemini **Tier 1** 결정(AI Studio 빌링 연결 → 키 교체 → Cloud Billing 알림 $20) — 무료 티어는 Google 약관상 입력이 학습에 쓰일 수 있고 심사 트래픽에서 429~~ | ✅ **2026-09-14 사용자 확인 — 이미 유료(Tier 1)였다** · `GEMINI_DATA_TIER='paid'`(`apps/web/src/lib/web/privacy.ts:25` · `41de61b`) · production `/privacy` 한·영 확인 | 🙋 남은 것: Google Cloud 결제 예산 알림 — `docs/DEPLOY.md` 「심사 기간 런북」($10) |
| 실데이터 픽스처(`brain`) 공개 가능 여부 판단 | 제품 결정이다 | P5 (안 되면 paylab 만 · SPEC §14 절삭 6번) |

⚠ 루프는 위 항목을 **추측으로 진행하지 않는다.** 값은 `.env.local` 에만 산다 (P1).
🔴 `SUPABASE_JWT_SECRET` 이 없으면 **HS256 세션은 아무것도 통과하지 못한다** — 우리가 서명하는 게스트·시드 세션과 옛 Supabase 프로젝트가 이 갈래다.
~~아무도 로그인하지 못한다~~ 는 2026-09-09 전의 말이다: 새 Supabase 프로젝트의 GitHub 로그인 토큰은 **ES256** 이고 secret 없이 JWKS 공개키(`kid`)로 확인한다(`VERIFIERS` 표 · `src/lib/api/session.ts:17-25·69-91`).
조용히 통과시키지 않는 것이 의도다.
