...
_마지막 갱신: 2026-09-07 · 루프 95바퀴 · 코드 `d95f7ca`(FINDINGS 157 ✅ — 팔레트의 둘째 묶음(프로젝트 전환) · 새 문 0개 · 시험 +9) · 눈 판정 뒤 목록 높이 한 줄과 문서는 그 다음 커밋_

---

## 지금 어디인가
**이번 바퀴(95)는 FINDINGS 157 — 구멍(팔레트에 프로젝트 전환이 없다)을 닫았다** (`d95f7ca`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **세 바퀴째 같다**).
...
> 155 를 ✅ 로 닫고(커밋 해시는 그 세션의 것) 바로 133 으로 간다 ② 아직 빨가면 **고치지 말고**
> 155 에 한 줄만 더하고 133 을 한다. ⛔ **README·랜딩을 이 루프가 고치지 마라** — 문장의 정본은
> 랜딩 상수라 둘을 같이 고쳐야 닫히고, 사람이 그 세션에 지시하고 있다. 이번이 **네 바퀴째**다.
> ⚠ 155 가 빨간 동안은 **관통도 막힌다**(`api` 단계 = `pnpm --filter web test`). CI 의 `test` 층
> 결과로 대신 읽고, 「관통 못 돌림」을 STATUS 에 적어라.

> **왜 133 인가** — 구멍이 다 닫혔다(156 은 배포 뒤에 재고 · 고르지 마라). 남은 것은 격차이고
> 그 차례의 맨 위가 133 이다 (INBOX 🟡 C · 주인은 PLAN P4 둘째 행).
>
> **133 을 하는 법** — 빈 상태에 **다음 행동**이 없다: 「항목이 없습니다」에서 갈 곳이 없어 첫 사용자가
> 거기서 막힌다. ① 정본은 `components/states.tsx` 의 `EmptyState` 하나다 — 화면마다 버튼을 적지 마라.
> ② 화면별 문구·목적지는 **표 하나**로 모으고(`screens.ts` 옆이 자연스럽다 — 목적지가 곧 화면이다)
> 화면은 그 표를 읽기만 한다. ③ 시험은 「표에 한 줄을 더하면 그 화면의 빈 상태에 저절로 나온다」와
> 「모든 목적지가 실제로 있는 화면이다(404 로 가는 버튼 0)」를 잠근다 — 132·157 이 쓴 것과 같은 모양.
> ⚠ accent 는 **화면당 주요 액션 하나**다 (DESIGN_BRIEF §3). 빈 상태의 버튼이 그 하나가 되는지
> 화면마다 본다 — 화면 5 는 [발행하기] 가 이미 accent 다.
> ⚠ **눈으로 봐라.** 157 에서 시험이 전부 초록인데 사람 눈에는 없는 것처럼 보인 자리가 나왔다.
