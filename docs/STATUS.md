...
_마지막 갱신: 2026-09-07 · 루프 93바퀴 · 코드 `f0e2f25`(FINDINGS 154 ✅ — 멈춘 job 은 되돌리지 않고 새로 만든다 · `AI_JOB_RETRY_RULES` 표 · 시험 +8) · 문서는 그 다음 커밋_
...
> **먼저 5분: `tools/ci.ps1` 을 한 번 돌려 155 를 확인해라.** ① `readme.test.ts` 넷이 **초록이면**
> 155 를 ✅ 로 닫고(커밋 해시는 그 세션의 것) 바로 100 으로 간다 ② 아직 빨가면 **고치지 말고**
> 155 에 한 줄만 더하고 100 을 한다. ⛔ **README·랜딩을 이 루프가 고치지 마라** — 문장의 정본은
> 랜딩 상수라 둘을 같이 고쳐야 닫히고, 사람이 지금 그 세션에 지시하고 있다.
> ⚠ 155 가 빨간 동안은 **관통도 막힌다**(`api` 단계 = `pnpm --filter web test`). CI 의 `test` 층
> 결과로 대신 읽고, 「관통 못 돌림」을 STATUS 에 적어라.

> **왜 131 이 아니라 132 인가** — 131(랜딩 첫 화면에 진짜 제품 그림)은 `landing.tsx` 를 고쳐야 닫히는데
> **그 파일을 지금 다른 세션이 사람 지시로 고치고 있다**(155). 두 세션이 같은 파일을 만지면 되돌리기
> 어렵다. 게다가 131 은 관통의 `shots` 단계에서 나온 **진짜 캡처**를 써야 하는데 그 단계는 아직 SKIP 이다
> (`apps/web/e2e` 가 없다) — 155 가 닫히고 캡처가 생긴 뒤에 한다. **131 을 손으로 만든 그림으로 닫지 마라.**
>
> **132 를 하는 법** — 명령 팔레트 `⌘K`: **화면 이동과 검색만**이다 (INBOX B 의 ⚠ — 과설계 금지 ·
> 하나뿐인 구현에 추상을 만들지 마라). 갈 곳의 목록을 **손으로 적지 마라** — 화면 9개의 경로가 이미
> 어딘가에 표로 있는지부터 찾아라(`components/` 의 내비게이션·`app/t/[team]/p/[project]/` 의 경로).
> 없으면 그 표를 만들고 내비게이션과 팔레트가 **그 하나를 읽게** 한다. 합격은 ⑦3층 화면 목록 +
> 「표에 한 줄을 더하면 팔레트에 저절로 나온다」를 시험이 잠근다.

🔴 PLAN 의 `- [ ]` 맨 위는 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이고, 그 몫인 격차 차례다: **132** → 133 → 134 → 137 → (155 가 닫힌 뒤) 131. ⚠ **100 은 이미 닫혀 있었다** (`a87994d` · 99 와 같은 바퀴) — 92바퀴까지의 STATUS 가 그것을 대기로 나열하고 있었다. 루프가 혼자 닫을 구멍은 없다 — 156 은 배포 뒤에 재고(고르지 마라) · 153·152 는 관찰 중 · 140 은 P5 둘째 행(🙋 새 PC)과 같이 · 117 은 절삭 1번.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(155 · 156 · 153 · 152 · 140 · 117 · 131~134 · 137) — **고장 1**(155 · 주인은 남의 세션) · 154 ✅.
