...
_마지막 갱신: 2026-09-04 · 루프 22바퀴 · `1bc1116`_

---

## 지금 어디인가

**화면 3 이 두드릴 문이 다 났고, 이번 바퀴는 그 문에 「멈췄나」를 붙였다.**
지난 바퀴가 `progress {done,total,unit}` 로 「지금 어디쯤인가」를 붙였는데,
**그 걸음이 언제 있었는지**를 아무도 안 냈다 — `ai_jobs.updated_at` 은 쓰는 자리가
다섯인데 읽는 자리가 0곳이었다 (FINDINGS 64 닫음). 그래서 `running` 인 job 이
10초 전에 움직였는지 40분째 안 움직였는지 같아 보였고, 서버가 chunk 중간에 죽으면
그 행은 영원히 `running` 이라(집기는 `queued` 만 집는다) 화면은 영원히 막대를 그렸다.
이제 목록·상세가 **`updated_at` 과 `stalled`** 를 같이 낸다.
잣대(`stallAfterSec`)는 **러너 표**의 한 칸이다 — 「한 걸음」의 낱말(`unit`)이 이미
거기 있어서, 같은 개념의 길이와 이름이 갈라지지 않게 한 자리에 뒀다
(structure 180초 = 10,000자 조각 하나 · conflict 300초 = 묶음 전체가 걸음 하나).
**판정은 서버가 해서 값으로 나간다** — 잣대가 서버 전용 표에 있고(플러그인 번들로
배포되면 안 된다) 화면의 시계는 서버와 어긋난다. 근거(`updated_at`)가 판정 옆에 있다.

⚠ **여전히 진짜 Claude 를 부른 적이 없다.** 키가 없어 스텁으로만 쟀다 (🙋 사람).
⚠ **화면 3·4 자체는 아직 없다.** 지난 다섯 바퀴는 전부 그 화면이 두드릴 **문**이다.
⚠ **멈춘 job 을 알아보기만 하고 되살리지는 못한다** — `running` 을 `queued` 로
되돌리는 문은 FINDINGS **59**(실패한 job 재시도)와 같은 자리라 거기서 같이 정한다.
⚠ **2-B(죽은 정의 찾기)로 격차 하나를 새로 찾았다** — `source_documents.kind` **6종**이
저장·되돌려주기만 하고 **읽는 코드가 0곳**이다. §7.1 프롬프트가 그 값을 모르므로
로드맵 문서와 잡기가 **똑같이 뜯긴다**(**65**). 그리고 **63**(만드는 라우트만 모양이
다르다)이 또 벌어졌다 — `POST /documents` 가 내는 `{id,status}` 에는 `progress` 도
`updated_at` 도 `stalled` 도 없다.
...
| `apps/web` — 라우트 **30개** · 테스트 **236** | **멈춘 job 을 되살리는 문** (FINDINGS 59+64 — 알아보기는 하는데 되돌릴 자리가 없다) |
| 🔴 **예산 가드** — `lib/ai/{features,budget,client,model}.ts` · `ai_usage` 표 · 시험 18 | `ask`·`demo` 를 부르는 자리 (기능 표의 나머지 둘 · §7.3·§7.4) |
| 🔴 **문서 구조화** — `lib/ai/{structure,prompt}.ts` · `AiStructureOutput` · 시험 24 | **문서 종류를 읽는 코드** (FINDINGS 65 — `kind` 6종이 아무것도 안 바꾼다) · **토큰 발급 화면** (FINDINGS 36) |
...
| 🔴 **job 자리** — `ai_jobs` 표(`progress`·`stalled` 판정 포함) · `AI_JOB_RUNNERS` · 라우트 **4개** · 시험 **37** | `origin='doc'` 을 찍는 코드 (FINDINGS 31 — 없으면 `doc_vs_code` 가 영원히 0건) |
...
검사 층: `principles OK 9 · typecheck 6초·멤버 4 · test 59초·멤버 4 · build 17초 ·
walkthrough` → **GREEN**. 관통 **7단계**
(fixture·compile·api·publish·scan·payload·sync). 남은 SKIP 하나(`shots`)의 prereq 는
`apps/web/e2e`. ⚠ 관통은 §7.1·§7.2 를 **지나지 않는다** — 키가 없다.
시험 합계 **659** (schema 113 · compiler 136 · plugin 174 · web 236).
DB 는 **`0005`** 까지다 — 표 18개 · 인덱스 8개.

## 다음 바퀴가 할 일

🔴 **`docs/PLAN.md` P3 둘째 행 — 웹 화면 3 (가져오기 · 구조화 진행).**
**문은 다 났고, 가벼워졌고, 진행률과 「멈췄나」까지 말한다.** 두드리는 화면만 없다.
🔴 **이제 문을 더 내지 마라 — 화면을 만들어라.** 지난 다섯 바퀴가 전부 이 화면의
문이었다. 문이 하나 더 필요하면 그건 화면을 만들면서 알게 되는 것이지 미리 알 수 없다.
...
- 🔴 **화면 3 이 새로고침 뒤에 하는 질의는 하나다** — `GET /projects/{id}/jobs
  ...
  **「아직 한 걸음도 보고 안 했다」**(총수를 모른다)이다 — 그때만 회전이고 그 뒤로는 막대다.
  ...
  ✅ **「도는 중」과 「멈춤」도 갈린다** (FINDINGS **64** 닫음) — 응답 실물은
  `docs/evidence/2026-09-04-jobs-stalled/stalled.txt` 에 있다.
  🔴 **`stalled` 는 서버가 이미 낸 판정이다 — 화면이 다시 재지 마라.** 잣대
  (`stallAfterSec`)는 서버 전용 표에 있고 브라우저의 시계는 서버와 어긋난다.
  화면이 할 일은 `stalled` 를 그리고 `updated_at` 으로 「N분째 그대로」를 말하는 것뿐이다.
  ⚠ `stalled:true` 여도 **화면이 눌러서 되살릴 문은 없다** (FINDINGS 59+64). 지금
  할 수 있는 말은 「멈춘 것 같다 · 다시 올려라」까지다 — 없는 버튼을 그리지 마라.
...
- **job 을 재는 본보기는 `test/ai-job.test.ts` 다.** 시험은 `runJob()` 을 **직접**
  ...
  그 자리가 job 이 반쯤 간 유일한 순간이다.
  🔴 **「시간이 지났다」를 재는 법**: 기다리지 말고 **행의 `updated_at` 을 뒤로 민다.**
  `toAiJob(row, now)` 의 둘째 인자로 시계를 옮길 수도 있다 (이번 바퀴의 새 시험 5개).
...
⚠ FINDINGS **24·25·29·31·33·35·56·59·63·65** 는 **P3 둘째 행**이, **36** 은 **P4 화면 9** 가,
