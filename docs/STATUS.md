...
_마지막 갱신: 2026-09-04 · 루프 37바퀴 · `6f06881`_

---

## 지금 어디인가

**이번 바퀴는 안 보이는 색으로 그린 문장 다섯을 읽히는 색으로 옮기고, 다시 쌓이지
...
**눈 판정** — 이 바퀴의 고장은 **색**이라 글자만 뽑는 캡처로는 안 보인다. 그래서
...
**2-B 이번 라운드** (`88-B`): `enforcement` 4종(**한 번도 안 팠던 축**)과 `ItemType`
...
🔴 **맨 위는 `87` 이다 — 대기 중에 고장도 구멍도 없고, 87 과 89 둘 다 [격차]다.**
...
**89 — 데모 Pack 이 `ENFORCEMENT_LABEL` 넷 중 하나만 보여 준다** (격차 · 87 보다 싸다).
관통이 낸 Pack 세 파일의 정책 줄이 전부 `강제: 리뷰에서 본다` 다. 죽은 코드는 아니다
(네 값이 다 Pack 을 바꾸고 `liveness.test.ts:72` 가 그걸 잠근다) — **심사자가 읽는
종이에만 한 갈래뿐**이다.

**시작하기 전에 아는 것 (89):**

- 🔴 **고칠 자리는 `apps/web/scripts/seed.ts:78`·`:98` 둘이다.** 둘 다
  `enforcement: 'review'` 다. `seed-questions.ts:66` 은 **건드리지 마라** — 거기서
  `'review'` 로 못 박은 판단은 맞다 (사람이 말로 답한 것을 hook 이 강제한다고 적으면
  거짓이다. 그 이유가 주석에 있다).
- ⚠ **서사와 안 맞는 값을 억지로 넣지 마라.** 데모가 거짓말을 하면 「강제 수단」이라는
  말 자체를 못 믿게 된다. `item_policy_retry` 는 리뷰가 사실이고, hook 이 어울리는 것은
  **로그에 PAN·CVC 를 안 남긴다** 류다 — 그런 항목이 없으면 늘리는 것도 갈래다
  (SPEC §10.1 픽스처 정본을 먼저 읽어라).
- ⚠ 값을 바꾸면 `.ci/walkthrough-pack/` 의 산출물이 따라 바뀐다. golden 이 걸리면
  `loop/PROMPT.md` ③ 의 「expected 를 갱신한 이유를 커밋 메시지에」 규칙을 읽어라.
