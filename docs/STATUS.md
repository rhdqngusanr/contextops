...
_마지막 갱신: 2026-09-04 · 루프 38바퀴 · `5858fbb`_

---

## 지금 어디인가

**이번 바퀴는 데모 Pack 이 「이 정책을 무엇이 강제하나」를 한 갈래로만 말하던 것을
...
🔴 **맨 위는 `90` 이다 — 이번 바퀴에 [구멍]이 하나 생겼다.** 심각도 순서는 언제나
**고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3). 그 다음은 87 · 91 (둘 다 격차).
(**89** 는 `5858fbb` · **88** 은 `6f06881` · **86** 은 `c6905df`+`b1d2190` ·
**84**·**31** 은 `c57b3fb` · **82**·**65** 는 `5fe0068` · **80** 은 `f9962cb` ·
**79** 는 `1aebc22`.)

**90 — 픽스처의 근거 범위가 원문을 안 가리킨다** (구멍 · **P7**).
`seed.ts` 의 `fromDoc()` 이 **모든** 항목에 `start_char: 0, end_char: 400` 을 붙인다.
**일곱 중 여섯은 그 범위 안에 그 항목이 주장하는 문장이 없다.** 관통은 「태그가 붙어
있나」까지만 세고 「따라가면 그 문장이 있나」는 아무도 안 센다.

**시작하기 전에 아는 것 (90):**

- 🔴 **표는 이미 다 재 놨다** — `docs/evidence/2026-09-04-fixture-offsets/offsets.md`
  (`.ci/` 밖이라 다음 관통이 안 지운다). 다시 재지 말고 그걸 읽어라.
  예: 「최대 5회까지 재시도」 goals.md **856자** · 「24시간 안에 종결」 **1154자** ·
  「어떤 로그에도」 **1437자** — 셋 다 근거는 `0–400` 이라고 적혀 있다.
- 🔴 **제일 아픈 것은 `item_road_m1` 이다 — 문서 자체가 다르다.** 근거가 폐기된
  `old-roadmap.md` 를 가리키는데 내용은 `goals.md` §4 의 M1 이다. 그 문서의 M1 은
  「웹훅 수신 v1」이고 경로도 `src/webhook/` 다. 근거 문서를 `goalsVersion` 으로 옮겨라.
- 🔴 **고칠 자리는 `fromDoc()` 하나다.** 근거를 손으로 적는 자리가 거기뿐이다.
  인자에 **원문에서 찾을 문장**을 더하고 `start_char` 를 `문서.indexOf(문장)` 으로
  계산해라. ⚠ **못 찾으면 던져라** — 조용히 `0` 으로 떨어지면 지금과 같은 상태가 된다.
- ⚠ **이번 바퀴가 더한 `item_policy_pii_log` 도 같은 자리를 가리킨다 — 일부러 그랬다.**
  하나만 정확하게 만들면 나머지 여섯이 안 보인다. **일곱 줄을 한 번에** 고쳐라.
- ⚠ 게이트를 같이 올려라: 관통이 태그의 `#start-end` 를 실제 문서에서 잘라 보고
  **그 안에 항목의 문장이 있는지**를 세게 해라. 발표 2:40 이 「Pack Explorer 역추적」이다.

**87 — 근거 글자가 「어느 문서인지」를 말하지 않는다** (격차 · **90 과 같은 사슬의
화면 쪽 면이다.** 90 을 먼저 고치면 여기 붙일 값이 정확해진다).
`SRC_LABEL.source_document` 가 내는 것은 `문서 §결제 › 재시도 · 1240–1520자` 이고
문서 이름도 `document_version_id` 도 없다. 화면 5·7 에서는 다른 문서에서 온 근거가
나란히 놓이는데 그때 둘이 구별되지 않는다.

**시작하기 전에 아는 것 (87):**

- 🔴 **고칠 자리는 `SRC_LABEL` 하나다** (`components/evidence.tsx`). 화면마다 조립하면
  어디선가 빠진다 — 그 함수가 하나인 이유가 파일 머리 주석에 있다.
- ⚠ **첫 걸음은 uuid 앞 8자**다. `proposal` 이 이미 그 모양이다 (`제안 c0ffee00`).
  문서 **제목**까지 가려면 화면이 그 이름을 알아야 하는데 **문서 목록을 내는 문이 없다.**
- ⚠ 표를 고치면 화면 3·4·5·7 이 한꺼번에 바뀐다. `web-tables.test.ts` 의
  「4종이 서로 다른 문자열을 낸다」가 그 표의 게이트다.

**91 — 도메인 규칙 파일이 어느 도메인인지 말하지 않는다** (격차).
`.claude/rules/domain-refund.md` 전문이 네 줄인데 그 안에 「환불」이 **0번** 나온다.
도메인 이름이 **파일 이름에만** 있어서, 도메인이 둘이 되면 두 파일의 본문 머리가
글자 하나 안 틀리고 같아진다.

**시작하기 전에 아는 것 (91):**

- 🔴 **고장이 아니라 가정이 어긋난 것이다.** 템플릿(`templates/index.ts:118`)의 주석이
  「제목은 절이 갖는다(`## {도메인 이름}`)」라고 적어 뒀는데, 그 절을 채우는 것은
  `domain` **ItemType 항목**이다. 파일이 생기는 조건은 「`scope.kind:'domain'` 인 항목이
  하나라도 있다」이고 **둘이 다르다.** paylab 에는 `domain` 항목이 없다.
- ⚠ 파일을 만드는 쪽은 이미 slug 를 안다 (`path: (slug) => …`). `head` 도 그 slug 를
  받게 하는 것이 제일 싸다. **이름**(`환불`)까지 가려면 `domain` 항목이 있어야 한다.
- ⚠ **템플릿을 고치면 golden 이 깨진다.** `loop/PROMPT.md` ③ 의 「템플릿 버전을 올리고
  expected 를 갱신한 이유를 커밋 메시지에」 규칙을 먼저 읽어라.
