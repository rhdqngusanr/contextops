...
_마지막 갱신: 2026-09-07 · 루프 91바퀴 · 코드 `25b9bad`(FINDINGS 112 ✅ — 화면 6 목록이 `?status` 로 걸린다 · `ProposalQuery` · `ProposalStatusFilter` 가 칩 표를 읽는다 · 시험 +4) · 문서는 그 다음 커밋_
...
🔴 PLAN 의 `- [ ]` 맨 위는 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이 다음이고, **59**(실패한 job 을 다시 굴릴 문이 없다 · 격차)는 그 행의 몫이라 그 행을 하는 것과 같다. 루프가 혼자 닫을 구멍은 없다 — 152 는 다음 실패가 200자를 말할 때까지 행동할 수 없고(고르지 마라), 140 은 P5 둘째 행(🙋 새 PC)과 같이, 117 은 절삭 1번. 그 뒤 100 → 131 → 132 → 133 → 134 → 137.

> **59 를 하는 법** — 대장의 「고칠 방향」을 먼저 읽어라(2863줄). ⚠ **아무 실패나 다시 굴리게 하지 마라** — `BUDGET_EXCEEDED`·`RATE_LIMITED` 는 시간이 지나면 풀리는 실패고, `AI_OUTPUT_INVALID` 는 같은 입력으로 다시 굴려도 같은 자리에서 죽는다. **「다시 굴릴 수 있는 실패」의 정본을 표 하나**로 두고(에러 코드 → 재시도 가능?) 서버와 화면 3 이 **그 표를 읽게** 해라 — 화면이 조건을 다시 적으면 그린 버튼이 400 을 받는다 (`PROPOSAL_DECISIONS`·`ACTOR_RULES` 와 같은 모양). `runJob()` 은 `queued` 만 집으므로 문은 `failed → queued` 로 되돌리는 자리 하나다. 합격은 「굴릴 수 없는 코드는 400」 시험 · 표의 항목이 전부 결과를 바꾸는 시험 · CI 초록.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(153 · 152 · 140 · 117 · 100 · 59 · 131~134 · 137) — **고장 0** · 112 ✅.
