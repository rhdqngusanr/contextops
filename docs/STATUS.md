...
_마지막 갱신: 2026-09-04 · 루프 26바퀴 · `e0148d0`_

---

## 지금 어디인가

**이번 바퀴는 P7 의 끊긴 반쪽을 이었다** (FINDINGS **68** · `e0148d0`).
...
⚠ **화면 4(정리)는 아직 없다.** 이제 볼 것이 둘이다 — 충돌 카드와 **씨앗 질문 10장**.
   🔴 화면 4 는 「AI 가 찾은 결정이 필요한 것 N건」이라고 쓴다. `GET /conflicts` 는
   **씨앗 질문도 같이 낸다** — 그 열 장은 AI 가 찾은 것이 아니다. 세는 자리에서
   `kind` 로 갈라라 (배지도 같이).
...
| `apps/web` — 라우트 **30개** · 테스트 **285** | sync 상태 `manual` 을 찍는 코드 (FINDINGS **69**) |
...
검사 층: `principles OK 9 · typecheck 6초·멤버 4 · test 52초·멤버 4 · build 15초 ·
walkthrough 56초` → **GREEN**. 관통 **7단계**
(fixture·compile·api·publish·scan·payload·sync). 남은 SKIP 하나(`shots`)의 prereq 는
`apps/web/e2e`. ⚠ 관통은 §7.1·§7.2 를 **지나지 않는다** — 키가 없다.
시험 합계 **708** (schema 113 · compiler 136 · plugin 174 · web **285**).
DB 는 **`0006`** 까지다 — 표 18개 · 인덱스 8개. 이번 바퀴에 **enum 값 하나**를 더했다
(`conflict_kind` += `seed_question`). CHECK 다섯 줄은 표에서 다시 생성되어 SQL 이 안 바뀌었다.

## 다음 바퀴가 할 일

🔴 **FINDINGS 대장의 「대기」가 69 하나뿐이고, 그건 지금 고를 것이 아니다.**
69 는 「sync 상태 `manual` 을 찍는 코드가 0곳」인데 **먼저 정할 것이 「Pack zip 을
내려받는 길을 만드나」**다. 그 답 없이 손대면 죽은 값을 하나 더 만든다.

**그래서 다음 바퀴는 화면 4(정리)다.** 이유 셋:
1. **P3 둘째 행의 완료 기준(「질문만으로 v1.0 발행」)을 재는 유일한 길**이다. 지금
   온 데까지는 「초안 항목이 생긴다」이고, 발행을 눌러 볼 화면이 없다.
2. 볼 것이 이제 **둘**이다 — 충돌 카드와 **씨앗 질문 10장.** 둘 다 `GET /conflicts` 가 낸다.
3. 그 화면이 없어서 **68 이 만든 `src:proposal:` 을 사람이 볼 자리도 아직 없다**
   (Pack Explorer 화면 7 도 없다). 지금은 시험과 증거 파일이 유일한 눈이다.

**그 다음 후보**: **FINDINGS 67 ①(zip)** — SPEC §11 의 경로·개수·용량 상한이 먼저다
(26 과 한 묶음이다).

**시작하기 전에 아는 것:**

- 🔴 **개정의 `source_refs` 는 초안의 것이 아니다.** 발행은 `withProposalRef()` 로
  `{kind:'proposal'}` 을 붙여서 넣는다 (`lib/api/publish.ts`). 거기서 `draft.source_refs`
  를 다시 읽는 코드를 쓰면 그 한 칸이 조용히 사라진다 — P7 이 반만 이어진다.
- **근거 상한은 `SOURCE_REFS_MAX` 다** (`packages/schema/src/common.ts` · 20).
  ⛔ 20 을 손으로 적지 마라 — 발행이 「자리가 있나」를 그 상수로 묻는다.
- 🔴 **씨앗 질문의 정본은 `apps/web/src/lib/api/seed-questions.ts` 표 하나다.**
  ⛔ **`question` 문장을 고치지 마라 — 행에 그대로 저장되고 그 문장이 행과 표를 잇는
  열쇠다** (`conflicts` 에 씨앗 id 칸이 없다). 고칠 일이 생기면 **줄을 하나 더해라.**
  ⚠ 줄을 더할 때 목적지 칸이 500자보다 좁은 타입(예: `WorkflowData.steps` 300자)을
  쓰면 `test/api-seed-questions.test.ts` 가 그 자리에서 빨개진다. 그게 설계다.
- 🔴 **화면 4 는 「AI 가 찾은 결정이 필요한 것 N건」이라고 쓴다.** `GET /conflicts` 는
  **씨앗 질문 10장도 같이 낸다** — 그건 AI 가 찾은 것이 아니다. 세는 자리와 배지에서
  `kind` 로 갈라라 (`QUESTION_CONFLICT_KINDS` 가 「사람에게 묻는 종류」다).
  ⚠ 갈래가 늘면 `ConflictKindRule` 에 **축을 하나** 더하는 쪽을 먼저 생각해라 —
  지금 안 더한 이유는 **읽는 코드가 0곳이면 그게 죽은 정의**이기 때문이다.
- 🔴 **화면에서 상태 있는 카드를 만들 때는 그리는 쪽을 순수 함수로 빼라.**
  `job-progress.tsx`(6모양) · `question-stack.tsx`(10모양)이 그 본보기다 —
  브라우저가 없어서 **그것만이 눈 판정이다.** 그리고 시험을 쓴 뒤에 **마크업을 직접
  읽어라**: 이번 바퀴에 고친 셋도 지난 바퀴에 고친 둘도 전부 글자를 읽어서 나왔다.
  뽑는 자리는 `apps/web/scripts/dump-question-stack.tsx` 를 본떠라
  (⚠ tsx 는 JSX 를 옛 방식으로 바꿔서 `globalThis.React` 를 꽂아야 돈다).
...
- 🔴 **job 한 장을 그리는 자리는 `components/job-progress.tsx` 하나다.**
  화면 4 가 `conflict` job 을 기다릴 때 **그것을 그대로 쓴다.** 거기에 `feature === …`
  갈래를 만들면 §7.3 이 job 이 되는 날 다시 짜야 한다.
- **화면 4 의 충돌 카드**: `anchor` 가 `items` 면 항목 두 장, `document` 면 원문 구간,
  **`none` 이면 물어보는 문장뿐**이다. 화면이 그 표를 **읽어서** 무엇을 그릴지 고른다
  (한쪽으로 접으면 P7 이 끊긴다).
...
- ⚠ **프로젝트를 만드는 라우트가 이제 행 11개를 만든다.** 개수를 세는 시험을 쓸 때
  `SEED_QUESTIONS.length` 를 더해라 — 이번 바퀴에 기존 시험 8개가 그래서 갈렸다.
- **DB 는 `0006` 까지다.** 표 18개 · 인덱스 8개.
...
⚠ FINDINGS **24·25·29·31·33·35·56·59·63·65** 은 **P3 둘째 행**이, **68** 은
**발행(§2.1)**이, **36** 은 **P4 화면 9** 가, **53** 은 **API 키가 생긴 뒤**가,
**69** 는 **「Pack zip 을 내려받는 길을 만드나」** 가 주인이다.
