...
_마지막 갱신: 2026-09-07 · 루프 89바퀴 · 코드 `eaaaf29`(FINDINGS 118 ✅ — 화면 9 요약이 「공식 vX.Y.Z 기준」 · `OFFICIAL_HINT` · `officialOf` · 시험 +4) · 문서는 그 다음 커밋_

---

## 지금 어디인가

**이번 바퀴(89)는 FINDINGS 118 — 격차(화면 9 가 「그래서 무엇으로 맞춰야 하나」를 안 말한다)를 닫았다** (`eaaaf29`). 표는 기기마다 `v1.1.0 · outdated` 를 그리는데 공식이 지금 무엇인지 화면 어디에도 없었다 — 화면 8 의 요약 타일은 `공식 v1.0.0 기준` 을 적는데 화면 9 만 없었다. INBOX 「할 것」비어 있음 · 관통 7단계 OK(1021) · 고장 0.
...
🔴 PLAN 의 `- [ ]` 맨 위는 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이 다음이고, **116**(`decided_by` 가 uuid 뿐이라 화면 6 이 「누가 결정했나」를 못 그린다 · 격차)은 그 행의 몫이라 그 행을 하는 것과 같다. 루프가 혼자 닫을 구멍은 없다 — 152 는 다음 실패가 200자를 말할 때까지 행동할 수 없고(고르지 마라), 140 은 P5 둘째 행(🙋 새 PC)과 같이, 117 은 절삭 1번. 그 뒤 112 → 59 → 100 → 131 → 132 → 133 → 134 → 137.

> **116 을 하는 법** — 대장의 「고칠 방향」: 문(`lib/api/user.ts` 의 `USER_REF_COLUMNS`)이 이미 있으니 `proposal.ts` 에서 `users` 를 **별칭으로 한 번 더** join 해 `decided_by` 를 `{id,name}` 으로 낸다(작성자 `toProposalWithAuthor()` 와 같은 모양 · 이메일 없음). ⚠ 응답 모양이 바뀌니 **스키마(`packages/schema` 의 Proposal 응답)가 먼저**, 그 다음 `components/proposals.tsx` 의 `ProposalHead` 에 「결정: 이름 · N분 전」 칸. 그릴 자리가 없는 필드를 먼저 넓히지 마라(④2-B). 합격은 `dump-proposals.tsx` 에 결정자 이름이 서고 `web-proposals`·`api-proposal` 시험이 이름을 세고 `@` 가 0 · CI 초록.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(152 · 140 · 117 · 116 · 112 · 100 · 59 · 131~134 · 137) — **고장 0** · 118 ✅.
