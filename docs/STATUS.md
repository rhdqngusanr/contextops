...
_마지막 갱신: 2026-09-07 · 루프 90바퀴 · 코드 `0d60992`·`0b259ac`(FINDINGS 116 ✅ — 화면 6 이 「누가 결정했나」를 이름으로 · `userRefColumns(자리, 표)` · `alias(users,'deciders')` · `DECIDED_BY_LABEL` · 시험 +6) · 문서는 그 다음 커밋_

---

## 지금 어디인가

**이번 바퀴(90)는 FINDINGS 116 — 격차(`decided_by` 가 uuid 뿐이라 화면 6 이 「누가 결정했나」를 못 그린다)를 닫았다** (`0d60992` + 눈으로 잡은 `0b259ac`). 상세는 거절 사유와 시각은 그리는데 **사람**만 없었다 — 113(작성자)과 같은 종류의 구멍이고, 문(`lib/api/user.ts`)이 이미 있어 join 한 번이었다. INBOX 「할 것」비어 있음 · 관통 7단계 OK(1027) · 고장 0.
...
🔴 PLAN 의 `- [ ]` 맨 위는 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이 다음이고, **112**(제안 목록에 거르개가 없다 — 라우트가 `?status` 를 안 받는다 · 격차)는 그 행의 몫이라 그 행을 하는 것과 같다. 루프가 혼자 닫을 구멍은 없다 — 152 는 다음 실패가 200자를 말할 때까지 행동할 수 없고(고르지 마라), 140 은 P5 둘째 행(🙋 새 PC)과 같이, 117 은 절삭 1번. 그 뒤 59 → 100 → 131 → 132 → 133 → 134 → 137.

> **112 를 하는 법** — 대장의 「고칠 방향」: `ProposalQuery = ListQuery.extend({ status: z.enum(PROPOSAL_STATUSES).optional() })` 한 줄(`packages/schema` — 외부 입력은 스키마가 먼저다) → `projects/[id]/proposals/route.ts` 의 `parseQuery` 를 그걸로 바꾸고 조건 한 줄(인덱스가 이미 `(project_id, status, created_at)` 다) → 화면은 **`PROPOSAL_STATUS_CHIP` 표를 읽어** 칩을 그린다(화면 4 의 `CONFLICT_KIND_CHIP` 과 같은 모양 — 손으로 5종을 적지 마라). ⚠ `author` 거르개는 하지 마라 — 고를 이름의 목록을 낼 문이 없다. 합격은 「`?status=rejected` 면 그것만 온다」 시험 · 칩을 누르면 목록이 갈리는 덤프 · CI 초록.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(153 · 152 · 140 · 117 · 112 · 100 · 59 · 131~134 · 137) — **고장 0** · 116 ✅.
