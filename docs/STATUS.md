...
_마지막 갱신: 2026-09-04 · 루프 23바퀴 · `0173c96`_
...
⚠ **화면 4(정리)는 아직 없다.**
⚠ **화면이 거짓말을 하는 자리를 하나 찾았다** — 예산이 소진되면 「샘플 결과를
   표시합니다」라고 말하는데 **샘플을 표시하는 코드가 0곳**이다 (**66**). §7.5 의
   「픽스처 결과」를 고르는 것이 화면 3 의 몫이라고 FINDINGS 52 가 미뤄 뒀는데,
   화면 3 을 만들면서도 안 골랐다 — 지금은 **문구를 줄이는 쪽(66①)이 먼저**다.
...
| `apps/web` — 라우트 **30개** · 테스트 **258** | **실패·멈춘 job 을 되살리는 문** (FINDINGS 59+64) |
| 🔴 **예산 가드** — `lib/ai/{features,budget,client,model}.ts` · `ai_usage` 표 · 시험 18 | **§7.5 「픽스처 결과」를 고르는 코드** (FINDINGS **66** — 그런데 화면은 이미 그걸 약속한다) |
...
검사 층: `principles OK 9 · typecheck 7초·멤버 4 · test 50초·멤버 4 · build 15초 ·
...
시험 합계 **681** (schema 113 · compiler 136 · plugin 174 · web 258).
DB 는 **`0005`** 까지다 — 표 18개 · 인덱스 8개. **이번 바퀴는 DB 를 안 건드렸다.**

## 다음 바퀴가 할 일

🔴 **FINDINGS 66 → 그 다음 67.** 둘 다 P3 둘째 행 안에 있다.

**66 이 먼저인 이유** — 지금 화면이 **거짓말을 한다.** 예산이 소진되면
「샘플 결과를 표시합니다」라고 적어 두고 아무것도 안 보여 준다. 한 줄짜리 갈래(①
문구를 줄인다)가 있고, 그걸 미루면 그 문장은 심사 데모에서 그대로 읽힌다.
⚠ 문구의 **정본은 `docs/DESIGN_BRIEF.md` §5** 다 — `ERROR_HINT` 표만 고치면
`web-tables.test.ts` 가 막는다. 둘을 같이 고쳐라.
⚠ ②(픽스처를 만든다)를 고르려면 SPEC §7.5 를 다시 읽어야 한다 — 「실패한 job」이
아니라 「성공했는데 샘플」이라는 **새 모양**이 필요하고 그건 `AI_JOB_STATUS_RULES`
옆의 결정이다. 한 바퀴에 하나만 만져라.

**67 이 그 다음인 이유** — 🔴 **P3 둘째 행의 완료 기준이 그것이다.**
...
⚠ FINDINGS **24·25·29·31·33·35·56·59·63·65·66·67** 은 **P3 둘째 행**이, **36** 은
