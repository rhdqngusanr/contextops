...
_마지막 갱신: 2026-09-07 · 루프 106바퀴 · 코드 `478ba42`+`f1a27c7`(FINDINGS 36 ✅ — `setup` 이 가리키던 토큰 발급 화면을 화면 9 에: 한 번만 보이는 값 + `contextops setup` 한 줄 통째 복사) · **CI GREEN 전 층** · 관통 1247검사 · 9단계 SKIP 0 · 문서는 그 다음 커밋_

---

## 지금 어디인가
**이번 바퀴(106)는 FINDINGS 36 을 닫았다 — `setup` 이 가리키던 토큰 발급 화면을 화면 9 에 만들었다** (`478ba42` + `f1a27c7`). INBOX 「할 것」비어 있음.
...
> 🔴 **왜 34 인가 — PLAN 이 먼저지만 남은 행 셋이 전부 사람을 기다린다.** ④3 규칙대로 PLAN 이 위인데
> `- [ ]` 셋은 P5 첫 행(🙋 Vercel 계정 · 새 PC)과 P6 두 행(🙋 영상 · 슬라이드)이다 (아래 「막힌 것」).
> 그래서 FINDINGS 로 간다 — **구멍 → 격차** 순의 맨 위다. **대장의 위쪽 격차는 이제 전부 닫혔다**
> (158·159·160 ✅). 남은 구멍 중 156 은 배포 뒤에 재고 · 152 는 「⛔ 지금은 행동할 수 없다」고
> 스스로 적어 뒀고 · 140 은 🙋 새 PC · 117 은 잘랐다 · 33·24 는 「지금 열지 마라」다. 그 다음이 **34** 다.
>
> **34 는 무엇인가** — 발행 트랜잭션이 `pack_files.source_map`(줄 범위 → 항목 ID)을 저장하는데
> **읽는 라우트가 0곳**이다. 화면 7 의 역추적은 본문에 박힌 `<!-- ctx:… -->` 태그를 되읽어서 한다
> (`traceLines`). 즉 **저장된 칸이 아무 일도 안 한다** — ④2-B 가 매 바퀴 찾는 바로 그 모양이다.
> 🔴 **둘 중 하나만 해라** (어중간하게 두지 마라): ① `GET …/packs/{semver}/files/{path}/map` 을 만들어
> 화면이 그걸 읽게 하거나 ② `source_map` 을 지우고 **태그를 정본으로 선언**한다.
> 항목이 스스로 적어 둔 판단은 **②가 유력하다** — 태그는 플러그인이 받는 **바이트 안에** 있어서
> 오프라인에서도 역추적이 되고(P7), `source_map` 은 서버에 물어봐야만 산다.
> ⚠ 지우기로 정하면 **SPEC §2 의 표에서도 지우고** 왜 지웠는지 커밋 메시지에 한 줄 남겨라.
> ⚠ 지우기 전에 먼저 재라 — `grep -rn "sourceMap\|source_map" apps packages plugin` 로 **읽는 곳이
> 정말 0곳인지** 이번에 다시 확인하고, 발행 트랜잭션에서 그 칸을 빼도 golden 이 그대로인지 본다.

- PLAN 의 `- [ ]` 중 남은 것 **셋**: P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(**161**(새로 적었다) · 156 · 152 · 140 · 117 · 34 · 25 · …) — **고장 0** · 이번 바퀴가 새로 찾은 고장도 **0** 이다(관통 GREEN · 9단계).
- 🔴 **새로 적은 것 — FINDINGS 161 [격차]**: 관통이 찍는 `public/shots/*.png` 가 **매 바퀴 바이트가 달라진다.** 이 바퀴에 CI 를 세 번 돌렸는데 세 번 다 어떤 장이 다시 `M` 이 됐고, 랜딩도 화면 5·7 도 한 줄 안 건드렸다. 그림 내용이 아니라 인코딩된 바이트로 보이지만 **아직 안 쟀다** — 매 바퀴 커밋에 코드와 무관한 diff 가 섞여서 「이 바퀴가 무엇을 바꿨나」가 흐려진다. 이번 바퀴는 바뀐 두 장을 `git checkout` 으로 되돌렸다.
- ⚠ **25 는 34 바로 다음이다** — 「충돌 해소가 항목 상태를 갱신하지 않는다」. 막고 있던 것(`a_item_id`)은 `8cde1f5` 로 없어졌고 남은 것은 `choice` → 항목 상태의 **표 하나**(`RESOLUTION_OUTCOME` 옆)다. 주인이던 PLAN P3 둘째 행은 이미 닫혔으므로 **주인 없는 구멍**이다.
