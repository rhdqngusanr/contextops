...
_마지막 갱신: 2026-09-07 · 루프 99바퀴 · 코드 `89d2a82`(FINDINGS 158 ✅ — 쓰는 법을 말하는 placeholder 가 안 잘린다 · 시험 +3) · 눈 판정 근거 3장 + 계산된 폭 4쌍 · 문서는 그 다음 커밋_
...
> 155 를 ✅ 로 닫고(커밋 해시는 그 세션의 것) 바로 159 로 간다 ② 아직 빨가면 **고치지 말고**
> 155 에 한 줄만 더하고 159 를 한다. ⛔ **README·랜딩을 이 루프가 고치지 마라** — 문장의 정본은
> 랜딩 상수라 둘을 같이 고쳐야 닫히고, 사람이 그 세션에 지시하고 있다. 이번이 **여덟 바퀴째**다.
> ⚠ 155 가 빨간 동안은 **관통도 막힌다**(`api` 단계 = `pnpm --filter web test`). CI 의 `test` 층
> 결과로 대신 읽고, 「관통 못 돌림」을 STATUS 에 적어라.

> **왜 159 인가** — 격차 차례에서 158 을 닫았으니 그 다음이 **159**(화면 3 의 「구조화 진행」 빈 상태가
> `아직 올린 문서가 없습니다` 라고 하는데 데모에는 문서가 2건 있다 · 주인은 PLAN P4 둘째 행).
> 98바퀴가 브라우저에서 눈으로 잡아 놓은 것이다.
>
> **159 를 하는 법** — ① 문구는 **화면에 적지 마라.** 정본은 `apps/web/src/lib/web/screens.ts` 의
> `EMPTY_PLACES` 그 한 줄이다 (FINDINGS 133 이 그렇게 옮겼다) ② 그 칸이 세는 것은 **문서가 아니라 job** 이다 —
> 문구를 그렇게 적어라 ③ 목적지(다음 행동 버튼)는 그대로 왼쪽 붙여넣기 칸이다. 바꾸지 마라.
> ④ 재는 법은 이번 바퀴와 같다: `docs/evidence/2026-09-07-demo-no-phantom-job/shot.mjs` 가 그 자리를 연다.

🔴 PLAN 의 `- [ ]` 맨 위는 여전히 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이고, 그 몫인 격차 차례다: **159** → 160 → (155 가 닫힌 뒤) 131. ⚠ 156 은 배포 뒤에 재고(고르지 마라) · 153·152 는 관찰 중 · 140 은 P5 둘째 행(🙋 새 PC)과 같이 · 117 은 절삭 1번.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(155 · 156 · 153 · 152 · 140 · 117 · 131 · 159 · 160) — **고장 1**(155 · 주인은 남의 세션) · 158 ✅.
