...
_마지막 갱신: 2026-09-05 · 루프 49바퀴 · `a87994d` + `3c4ad2f`_

---

## 지금 어디인가

**이번 바퀴는 종이가 같은 말을 두 번 하던 것을 멈췄다 — 항목 하나가 낸 줄 안에서
같은 문장이 글자까지 똑같이 두 번 나오던 세 자리를, 씨앗의 제목 칸만 고쳐서 닫았다**
(FINDINGS **99**·**100** · `a87994d` + `3c4ad2f`).

🔴 **무엇이 문제였나.** 세 절이 각자 다른 모양으로 겹쳤는데 원인은 하나였다 —
**씨앗이 제목을 데이터와 같게 줬다.**
...
다섯 블록 × 두 겹이면 **종이의 절반이 메아리**다. agent 에게는 같은 문장을 두 번
실어 보내는 것이다.

🔴 **템플릿을 깎지 않았다.** 제목이 데이터와 같은 것은 **이 픽스처의 선택**이지
템플릿의 잘못이 아니다 — 다른 팀의 항목은 제목이 다르다. 고친 것은 씨앗의 제목 칸
셋뿐이고, `ARCHITECTURE` 표는 다섯째 칸(`title`)이 늘어 「표에 한 줄」 모양 그대로다.

**잰 것:**

| | 전 | 후 |
|---|---|---|
| 같은 문장을 두 번 적는 항목 | **5자리** (arch 3 · goal 2) | **0** |
| 그것을 세는 검사 | **0개** | **1개** (`pack-echo.ts` · 세 절을 같이 잠근다) |
| `### ` 줄이 「어느 항목의 것」인가 | **아무 항목도 아니다** (FINDINGS 104) | 그 항목의 것 |
| 컴파일러 시험 | 150 | **151** |
| 관통 검사 | 648 | **650** |
| Pack byte | — | **안 변한다** (`traceLines` 는 읽는 쪽 · P4) |
| CI | — | principles OK 9 · typecheck · test · build · walkthrough **650** · docs OK → GREEN |

**게이트가 자기 몫을 했다.** 눈으로 본 것은 두 자리(arch·goal)인데, 검사를 켜자
**셋째**를 짚어 줬다 — 관통이 제안으로 넣는 G3(`item_goal_settlement`)도 제목과
`outcome` 이 `정산 오차 0원` 으로 같았다 (`v1.1.0/CLAUDE.md`). 100 이 「다른 칸도
같이 훑어라」라고 적어 둔 그 자리다.
**빨개지는 것도 봤다** — 제목 둘을 예전 값으로 되돌리니 다섯 자리를 짚으며 FAIL 했다.

🔴 **한 가지가 더 필요했다 (FINDINGS 104).** 게이트가 「항목 하나가 낸 줄」을 물으려면
그 물음의 정본(`traceLines`)이 맞아야 하는데, 그게 `startsWith('#')` 로 **h3까지**
절 머리로 끊고 있었다. 그래서 `### 제목` 줄이 **어느 항목에도 안 칠해졌고**,
화면 7 에서 그 줄만 근거가 사라진다. 절 머리를 내는 자리는 `templates/index.ts`
하나뿐이고 거기 값은 전부 `#`·`##` 다 — 경계를 `^#{1,2} ` 로 좁히고
「`###` 줄은 반드시 칠해진다」를 잠갔다.

**눈으로 읽고 하나를 더 고쳤다** (`3c4ad2f`). `ledger` 의 제목 `돈이 남는 기록` 은
「돈이 남아돈다」로도 읽힌다 — `돈이 쌓이는 장부` 로 바꿨다. **기계는 이걸 안 잡는다:**
메아리 검사는 「같은 문장이 두 번인가」만 세고 「읽히는가」는 안 센다.
근거 캡처: `docs/evidence/2026-09-05-echo/` (CLAUDE.md · architecture.md).

⚠ **게이트가 못 잡는 것을 적어 둔다.** 이 검사는 **문장**이 두 번 나오는 것을 세지
**낱말** 겹침을 안 센다. 100 의 ②(`**PL-M1 M1 — …**`)는 사람 눈이 잡은 것이고 지금도
게이트로는 안 잡힌다 (`PL-M1 M1` 이 조각 하나다). 기준을 낱말까지 내리면
`승인 성공률` 같은 정상적인 되풀이가 전부 빨개진다 — **늘 빨간 게이트는 다음 사람이 끈다.**

**2-B 49바퀴 라운드** (`104-B`): 103 이 남긴 위험(「기대를 표에서 파생시키면 표를
통째로 뒤집는 것을 못 잡는다」)을 들고 **왕복하는 표**를 골랐다. **둘 다 잠겨 있다.**
- **`SRC_TAG` 접두사 4종** — 왕복 시험(`srcKindOf(srcTag(ref))`)은 접두사를 바꿔도
  초록이다(쓰는 쪽과 읽는 쪽이 같이 움직인다). 그런데 `doc` → `ZZdoc` 으로 바꾸니
  **컴파일러 시험 10개**가 빨개졌다 — golden·traceability 가 태그 **본문**을 글자로
  들고 있어서다. 되돌렸다. **왕복이 아니라 골든이 잠근다.**
- **`AI_FEATURE_LIMITS` 빈도 표** — `ask` 를 `분당 3회·actor` → `10분당 30회·project` 로
  바꾸니 **웹 시험 2개**가 빨개졌다. 되돌렸다. 잠근 것은 상한 `3` 을 **손으로 적은** 줄이다.
- ★ **표 밖에 있는 무언가가 기대의 출처여야 한다** — SPEC 문장이든, 산출물 본문 사본이든,
  손으로 적은 수든. 103 의 교훈과 같은 말이고, 이번에 두 표에서 다시 확인했다.

**다음 바퀴의 일 — FINDINGS 101** · [격차] · 「`지표:` 의 낱말이 근거 원문에 없다」
...
「다음에 고칠 것」의 대기는 이제 **둘**이고 전부 [격차]다 — 고장·구멍은 없다.
**101**(`지표:` 의 낱말이 근거 원문에 없다) · **87**(근거 글자가 어느 문서인지 안 말한다).
🔴 **101 은 「하나를 고치면 끝」이 아니다** — 항목이 스스로 적어 뒀다:
씨앗의 `metric` 을 원문 칸 그대로(`'PSP 장애 구간을 포함한 주간 성공률'`) 두면 그 줄은
닫히지만, **다른 항목의 `data` 칸도 같이 훑어야** 다음 칸에서 또 난다.
★ **값싼 게이트가 이미 그려져 있다**: 「씨앗의 `data` 문자열이 근거 원문 발췌 안에 있나」를
관통이 세는 것. 관통은 이미 `repo:` 태그의 줄 범위를 픽스처에서 잘라 본다 —
**같은 모양을 `doc:` 에** 만들면 된다 (`walkthrough-publish.ts` 의 `followEvidence`).
★ 이번 바퀴가 남긴 질문도 들고 가라 — **「기대의 출처가 표 밖에 있나」**.
101 의 게이트는 기대를 **픽스처 원문**에서 뽑으므로 그 조건을 이미 만족한다.
