...
_마지막 갱신: 2026-09-05 · 루프 48바퀴 · `1606e37`_

---

## 지금 어디인가

**이번 바퀴는 P4 의 심장(`SCOPE_ORDER`)에 자물쇠를 걸었다 — 표를 뒤집어도 컴파일러
시험 147개가 전부 초록이던 것을 「빨개진다」로 바꿨다** (FINDINGS **103** · `1606e37`).

🔴 **무엇이 문제였나.** `packages/compiler/src/sort.ts` 의 `SCOPE_ORDER`
(`project:0 · domain:1 · path:2`)는 **죽은 표가 아니다** — 값을 바꾸면 `CLAUDE.md` 안의
줄 순서가 실제로 갈린다. 그런데 그걸 세는 시험이 **0개**였다. 그 파일 머리에
「여기가 P4 의 심장이다」라고 적혀 있는 자리다. **손을 대도 CI 가 초록**이면
「같은 snapshot → byte-identical Pack」은 사람의 조심성에 걸려 있는 주장이다.

🔴 **왜 golden 이 못 잡았나 — 이게 이번 바퀴의 진짜 수확이다.** `SCOPE_ORDER` 는
**priority 가 같고 scope 만 다른 두 항목이 한 절에 설 때만** 일한다. `policy`·`constraint`
는 `byScope()` 가 scope 별로 **다른 파일**로 보내서 그 조건이 안 생기고, 픽스처·golden
어디에도 그런 짝이 없었다. **golden 은 픽스처가 밟는 길만 잠근다.**

**잰 것:**

| | 전 | 후 |
|---|---|---|
| `SCOPE_ORDER` 를 정반대로 뒤집었을 때 | **147개 전부 초록** (golden 3케이스 포함) | **FAIL** (`expected 2 to be less than 1`) |
| `compareItems` 의 scope 비교를 껐을 때 | 초록 | **FAIL** (순서 시험) |
| 그 표를 세는 시험 | **0개** | **3개** (`scope.kind 3종 · 정렬 (SCOPE_ORDER)`) |
| 컴파일러 시험 | 147 | **150** |
| 표의 값 | `project:0 · domain:1 · path:2` | **한 칸도 안 고쳤다 — 지금 맞다** |
| CI | — | principles OK 9 · typecheck · test · build · walkthrough **648** · docs OK → GREEN |

**어떻게 잠갔나 — 자물쇠가 셋인 이유.** 재료는 항목이 적어 둔 그대로다: `roadmap` 은
`PARTITION` 이 scope 를 안 보고 전부 `CLAUDE.md` 로 보내므로, priority 를 같게 두고
scope 만 다른 셋을 넣으면 **순서를 정하는 것이 `SCOPE_ORDER` 뿐**이다. 제목·ID 는
기대 순서와 **반대로** 매겨서, 비교가 빠지면 다음 열쇠(제목→ID)가 정반대를 낸다.
입력 순서를 뒤집어도 같은 순서가 나오는지를 같이 잰다 (P4).

🔴 **그런데 「기대를 표에서 읽어 만들어라」만으로는 부족했다 — 직접 재서 알았다.**
기대를 표에서 파생시키면 **표를 통째로 뒤집을 때 기대도 같이 뒤집혀서** 그대로 초록이다
(뒤집고 돌려서 150개가 초록인 것을 봤다). 그래서 **방향은 정본이 적은 문장**으로 따로
잠갔다 — SPEC §4.1 3단계 `scope(project<domain<path)`. 산출물을 손으로 베낀 기대값이
아니라 **정본의 주장**이라, 이 줄을 고치려면 SPEC 을 같이 고쳐야 한다.
셋째는 **메타 자물쇠**다 — 그 순서가 제목·ID 순과 **다름**을 잰다(둘이 같아지면 첫
시험은 표를 지워도 통과한다).
★ **남길 한 줄** — 「기대를 표에서 읽어라」는 **표가 죽었는가**를 잠그고,
**방향이 맞는가**는 못 잠근다. 순서를 정하는 표는 **둘을 따로** 재라.

**이름도 하나 고쳤다.** 기존 describe 는 `scope.kind 3종 · 배치 (SCOPE_DOC)` 가 됐다 —
「어느 파일로 가나」(배치)와 「누가 먼저 서나」(정렬)는 **다른 표 둘**인데 「scope.kind
3종」이라는 한 이름으로 묶여 보여서, 정렬 쪽이 안 잠긴 채 남아 있었다.
`sort.ts` 의 표 옆에는 **잠그는 자리**를 적었다.

**2-B 48바퀴 라운드** (`103-B`): 103 이 낸 질문(「살아 있는데 안 잠긴 표가 또 있나」)을
**수치·순서** 쪽에서 이어 봤다. **둘 다 잠겨 있다.**
- **분량 한도 2개**(`CLAUDE_MD_MAX_CHARS` 12,000 · `RULES_MAX_CHARS` 30,000)를 1000배로
  올리니 **7개가 빨개졌다** — `budget.test.ts` 4 + golden `case-3-overflow` 3. 되돌렸다.
- **CLAUDE.md 절 순서 표**(`templates/index.ts` 의 `claude.slots`)에서 `roadmap`·`policy`
  두 줄을 맞바꾸니 **golden `case-1-small` 2개**가 빨개졌다. 되돌렸다.
- ★ **golden 이 무엇을 잡고 무엇을 못 잡는지가 갈렸다.** 절 순서처럼 **모든 픽스처의
  본문**을 바꾸는 것은 golden 이 바로 잡는다. 103 이 안 잡힌 이유는 그 표가 **픽스처에
  없는 조건**에서만 일하기 때문이다. 다음 2-B 는 「이 값이 일하는 조건이 픽스처에
  **있나**」를 먼저 물어라 — 없으면 golden 은 초록이다.

**다음 바퀴의 일 — FINDINGS 99** · [격차] · 「아키텍처 블록이 같은 문장을 연달아 두 번」
...
「다음에 고칠 것」의 대기는 이제 **셋**이고 전부 [격차]다 — 고장·구멍은 없다.
**99**(아키텍처 블록이 같은 문장을 연달아 두 번) · **100**(`## Goals` 한 줄도 같은 문장을
두 번) · **101**(`지표:` 의 낱말이 근거 원문에 없다).
...
★ 그리고 **이번 바퀴가 남긴 질문을 들고 가라** — 「이 값이 일하는 조건이 픽스처에 있나」.
99·100 을 잠글 때도 그 검사가 **픽스처가 밟는 길에만** 있으면 다음 팀에서 조용히 샌다.
