...
_마지막 갱신: 2026-09-06 · 루프 57바퀴 · `aee5de2`_
...
(54·55·56바퀴와 같은 이유다. 그 줄의 모양을 넓히는 것이 FINDINGS **109** 다).

> **다음 바퀴의 일 = `docs/PLAN.md` P4 **둘째 행의 남은 조각** — 「게스트 데모 · 랜딩 v1」**
> 화면 9 는 닫혔다 (`8c3e8c5`). 남은 것은 **게스트 데모 테넌트·시드·`/demo/ai-once`** 와
> **랜딩 v1** 이고, 그 둘이 GATE 3(시크릿 창에서 링크만으로 3분 체험)의 전부다.
> **게스트 데모부터 해라** — 랜딩은 「둘러보기」 버튼이 갈 곳이 있어야 서는데, 지금
> `/demo` 도 데모 테넌트도 없다. SPEC §9 아래 「게스트 데모」 문단 · §10.3 시드
> (`fixtures/seed/demo.json` — 팀 1 · 항목 60 · devices 12(applied 9/outdated 2/manual 1)) ·
> DESIGN_BRIEF §4 「게스트 데모 배너」.
> ⚠ **먼저 정할 것**: 게스트는 **세션 쿠키로 읽기 전용**인데, 지금 모든 라우트가
> `requireProject(…, 'member')` 를 지난다 (`lib/api/guard.ts`). 「읽기 전용 게스트」를
> 등급으로 만들지(`ROLE_RANK` 에 한 줄) 아니면 데모 팀에 진짜 member 를 만들지
> **정하고 시작해라.** ⚠ 앞엣것을 고르면 **권한 표가 넓어진다** — 그 표는 P1 의 방어선
> 옆이라 넓히는 값이 크다.
> ⚠ `/demo/ai-once` 는 서버측 LLM 이라 **P3** 가 따라온다 (`withBudget()` 경유 · 4개 기능
> 한정). 그 문이 아직 없으면 배너의 버튼도 만들지 마라 (누르면 아무 일도 안 하는 버튼).
> ⚠ 질의창(§7.3 · FINDINGS 117)은 절삭 순서 **1번**이다 — GATE 3 은 그것 없이도 선다.

- PLAN 의 `- [ ]` 중 **위의 둘은 사람이 막고 있다** (🙋 Supabase · 🙋 Anthropic 키).
- 대장의 대기(118 · 117 · 116 · 115 · 114 · 112 · 111 · 108 · 25 · 33 …)는 **PLAN 을 막지
  않는다** — 적어 두고, 그 항목의 주인이 될 PLAN 행을 할 때 같이 닫는다 (④3 ②).
