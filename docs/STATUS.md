...
_마지막 갱신: 2026-09-07 · 루프 103바퀴 · 코드 `f29133f`(FINDINGS 131 ①② — 관통 `shots` 를 켜고 `shotcopy` 를 더했다 · 진짜 캡처 3장) · **CI GREEN 전 층** · 관통 1172검사 · 9단계 SKIP 0 · 문서는 그 다음 커밋_
...
> **왜 또 131 인가** — 같은 항목의 **③ 이 남았다.** ①② 로 재료(진짜 캡처 3장 + `manifest.json`)가
> 생겼으니 이제 랜딩이 그것을 읽으면 131 이 닫힌다. PLAN 의 `- [ ]` 맨 위는 여전히 **P4 둘째 행**
> (GATE 3 · 눈 판정)이고 131 은 그 행의 몫으로 남은 **마지막 격차**다.
>
> **③ 을 하는 법** — ⛔ **파일 이름을 화면 코드에 적지 마라.** `landing.tsx` 가 `public/shots/manifest.json`
> 을 읽고 `file` · `alt` · `width` · `height` 를 그대로 쓴다 (Next 의 `<Image>` 든 `<img>` 든 폭·높이를
> manifest 가 준다). 그래야 계획(`e2e/plan.ts`)에 한 줄을 더하면 랜딩이 따라온다.
> ⚠ **`landing.tsx` 는 다른 세션이 사람 지시로 만지던 파일이다** (`f87da1b` · 09:48). 손대기 전에
> `git status --short` 로 확인하고, 겹치면 그 바퀴는 **멈추고 STATUS 에 적어라** — 남의 미완성 작업 위에
> 쌓지 마라.
> ⚠ 캡처는 **1440×900** 이라 랜딩 첫 화면에 그대로 넣으면 크다. 폭을 줄일 때 `DESIGN_BRIEF` 토큰 밖의
> 임의 수치를 만들지 마라.
...
- 대장의 대기(156 · 152 · 140 · 117 · **131**) — **고장 0** · 131 은 ①② 만 ✅(`f29133f`) 이고 항목은 그대로 대기.
