...
_마지막 갱신: 2026-09-04 · 루프 35바퀴 · `c57b3fb`_

---

## 지금 어디인가

**이번 바퀴는 「화면이 찾았다고 말하고 아무 데도 안 보내던 길」을 이었다 — 문서에서
뽑은 항목 후보가 이제 항목이 된다** (FINDINGS **84**·**31** · `c57b3fb`).

🔴 **무엇이 문제였나.** 화면 3 에 문서를 붙여넣고 [구조화하기] 를 누르면 §7.1 이 돌고,
끝나면 **「✓ 항목 후보 6개 · 질문 2개를 찾았습니다」** 와 **[Context 보기]** 가 떴다.
그 버튼을 누르면 **Context 가 비어 있었다.** 질문 2개만 충돌 행이 됐고 항목 후보 6개는
`ai_jobs.result` JSON 안에만 있었다 — 꺼낼 문이 **서버에도 화면에도 0곳**이었다.
문서를 올리는 길로 들어온 사람은 발행까지 갈 수 없었다. 제품의 첫 문장(「문서를 올리면
팀 규칙이 된다」)이 중간에 끊겨 있었고, 그동안 관통은 **초록**이었다 — 관통이
`batch-draft`(scan 길)로만 항목을 넣기 때문이다.

**바꾼 것 넷:**

1. **`POST /projects/{id}/jobs/{jobId}/items`** — §7.1 후보가 항목이 되는 **유일한 문**.
   body 는 `AcceptJobItems`(`packages/schema/src/api.ts`)이고 오는 것은 후보의
   **id 뿐**이다. 본문은 **job 의 `result` 에서** 온다.
   ⚠ **본문을 요청으로 받지 않은 것이 이 문의 핵심 결정이다** — 받으면 화면이 모델
   출력을 고쳐 되보낼 수 있고, 그 항목의 근거는 여전히 원문 구간을 가리켜서
   **원문에 없는 문장이 원문을 근거로 배포된다** (P7). 고치는 문은 항목이 된 **뒤**의
   부분 갱신이다.
   ★ 자리를 job 아래로 둔 이유 — 「남의 job 을 못 읽는다」를 지키는 조건
   (`requireProject` + `project_id`)이 **이미 그 경로에 있다.** 최상위면 job id 하나로
   남의 팀 문서에서 뽑은 초안을 자기 프로젝트에 심는다.

2. **`insertDrafts()` (`lib/api/item.ts`) — 초안을 행으로 만드는 유일한 자리.**
   전까지 그 코드가 `batch-draft` 와 `questions` 두 곳에 **베껴져** 있었고 각자
   ①`status` 를 서버가 정하는가 ②요청 안 중복 ③이미 있는 id ④`origin` 을 따로 정했다.
   이제 문마다 고르는 것은 **`origin` 한 칸**뿐이다.
   ⚠ **`status` 를 인자로 두지 않았다** — 두는 순간 「초안은 언제나 draft」가 부르는 쪽
   셋의 합의가 되고, 하나만 어긋나면 승인 없이 `active` 가 생긴다.

3. **`origin:'doc'` 을 찍는 자리가 생겼다** (FINDINGS **31** — 0곳이었다).
   그래서 §7.2 의 `doc_vs_code` 가 **실데이터로 날 수 있는 상태**가 됐다. 전에는
   한쪽 값이 영원히 0건이라 탐지가 도는데도 그 카드가 절대 안 나왔다.

4. **화면 3 의 성공 카드가 「고르는 자리」로 끝난다**
   (`components/structure-candidates.tsx`). DESIGN_BRIEF §2-4 「AI 결과 카드는 항상
   사람의 선택 버튼으로 끝난다」가 **이 화면에서 처음 지켜졌다.** 기본은 전부 선택 —
   빼는 것이 고르는 것보다 싸야 그 화면이 쓰인다. 캡션이 누르기 **전에**
   「초안으로 들어갑니다 · 승인은 Context 화면에서 합니다」를 말한다.

**게이트는 「소비처가 생겼다」에서 멈추지 않았다** (2-B ②단계):
`ai-job.test.ts` **+8** — job 이 끝나도 항목 **0건**(사람이 고르기 전엔 행이 없다) ·
고른 것만 행이 됨 · 같은 프로젝트에서 `doc` 과 `code` 가 **갈림** · 근거가 문서 버전
uuid 를 그대로 뭄(P7) · 후보 아닌 id 거절 · 둘째는 「이미 있다」 · 안 끝난 job 과 탐지
job 은 400 · 남의 프로젝트 job 은 404.
`web-structure-candidates.test.ts` **+12** — 고른 수가 버튼에 따라오고, 0개면 잠기고,
만든 뒤 문구에 **「발행」이라는 낱말이 없다**(있으면 사람이 다 끝났다고 읽는다).

**눈으로 읽은 것** — `docs/evidence/2026-09-04-structure-candidates/card.txt`
(일곱 상태: 기본·부분선택·0개·저장중·실패·만든뒤·빈상태). 브라우저가 없어서
마크업에서 글자를 뽑아 읽었다 (`scripts/dump-structure-candidates.tsx`).
읽고 판정한 것:
- ✅ 고른 수가 버튼에 따라온다(5 → 3) · 만든 뒤에 「초안」이라고 말한다 ·
  타입을 아이콘만으로 말하지 않는다(이름 병기) · 빈 상태에 다음 할 일이 있다.
- 🔴 **못 미더운 것 하나를 찾았다 → FINDINGS 86**: 후보 한 줄이
  `[v] | § | 재시도 정책 | policy` 로 끝난다 — **근거가 없다.** 사람은 어디서 온
  문장인지 모른 채 팀 규칙이 될 초안을 받아들인다 (DESIGN_BRIEF §2-1 정반대).
  ⚠ 근거는 **이미 데이터에 있다** (`source_refs`) — 화면이 안 꺼낼 뿐이다.

**SPEC 도 비어 있었다.** §7.1 은 「사람이 화면 4 에서 고른다」고만 적고 **그 문을 §5 표에
안 적었다** — 코드가 SPEC 을 어긴 게 아니라 둘 다 비어 있었다 (82 와 같은 모양이다).
§5 에 한 줄 · §7.1 마지막 줄을 고쳤다.

⚠ `packages/schema` 가 바뀌어 **플러그인 번들을 다시 만들었다**
(`pnpm --filter @contextops/plugin build` — `test/bundle.test.ts` 가 그것을 잰다).
안 하면 CI `test` 층이 그 한 파일 때문에 빨개지고, **웹 시험은 아예 안 돈다**
(pnpm 이 첫 실패에서 멈춘다 — 이번 바퀴에 실제로 밟았다).

## 다음 바퀴가 할 일

🔴 **맨 위는 `86` 이다 — 이번 바퀴 눈 판정이 찾은 [격차]이고, 대기 중에 고장도
구멍도 없다.** 심각도 순서는 언제나 **고장 → 구멍 → 격차**다 (`loop/PROMPT.md` ④3).
(**84**·**31** 은 `c57b3fb` · **82**·**65** 는 `5fe0068` · **80** 은 `f9962cb` ·
**79** 는 `1aebc22`.)

**86 — 후보 줄에 근거가 없다** (격차). 84 가 낸 후보 고르기 카드가 한 줄에
**아이콘 · 제목 · 타입** 셋만 낸다. 사람은 「재시도 정책 · policy」 다섯 글자만 보고
체크를 남겨 두고, 그 항목은 다음 Pack 에 나갈 초안이 된다.
DESIGN_BRIEF §2-1 「**근거 없는 숫자·판정은 화면에 없다**」와 정반대다.

**시작하기 전에 아는 것 (86):**

- 🔴 **근거는 이미 데이터에 있다.** `structureDocument()` 가 chunk offset 을 문서
  offset 으로 바꿔 `source_refs:[{kind:'source_document', document_version_id, …}]` 를
  채워 두었고, 받아들인 항목의 근거가 그것이라는 것을 `ai-job.test.ts` 가 잰다.
  **화면이 안 꺼낼 뿐이다** — `structureCandidates()`(`lib/web/queries.ts`)가
  `id`·`type`·`title` 셋만 낸다. 고칠 자리는 그 함수와 카드 둘이다.
- 🔴 **초안 전체를 화면 상태로 들고 오지 마라.** 그러면 다음 사람이 그것을 고쳐서
  되보내는 문을 만들게 되고, 근거는 여전히 원문 구간을 가리켜 **원문에 없는 문장이
  원문을 근거로 배포된다** (P7 · `AcceptJobItems` 주석에 그 판단이 적혀 있다).
  **읽기 전용으로 보여 주기만 하는 칸**을 늘리는 것이 답이다.
- ⚠ **원문 인용까지 가려면 문서 본문을 가져올 문이 필요하고 지금은 없다.**
  `document_version_id#start-end` 를 **글자로** 내는 것이 첫 걸음이다
  (화면 7 의 `EvidenceLink` 와 같은 모양). 인용은 그 다음 바퀴다.
- ⚠ **눈 판정 자리가 이미 있다** — `scripts/dump-structure-candidates.tsx` 가 일곱
  상태를 글자로 뽑는다. 고친 뒤에 그것을 다시 돌려 **마크업을 직접 읽어라.**
