...
<https://contextops-rosy.vercel.app> — 첫 화면에서 문제(같은 질문에 5번 대 3번)를 보고 [샘플 팀으로 둘러보기]로 들어갑니다. 로그인 없이 바로 체험: <https://contextops-rosy.vercel.app/demo> · 저장소 <https://github.com/rhdqngusanr/contextops>.
...
AI에게는 흩어진 문장에서 규칙을 뽑고 서로 어긋난 곳을 찾는 일만 맡기고, 결정과 배포는 사람과 정해진 코드가 합니다. ① 정리: 팀장이 붙여 넣은 문서를 Gemini가 규칙·마일스톤 등 10종 항목으로 나누고 근거 문장을 인용합니다. 서버가 그 인용을 원문에서 직접 찾아 확인하고, 못 찾으면 받지 않고 다시 묻습니다(지어낸 근거 차단). ② 충돌 찾기: 항목끼리 다르게 말하는 곳을 판정하지 않고 "어느 쪽을 따를까요?" 질문 카드로 올리며, 결정은 팀장이 합니다. ③ 개발자 쪽: 팀원 본인의 Claude Code에서 플러그인(Skill·훅)이 저장소를 로컬로 훑어 코드 근거(파일 경로·줄 번호)와 진행을 보고합니다. 코드 본문은 서버로 가지 않습니다. 승인 뒤 배포에는 AI가 없어 같은 승인본은 언제나 똑같은 파일이 되고, 모든 줄이 원문까지 되짚힙니다. 서버 AI 호출은 하루·한 달 예산 상한(월 $10) 안에서만 나갑니다. 실측: 문서 둘(4,220자)에서 항목 후보 22 · 충돌 5 · 인용 26/26 일치 · 약 29초 · 약 $0.08. 로그인 없이 데모의 [AI 에게 지금 직접 찾게 해 보기]로 한 번 돌려 볼 수 있습니다.

### 사용한 AI 툴 (≤ 200자)

Claude Code — 설계·구현·시험·배포 전 과정(자율 개발 루프 · 9/3부터 커밋 350여 개)과 제품 플러그인(Skill · 훅)의 실행 환경. Gemini API(3.5 Flash) — 서버측 문서 구조화·충돌 탐지(우리 API 키 · JSON 스키마 출력 · 월 $10 상한). Codex CLI — 랜딩 일러스트 이미지 생성.
