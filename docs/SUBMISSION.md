...
> **Wanted AI Championship 2026** · 2026-09-14 제출(9/20 23:59:59 까지 고칠 수 있다) · 개발 1인 + Claude Code
>
...
| 항목 | 값 | 어디에도 같이 적나 |
|---|---|---|
| 제출 팀명 | 퇴직했는데저좀이직시켜주세요 | 정본은 `SUBMISSION_IDENTITY`(`apps/web/src/lib/web/submission.ts`) · README 머리 · 랜딩 푸터 (`LANDING_FOOT`) — 셋이 같은 글자인지 `apps/web/test/readme.test.ts` 가 잰다 |
...
| 2분 영상 링크 | 🙋 안 올린다 — 만들어 둔 세로 쇼츠도 올리지 않기로 했다 (2026-09-14 사용자 결정) · 규정·제출 폼에 영상 칸이 없다 · 올리면 투표·본선 자산 | 쇼츠를 만드는 명령은 `tools/make-video.ps1` · 옛 컷 표·대본은 [PITCH.md](PITCH.md) |
| 슬라이드 링크 | 🙋 없음 — 규정·제출 폼에 없는 칸이다 · 본선(TOP20)에 오르면 발표용으로 만든다 | 8장 뼈대는 [PITCH.md](PITCH.md) |
...
- **제출물 넷** — 배포된 **서비스 링크(정상 작동 필수)** · 해결하고자 한 문제 · AI 활용 방식 · **사용한 AI 툴 이름과 활용 방식(필수 기재)**. 임시저장은 제출로 인정하지 않는다. 영상·슬라이드·썸네일은 요구하지 않는다. ⚠ 실제 제출 폼(2026-09-14)은 일곱 칸이었고 대표 이미지와 16:9 스크린샷이 필수였다 — 아래 「제출 폼 원문」.
...
- **이 제출서가 그 규정에 답하는 자리** — 링크 → 🙋 표의 production URL(`/demo` · 로그인 벽 없음) · 문제·AI 활용·AI 툴 → 아래 「제출 폼 원문」 · 개인정보 → 데모 데이터는 전부 합성 팀(`paylab`)이고 실제 개인정보가 0건 · 라이선스 → MIT(`LICENSE`) · Gemini API 이용 조건은 `docs/KNOWN_LIMITATIONS.md` 「AI 처리 데이터의 행방」이 맡는다.
...
2. **Pack Explorer** — 발행된 CLAUDE.md 를 열어 아무 문단이나 누르면, 그 문단이 어느 문서 몇 절·어느 코드 몇 줄에서 왔는지 보입니다.
3. **Roadmap** — 마일스톤마다 완료 조건 3개 중 몇 개에 근거가 붙었는지 보입니다. 근거는 개발자의 AI 가 보고한 파일 경로이고, 완료 확인은 사람이 합니다.
4. **Sync** — 기기 14대가 어느 버전을 받았는지 보입니다. 받은 파일이 공식 판과 정말 같은지 서버가 한 줄씩 대조하니 「적용됨」을 믿을 수 있습니다.
...
1. **가져와서 정리한다** — 목표 문서·회의록을 붙여 넣으면 AI 가 규칙·마일스톤 후보를 뽑고 서로 어긋난 것을 찾아 질문으로 만듭니다. 문서가 없으면 질문 10개에 답해 시작합니다. 결정은 사람이 합니다.
...
코어(컴파일러 · 스키마 · 플러그인 · 웹)는 MIT 로 공개하고 셀프호스팅은 무료입니다. 호스팅 서비스를 연다면 팀 단위 구독에 AI 비용을 사용량만큼
투명하게 얹는 모양을 가설로 둡니다(위 표의 「비용」이 그 단가입니다). 대회 뒤 먼저 만들 것은 SSO 와 감사 로그(누가 무엇을 승인했나)입니다 — 팀장의 승인 흐름이
이미 그 기록을 남기고 있습니다. 지금 배포(Vercel Hobby · 비상업)에는 가격도 결제도 없습니다.

## 신뢰 경계 — 깨면 안 되는 원칙 7개

[`tools/principles.ps1`](../tools/principles.ps1) 과 시험이 커밋할 때마다 **기계로 셉니다** — P7 의 「줄마다 태그」는 컴파일러 시험(`packages/compiler/test/traceability.test.ts`)이 잽니다. 문장은 README 와 같습니다.
...
- **개발 전 과정 Claude Code** — 이 저장소는 자율 루프(`loop/`)가 만들었습니다. 한 바퀴마다 새 헤드리스 세션이
  ...
  2026-09-07 에 루프를 멈춘 뒤로는 사람이 Claude Code 세션으로 같은 문서를 읽으며 다듬었습니다.
  이 루프는 배포되는 제품이 아니라 개발 도구이므로 P2 의 예외입니다 (`CLAUDE.md`).

## 어떻게 보나

**샘플 팀** — <https://contextops-rosy.vercel.app/demo> 를 열면 게스트 세션으로 샘플 팀에 들어갑니다. 읽기 전용이고 매일 새벽 3시쯤(KST) 초기화됩니다.
...
- **배포는 Vercel Hobby 위입니다** — cron 은 하루 1회(리셋이 03:00 정각이 아니라 그 시간 안 임의 분에 돕니다) · 함수 300초 · 리전 하나(서울). 심사 기간의 링크 유지는 GitHub Actions 감시(30분 간격으로 걸었지만 실제 간격은 몇 시간까지 벌어진다)와 「옆자리에 심고 바꾸는」 리셋이 지킵니다 (`docs/DEPLOY.md` 「심사 기간 런북」).
