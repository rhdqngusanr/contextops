...
| 참가 접수 | 🙋 (원티드 계정 · **2026-09-18(금) 23:59:59 마감**) — 2026-09-14 에 제출 폼까지 열었다. 필수 일곱 칸을 채우고 **[과제 제출하기]** 까지 9/18 전에 누른다 · 임시저장은 제출이 아니다 · 제출한 뒤에도 9/20 마감 전까지 고칠 수 있다(폼 안내) | 폼의 칸·상한은 캡처대로 아래 「제출 폼 원문」에 옮겼다 — 사본 [evidence](evidence/2026-09-14-submission-form/README.md) |
...
## 제출 폼 원문 — 실제 폼 일곱 칸 (2026-09-14 확인)

> 폼에 **글자 그대로** 붙여 넣는 원문이다. 칸 이름·차례·모양은 사용자가 2026-09-14 에 연 실제 폼(`event.wanted.co.kr/ai-championship/2026/apply` 「과제 제출하기」)에서 옮겼고, 일곱 칸 전부 필수(*)다.
> 글자 세기가 보이는 칸은 「AI 활용 방식 및 결과」(0/500) 하나이고, 「제목」·「해결하고자 한 문제」는 한 줄 입력칸이다. `apps/web/test/readme.test.ts` 가 칸 이름·상한·한 줄·그림 파일(16:9)을 잰다.
> 「사용 AI툴 및 기술 스택」은 **고르는 칩**이라 글을 못 적는다 — 유의사항이 요구하는 「주요 AI 도구의 명칭과 활용 방식」은 500자 칸 안에 적었다. 폼 칸·유의사항 사본은 [evidence](evidence/2026-09-14-submission-form/README.md). 코드에 없는 기능은 적지 않았다.

### 대표 이미지

`docs/evidence/2026-09-14-submission-form/1-landing.png` — 첫 화면(16:9). 폼 안내는 「서비스 대표 이미지 또는 스크린샷」.
...
팀원마다 AI 지식 파일(CLAUDE.md·AGENTS.md·Cursor 규칙)이 달라 같은 질문에도 답이 갈릴 때, 어긋난 규칙을 AI가 찾고 정한 하나로 맞춥니다.

### AI 활용 방식 및 결과 (≤ 500자)

AI는 결정을 대신하지 않고, 결정에 필요한 정리와 확인을 맡습니다.

팀장이 문서를 붙여 넣으면 Gemini API가 목표, 정책, 로드맵 같은 항목으로 나누고 근거 문장도 뽑습니다. 서버는 그 문장이 원문에 있는지 확인해 AI가 근거를 지어내지 못하게 합니다. 또 AI가 규칙끼리 어긋나는 곳을 찾아 "어느 쪽을 따를까요?"라는 질문으로 보여 주면, 답은 팀장이 고릅니다.

각 개발자의 Claude Code 플러그인은 저장소를 훑어 코드 근거(파일과 줄 번호)와 작업 진행을 보고합니다. 코드 내용은 서버로 보내지 않습니다. 승인 뒤에는 AI가 끼어들지 않아 같은 내용은 항상 같은 파일로 배포됩니다.

문서 두 개(4,220자)로 시험한 결과 약 29초 만에 항목 후보 22개와 충돌 5건을 찾았고, 비용은 약 $0.08였습니다. 데모에서는 로그인 없이 충돌 찾기를 직접 해 볼 수 있습니다. 개발에는 Claude Code, 랜딩 이미지에는 Codex CLI를 썼습니다.

### 사용 AI툴 및 기술 스택 (칩 선택)

Next.js · React · Claude · Gemini · Vercel · Supabase — 폼의 칩 22개 중 실제로 쓴 것만 고른다. Codex CLI 는 칩이 없어 위 500자 칸에 이름과 쓴 곳을 적었다.

### 서비스 링크

https://contextops-rosy.vercel.app
...
첫 걸음(정리) 화면 맨 위의 **[AI 에게 지금 직접 찾게 해 보기]** 로 AI 충돌 탐지를 로그인 없이 한 번 돌려 볼 수 있습니다 — 결과는 저장되지 않습니다.
...
팀 전체의 AI 에 같은 지침을 나눠 주는 방법은 이미 있습니다 — Claude Code 관리형 CLAUDE.md · Cursor Team Rules · GitHub Copilot 조직 지침 · 오픈소스 ruler · rulesync.
파일을 **나르는** 일은 그들이 합니다. ContextOps 는 나르기 전에 무엇이 팀의 규칙인지 정하고(어긋난 곳을 AI 가 찾아 묻고 팀장이 결정),
나른 뒤에 모두가 같은 판을 받았는지와 계획이 어디까지 왔는지를 근거로 보여 주는 층입니다. 여러 도구 형식으로 옮기는 폭과 끌 수 없게 강제하는 힘은 그쪽이 더 넓고 셉니다 —
대신하지 않고 그 위에 얹습니다. 도구별 비교표와 출처(2026-09-14 확인)는 [README](../README.md) 의 「비슷한 도구와 무엇이 다른가요?」 절에 있습니다.
...
없습니다. 이 제출서는 그 기능을 적지 않습니다 (`docs/KNOWN_LIMITATIONS.md` 「서버측 AI 4종 중 하나는 문이 없다」 · FINDINGS 117).
...
| 제품 안(서버) | Gemini API — 문서 구조화 · 충돌 탐지. `withBudget()` 한 문 · 하루·한 달 예산(월 $10) · 빈도 상한 | 우리 API 키 |

랜딩의 클레이 그림(히어로 · 단계 그림)은 **Codex CLI 의 이미지 생성**으로 만들고 사람이 골랐습니다 — 제품 기능이 아니라 소개 페이지의
그림이라 위 표에서는 뺐고, 규정의 「사용한 AI 툴」 칸에는 적었습니다. 프롬프트와 원본은 `docs/evidence/2026-09-10-redesign/art/` 에 있습니다.
...
「이 샘플 팀에서는 다시 찾지 않고 그때의 기록을 그대로 보여 줍니다」라고 스스로 말합니다. AI 가 **지금 도는** 장면은 같은 화면 맨 위의
[AI 에게 지금 직접 찾게 해 보기] 입니다 — 새 메모 한 줄(셋 중 하나)을 고르면 같은 AI 가 승인된 규칙과 어긋나는 곳을 몇 초 만에 질문으로 올리고,
걸린 시간·토큰·값을 그대로 보여 줍니다. 결과는 저장되지 않습니다 (`POST /demo/ai-once` · 2026-09-13).
...
- **서버측 AI 4종 중 하나(질의)는 문이 없습니다** (FINDINGS 117).
