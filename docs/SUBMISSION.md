...
같은 질문에 팀원마다 AI 답이 다를 때, 어긋난 규칙을 AI가 찾고 팀장이 정한 답 하나로 맞춥니다.

### AI 활용 방식 및 결과 (≤ 500자)

① Gemini API(서버): 팀 문서를 규칙·마일스톤 등 10종 항목으로 나누고 근거 문장을 인용합니다. 인용은 서버가 원문에서 찾아 확인하고, 없으면 다시 묻습니다(지어낸 근거 차단). 항목끼리 어긋난 곳은 판정 없이 "어느 쪽을 따를까요?" 질문으로 올려 팀장이 고릅니다. ② Claude Code: 팀원 본인의 플러그인(Skill·훅)이 저장소를 로컬에서 훑어 코드 근거(파일·줄 번호)와 진행을 보고합니다. 코드 본문은 서버로 가지 않습니다. 이 서비스 자체도 Claude Code 자율 루프로 만들었습니다(커밋 350개 이상). ③ Codex CLI: 랜딩 그림 생성. 승인 뒤 배포에는 AI가 없어 같은 승인본은 늘 같은 파일이 됩니다. 결과 — 실측: 문서 둘(4,220자)에서 항목 후보 22 · 충돌 5 · 인용 26/26 일치 · 약 29초 · 약 $0.08. 로그인 없이 데모에서 AI 탐지를 바로 돌려 볼 수 있습니다(약 2초).
