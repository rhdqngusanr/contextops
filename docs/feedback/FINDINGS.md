...
### 161. **관통이 찍는 캡처가 매 바퀴 바이트가 달라진다** — 랜딩이 읽는 그림 3장이 코드와 무관하게 워킹트리를 더럽힌다   [격차]
...
- **왜 고장이 아닌가**: 랜딩은 정상으로 그려지고 게이트(`web-landing-shots.test.ts` 24개)도 초록이다.
  그림의 **내용**이 달라지는 것이 아니라 인코딩된 바이트가 달라진다(폰트 래스터·타이밍으로 보인다 — **아직 안 쟀다**).
- **왜 값이 나나**: ① 매 바퀴 커밋에 **코드와 무관한 diff** 가 섞여서 「이 바퀴가 무엇을 바꿨나」가 흐려진다
  ② 남의 세션이 같이 일할 때 이 파일들이 늘 충돌 후보다 ③ `.png` 라 리뷰가 무엇이 달라졌는지 못 본다
- **고칠 방향**: **먼저 재라** — 두 캡처의 픽셀이 실제로 다른지(같은 픽셀에 다른 바이트면 인코더 문제다),
  다르다면 어디가 다른지. 그 다음에 고른다: ① 바뀐 장만 옮긴다(`publish-shots.ts` 가 **픽셀 해시**를 견줘
  같으면 안 쓴다 — 파일 바이트가 아니라 그림이 정본이다) ② 찍는 조건을 더 조인다(폰트 로딩 완료를 기다린다)
  ③ 그림을 저장소에서 빼고 배포 때 만든다(⚠ 랜딩이 **읽는** 파일이라 빌드가 관통에 묶인다 — 제일 비싸다).
  ①이 제일 싸고 「그림이 낡으면 관통이 먼저 안다」(FINDINGS 131)를 안 깬다.
- **상태**: 대기 (주인 없음 · 값싸다 · 관통을 만지는 바퀴에 같이)
