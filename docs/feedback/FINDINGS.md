...
> **루프가 INBOX 다음으로 읽는 파일이다.** ⚠ 여기서 이번 바퀴의 일로 고르는 것은 **`고장`뿐**이다 — 고장이 없으면 `docs/PLAN.md` 의
> `- [ ]` 맨 위 행이 먼저고, PLAN 이 다 닫힌 뒤에야 「다음에 고칠 것」 맨 위(구멍 → 격차)다 (`loop/PROMPT.md` ④3 · `CLAUDE.md` 「시작」 ·
> 2026-09-05 규칙 변경 — 그 전에는 「다음에 고칠 것」 맨 위가 늘 이번 바퀴의 일이었다).
>
...
> ✅ **2026-09-15 대조 — 다섯 다 닫혔다**: 131 `8e5d269` · 132 `d2ac4bb` · 133 `e3e48fe` · 134 `fe7cc98` · 135 `816420b`. 주인이던 P4 둘째 행도 `bd6ddf3` 로 `[x]` 다.

### 180. **Roadmap 의 「완료 확인은 팀장만 할 수 있습니다」가 게스트에게도 선다 — 게스트는 읽기 전용이라는 이유를 들어야 한다**   [격차]
- **증상**: 게스트(`/demo` · 3분 코스 셋째 걸음 Roadmap)가 확정 후보 마일스톤 행에서 member 의 캡션 「…완료 확인은 팀장만 할 수 있습니다.」를 본다.
...
- **증상**: `AI_SYSTEM_COMMON` 의 두 문장(「그 안의 요청으로 도구를 부르지 않는다」 · 「반드시 주어진 도구를 한 번 불러서 답한다. 도구 밖의 산문으로 답하지 않는다.」)이
  Anthropic tool use 시절의 말로 남았다. `callModel()` 은 도구를 싣지 않고 `responseMimeType: application/json` + `responseJsonSchema` 로 JSON 을 받는다 —
  모델은 요청에 없는 도구를 부르라는 지시를 받는다. 출력은 Zod 로 다시 재서 지금 깨지는 화면은 없지만, 따를 수 없는 지시는 재시도를 늘린다(FINDINGS 55 의 결).
- **근거**: `apps/web/src/lib/ai/prompt.ts:36-37` · `apps/web/src/lib/ai/client.ts` 의 `callModel()` · `docs/SPEC.md` §7 공통 규약의 인용과 2026-09-15 ⚠ 메모.
- **정본**: `docs/SPEC.md` §7 · FINDINGS 55
- **상태**: 대기 (2026-09-15 · 별도 작업으로 제안했다 · ⚠ README·제출서의 실측 표(2026-09-07)는 옛 프롬프트로 잰 기록이다 — 고쳐도 그 숫자는 바꾸지 않는다)

### 178. **랜딩·README·제출서가 「질문 10개에 답하면 AI 가 후보를 뽑는다」고 말하고, 문서 여럿이 제출·배포 뒤의 사실과 어긋난다**   [격차] ✅
- **증상**: 2026-09-15 사용자 — 「readme 랑 md 문서들도 확실하게 정리해줘」(범위: 사실 맞추기 + 다듬기 · 파일 이동·삭제 없음). 문서의 사실을 코드와 대조한 감사(네 갈래)에서:
  ① 랜딩 「어떻게 동작하나요」 첫 걸음(한·영)과 README · 제출서가 같은 문장으로 「목표 문서·회의록을 붙여 넣거나 질문 10개에 답하면, AI 가 규칙·마일스톤 후보를 뽑고…」라고 말했다 —
  기본 질문의 답은 표가 정한 자리로 옮겨질 뿐 AI 를 거치지 않는다. README 시험은 세 곳이 **같은 글자**인지만 재서, 셋이 같이 틀려도 초록이었다.
  ② 공개 문서의 낡은 사실 — README 머리의 개발자 메모가 `SUBMISSION_IDENTITY` 를 옛 파일에 있다고 적음 · 「제출 2026-09-20」 · CLI 표에 `setup` 없음 · 「setup 이 남기는 것 둘」 ·
  DEPLOY 의 월 예산 기본값 0(실제 $10) · 오래 도는 문 4(5) · items 27(30) · RLS 18(19) · 「넣지 않는 것 둘」(여섯) · CLAUDE.md 「CI 5층」(6) · 「에러 코드 9종」(12).
  ③ 내부 기록 — 끝난 할 일이 대기로 남음(PLAN 의 열린 행 셋이 전부 끝났거나 사람 몫 · STATUS 머리 「마지막 갱신」 셋 · INBOX 「할 것」) · SPEC·DESIGN_BRIEF 의 없는 기능
  (Realtime · Tailwind · 질의창 · 발행 모달의 추천) · Skill 설명이 CLI 동작과 어긋남(오프라인 `sync --check` 의 exit · `manual` 상태 · init 의 exit 30) · 루프 README 가 「창을 닫아도 계속 돈다」.
- **근거**: `apps/web/src/app/api/v1/projects/[id]/questions/route.ts`(답 → `draftForAnswer` · AI 호출 없음) · `apps/web/src/lib/web/submission.ts:19` · `apps/web/src/lib/ai/features.ts:205` ·
  `apps/web/src/lib/api/vercel.ts` 의 `LONG_RUNNING_ROUTES` · `packages/schema/src/api.ts` 의 `ERROR_CODES` · `plugin/contextops/src/cli/commands.ts` · `plugin/contextops/src/cli/sync.ts:117-121`.
- **정본**: `docs/SUBMISSION.md` 머리(「여기 적힌 기능은 전부 지금 코드에 있는 것」) · CLAUDE.md 「SPEC 을 현실로 믿지 마라」
- **고친 것**: ① 세 곳의 문장을 「붙여 넣으면 AI 가 … · 문서가 없으면 질문 10개에 답해 시작」으로(랜딩 한·영 · README · 제출서). 같은 틀린 말이 176 에 이어 두 번째라 게이트로 올렸다.
  ⚠ 원티드에 실제로 낸 폼 일곱 칸의 문장에는 이 말이 없다 — 제출물은 고칠 것이 없다.
  ② 공개 문서(README · 제출서 · 피치 · 알려진 한계 · 배포 · CLAUDE.md)를 2026-09-15 사실로 · README 머리의 개발자 메모는 「기여」로 옮겼다 · 루프를 켜면 커밋이 곧 production 배포라는 경고.
  ③ 루프 안내 · Skill 설명 · SPEC · DESIGN_BRIEF · STATUS · PLAN · INBOX 는 틀린 사실과 끝난 할 일만 고쳤다(자르거나 바꾼 것은 지우지 않고 그 자리에 날짜와 근거) ·
  이 파일의 머리말과 상태 줄(#13 · #107 닫음 · #29 · #105 에 빠진 상태 줄 · #24 · #33 · #53 · #156 주인 메모).
  ⚠ `loop/HANDOFF.md` 에 새 규칙 하나 — 심사 기간에 `main` 에서 돌린다는 사람의 결정이 적혀 있지 않으면 루프는 아무것도 고치지 않고 멈춘다.
  감사가 찾은 코드 쪽 틈 둘은 179 · 180 으로 적었다.
- **시험**: `web-landing.test.ts` 「기본 질문의 답은 AI 를 거치지 않는다」 — 답을 받는 문과 초안 모듈이 `lib/ai`·`withBudget`·`startJob` 을 안 부른다 ·
  한·영 첫 걸음이 「질문에 답하면 → AI」로 잇지 않는다(옛 문장으로 돌리면 빨개지는 것을 확인했다). 나머지는 기존 게이트
  (readme · deploy-doc · watch-prod · web-privacy · markdown-emphasis · spec-drift · design-tokens · status-shape · skills)가 초록인 채로 고쳤다.
- **상태**: ✅ 2026-09-15 사람 세션

### 177. **안내 문장이 설명과 한 문단에 섞여, 할 일과 명령이 눈에 안 들어온다**   [격차] ✅
- **증상**: 2026-09-15 사용자 — 「명령들 텍스트를 좀 눈에 잘 보이게 해 줘. 지금은 그냥 엄청 긴 설명문 보는 느낌이야」(범위: 전체적으로).
  할 일(「이름을 적으세요」·「이 한 줄을 붙여넣으세요」)과 명령(`/contextops:sync`·`/contextops:propose`)이 이유·배경과 같은 11.5px 회색 문단에 섞여 있었다.
  Sync 기기 추가 카드는 「그 저장소에서 Claude Code 를 열고 이 한 줄을 그대로 붙여넣으세요」 한 문장이 라벨 글씨로 서고 명령은 맨 글자였고,
  Sync 요약의 다음 걸음은 명령 하나가 세 문장짜리 문단 가운데 묻혀 있었다. 176 에서 칸마다 설명을 붙인 팀·프로젝트 만들기도 설명이 칸보다 길어졌다.
- **근거**: `components/sync.tsx` 의 `IssuedDeviceCard`(`commandLead` 라벨 + `<code>` 맨 글자) · `SYNC_NEXT_STEP`(한 문단) ·
  `app/t/[team]/p/[project]/import/page.tsx` 의 머리 문단·붙여넣기 카드 설명 · `lib/web/screens.ts` 의 `proposals.list` 빈 문장 · 176 의 두 화면.
- **정본**: `docs/DESIGN_BRIEF.md` §3 「공통 컴포넌트」(안내 문장) · §4 화면 2 · 3 · 9
- **고친 것**: 시안 셋(굵은 한 줄 · 번호 걸음과 접는 설명 · 할 일 상자 — 비공개 아티펙트로 실제 문구·제품 토큰으로 그림) 중 **번호 걸음과 접는 설명**을 골랐다.
  모양은 `components/guide.tsx` 하나다 — `Steps`/`Step`(검정 번호 원 + 굵은 할 일 제목) · `Why`(이유는 질문 한 줄로 접는다) ·
  `CommandBox`(복사할 명령은 검은 상자 + 복사 버튼) · `CommandText`(문장 속 명령만 모노 칩 · 명령 머리는 스키마의 `SETUP_COMMAND_NAME` 에서 자른다).
  적용: 팀·프로젝트 만들기(칸마다 걸음 · 「한글도 되나요?」「만든 뒤에 바꿀 수 있나요?」 등은 접음) · 가져오기(머리는 할 일 한 줄 + 붙여넣기 걸음 셋) ·
  Sync 기기 추가(① Claude Code 를 연다 ② 이 한 줄을 붙여넣는다 ③ 닫는다 · 「토큰만 따로 필요하면요?」는 접음) · Sync 요약의 다음 걸음(명령은 보이고 달라지는 것은 접음) · 빈 상태 문장 속 명령.
  🔴 접지 않는 것 — 토큰은 한 번만 보인다 · AI 전송 고지 · 버튼이 잠긴 이유. 누르기 전에 읽어야 하는 문장이라서다.
  ⚠ 예시 알약(테두리 알약)은 버렸다 — 이 앱에서 테두리 알약은 버튼의 얼굴이라 누르면 채워지는 줄 안다. 예시는 `meta` 한 줄이다.
  ⚠ 랜딩의 설치 칸은 이미 같은 모양(번호 · 검은 명령 상자)이라 손대지 않았다.
- **시험**: `web-guide.test.ts`(모양 넷의 마크업 · 번호를 글자로 안 적는다 · 접힌 채로 시작 · 문장 속 명령 · 모양의 클래스 이름이 `guide.tsx`·`globals.css` 밖에 없다 · 네 자리가 모양을 쓴다 · AI 전송 고지는 안 접힌다) ·
  `web-setup-guide.test.ts` ④(칸은 걸음이고 제목이 이름표 · 제목은 할 일, 접힌 줄은 질문 · 잠긴 이유는 안 접힌다) · `web-sync.test.ts`(명령은 보이고 사정은 접힘) ·
  `web-add-device.test.ts`(명령이 검은 상자 · 걸음 셋의 차례 · 경고가 걸음보다 먼저, 접힌 설명 밖). 빈 상태·제안 목록 시험은 문장을 글자로 재도록 고쳤다(명령이 `<code>` 로 끊긴다).
- **상태**: ✅ 2026-09-15 사람 세션

### 176. **로그인 뒤 팀·프로젝트 만들기에 안내가 없어 무엇을 넣을지 모르고, 만들면 빈 Context 로 떨어진다**   [격차] ✅
- **증상**: 2026-09-15 사용자 — 「로그인 → 팀 프로젝트 만들 때 전부 가이드가 없으니 뭘 넣어야 할지 감이 안 잡힌다」.
  두 화면에는 칸 이름과 placeholder 뿐이었다: 「팀」이 회사인지 개발팀인지 · 한글 이름이 되는지 · 주소용 이름을 나중에 못 바꾼다는 것 ·
  팀원은 어디서 초대하는지 · 만든 뒤 무엇을 하는지를 아무 데서도 말하지 않았다. 프로젝트 이름 예시(`paylab-api`)가 저장소 이름 예시와 같아
  두 칸이 헷갈렸고, 팀 이름이 한글이면 「결제팀 팀에 만듭니다」가 됐다. 한글 프로젝트 이름은 버튼만 잠기고 이유가 안 떴다.
  프로젝트를 만들면 빈 Context 표(`…/context`)로 떨어졌다 — DESIGN_BRIEF 화면 2 는 「완료 시 바로 화면 3으로」였다.
- **근거**: `apps/web/src/app/t/new/page.tsx` · `apps/web/src/app/t/[team]/p/new/page.tsx`(이동 `…/context`) · 로그인한 사람에게는 게스트 코스(`DemoTour`) 같은 안내가 없다.
- **정본**: `docs/DESIGN_BRIEF.md` §4 화면 2
- **고친 것**: 사용자가 시안 셋(칸마다 설명 · 옆에 예시 · 시작 안내와 질문형 한 장 — 비공개 아티펙트로 실제 문구·제품 토큰으로 그림) 중 **A** 를 골랐다.
  두 화면 머리에 걸음 줄(`1 팀 → 2 프로젝트 → 3 첫 문서`) · 칸마다 한 줄 설명 · 팀원 초대 자리 · 만든 뒤 갈 곳을 말하고, 문장은 `components/setup-guide.tsx` 의
  `SETUP_TEXT` 한 곳에 뒀다. 프로젝트를 만들면 **가져오기**로 간다. 이름 예시는 `Payments`, 첫 문장은 「「결제팀」에 만듭니다」, 한글 프로젝트 이름에도 주소용 이름 안내가 뜬다.
  ⚠ 문장을 쓰다 잡은 것 — 시안의 「질문 10개에 답하면 AI 가 규칙 후보를 만든다」는 틀렸다(기본 질문의 답은 표가 정한 자리로 옮겨질 뿐 AI 를 안 거친다) → 「문서를 붙여넣으면 AI 가 · 문서가 없으면 기본 질문에 답해 시작」.
- **시험**: `web-setup-guide.test.ts` — 걸음 줄 · 두 화면이 표를 읽는다 · 사실 문장 셋이 코드와 같다(팀·프로젝트를 고치는 PATCH 문이 없다 · 「내 팀」이 설명과 초대를 실제로 그린다 · 만든 뒤 가는 곳이 `/import`).
- **상태**: ✅ 2026-09-15 사람 세션
...
- **증상**: `plugin/contextops test/hooks.test.ts > 🔴 P6 … > stop 이 바꾼 경로가 _writes 선언과 정확히
  ...
  단독 재실행은 24/24 · 3.49초 초록이고, 그 커밋은 `plugin/` 을 한 줄도 안 건드렸다.
  기록된 것만 90바퀴 · 92바퀴 · 2026-09-10 · 2026-09-11 — **네 번**이다.
- **⚠ 153 이 아니다**: 153 은 `testTimeout`(vitest 가 시험을 죽이는 시간)이고 `30f0fd8` 로 이미 닫혔다.
  이 시험은 시간 초과로 죽은 것이 아니라 **끝까지 돌고 단언에서** 실패했다(상한 30초 · 실제 4670ms).
  증상이 비슷해 2026-09-11 세션이 한 번 153 으로 잘못 묶었다 — 그래서 여기 따로 적는다.
- **근거**: `docs/evidence/2026-09-12-git-budget/` (잰 스크립트와 수). `stop.mjs` 의 `git()` 이
  `execFileSync(timeout)` 을 `try/catch` 로 감싸 실패를 `''` 로 돌려주므로 **「저장소가 아니다」와
  「시간 초과」가 같은 값**이다 → `changedPaths()` 가 비고 훅이 「변경 없음」으로 물러선다.
  실측: 한가할 때 git 두 번 합 62~92ms(한 번 최악 39ms) · CPU 16개 부하 154~420ms ·
  **실제 `pnpm -r test` 중 68~1393ms(한 번 최악 1235ms)** — 한가할 때의 약 **32배**.
  옛 한도 `GIT_TIMEOUT_MS = 1400` 은 그 최악의 **1.13배**뿐이었다.
- **정본**: `docs/SPEC.md` §8.6 · P6 · `plugin/contextops/hooks/hooks.json` 의 timeout(5초)
- **왜 한도를 올리는 것으로 못 고쳤나**: 단계마다 고정 한도를 주면 최악이 `2 × git + 네트워크` 라
  **git 은 1500 위로 못 간다**(네트워크를 0 으로 해도). 2026-09-10 의 1000→1400 이 그 한계에 부딪힌
  시도였고 다음 날 또 났다. **튜닝으로 못 여는 자리였다.**
- **고친 것** (2026-09-12): 단계별 한도 대신 **마감 하나**. `HOOK_BUDGET_MS = 4800` 을 두고 각 단계가
  「자기 상한」과 「마감까지 남은 시간」 중 작은 쪽을 쓴다(`withinBudget()`). 보통은 git 이 60ms 에
  끝나 남은 예산이 네트워크로 가고, 느린 날엔 git 이 **안 쓰인 예산을 빌려** 2800ms 까지 버틴다
  (실측 최악의 2.3배). 합은 어떤 경우에도 4800 < 5000 이다.
- 🔴 **게이트 셋을 더하고 뒤집어 확인했다** (`test/hooks.test.ts` 「timeout 이 …」): ① 마감이
  `hooks.json` timeout 보다 작은가 ② **단계 상한이 마감보다 작은가**(크면 한 번도 안 걸리는 죽은 수다)
  ③ **모든 단계가 `withinBudget()` 를 거치는가**(하나라도 그냥 쓰면 그 단계만 마감 밖에 선다).
  ②③ 은 각각 `2800→9000` · `withinBudget(X)→X` 로 뒤집어 **실제로 빨개지는 것을 봤다**.
- **상태**: ✅ (`tools/ci.ps1` GREEN 2026-09-12 05:27 · plugin 단독 3회 연속 200 통과 · 관통 1529)

### 171. 로드맵 셋째 타일은 「결정을 기다리는 것」을 종류별로 못 가른다   [고가치]
- **증상**: 화면 8 의 셋째 타일이 12 를 세는데, 정리 화면(화면 4)은 같은 것을 「충돌 3 · 팀에게
  묻는 질문 9」로 가른다. 심사위원이 1분 사이에 같은 것을 두 숫자로 본다.
- **근거**: `GET /api/v1/projects/{id}/roadmap` 이 `conflicts.status='open'` 을 전부 세어
  `conflicts: number` 하나로 낸다 (`route.ts:91-93·141`).
- **지금 한 것** (2026-09-11): 타일 라벨을 정리 화면과 같은 낱말로 바꾸고, 힌트가
  「정리 화면의 「전체」와 같은 수입니다」라고 **말해서** 두 숫자가 왜 다른지는 읽히게 했다.
- **고칠 방향**: 응답을 `{ conflicts: { ai: number, ask: number } }` 로 넓히고
  `packages/schema` 계약 · `lib/web/queries.ts` 의 로드맵 타입 · `test/api-routes.test.ts` 를
  **같은 바퀴에** 고친다. 화면은 그때 「충돌 n · 팀에게 묻는 질문 m」으로 그린다.
- **상태**: 열림 (문구로 덮어 뒀다 · 수를 가르는 것은 계약 변경이다)

### 170. **`item_goal_settlement_zero` 의 `body` 도 제 `metric` 을 말만 바꿔 되풀이한다**   [격차]
- **증상**: 169 를 고치며 같은 표(`GOALS`)의 셋째 줄에서 하나 더 봤다. 169 가 적어 둔 둘과 **같은 갈래**인데,
  169 를 쓴 바퀴가 종이를 읽을 때 이 줄을 못 봤다.
  | 항목 | `data.metric` | `body` |
  |---|---|---|
  | `item_goal_settlement_zero` | 일 배치 후 원장 대사 차액 | 일 배치 뒤 원장 대사 차액으로 잰다 |
- **근거**: `.ci/walkthrough-pack/CLAUDE.md` Goals 절 셋째 줄 (117바퀴가 눈으로 읽었다) · `apps/web/src/lib/demo/seed.ts` 의 `GOALS`
- **왜 게이트가 안 무나**: 「후」 → 「뒤」 한 글자라 `pack-echo` 의 **들어 있음**(169 가 더한 규칙)에도 안 걸린다.
  말을 바꾼 되풀이는 사람이 읽고 잡는다고 그 게이트 옆에 적어 둔 그대로다 — 기준을 낱말까지 내리면 늘 빨개진다.
- **정본**: `docs/SPEC.md` §10.3 (데모 씨앗)
- **고칠 방향**: 169 와 같다 — `body` 는 세 칸이 **안 말하는 것**만 적는다 (`GOALS` 표 옆 주석의 규칙).
  ⚠ 원문(`fixtures/paylab-docs/goals.md`)에 있는 말로 적어라. G3 에 붙일 말이 원문에 마땅치 않으면
  **`body` 를 비우는 것**도 답이다 — roadmap 셋이 이미 그 모양이다.
  ⚠ 둘째 줄(`item_goal_refund_sla`)은 이 갈래가 아니다: `metric` 이 식(`refund.closed_at - …`)이고
  `body` 는 그 식을 사람 말로 푼 것이라 **안 말하는 것**을 적고 있다.
- **상태**: ✅ 5d024ed (`body` 를 **비웠다** — 원문에 G3 밖의 말이 없다. 「여기를 채우지 마라」를 표 옆에 남겼다)

### 169. ✅ **데모 씨앗의 `body` 가 제 `data` 를 말만 바꿔 되풀이하는 자리가 둘 남았다** (`89c80e9`)   [격차]
...
- **상태**: ✅ 닫음 (117바퀴 · `89c80e9`) — 둘의 `body` 를 `data` 가 **안 말하는 것**으로 바꿨다:
  `item_goal_success_rate` 는 「G2 와 서로 당긴다 — … 둘이 부딪히면 G2 가 우선이다」(원문 §2 표 아래 두 줄),
  `item_constraint_settlement_batch` 는 「원장(`ledger`)은 append only 라, 그날의 정산은 배치가 끝난 뒤에야
  확정된다」(원문 §7). `GOALS` 표 옆 주석의 「줄여 쓴 요약이 필요하면 그 자리는 `body` 다」를 **「`body` 는 세 칸이
  안 말하는 것만 적는다」**로 바꿨다 — 그 문장이 이 되풀이를 만들었다.
  **게이트로 올렸다**(같은 지적 둘째다 · 99·100 이 첫째): `pack-echo` 가 이제 **한 조각이 다른 조각 안에 통째로
  들어 있는 것**도 문다(「…성공률」 ⊂ 「…성공률로 잰다」). 길이 문턱은 따로 **16자**다 —
  6자면 `payment` 가 제 경로 `src/payment/` 안에 있어 20건이 늘 빨개진다(실측).
  `test/pack-echo.test.ts` 5개가 무는 것 둘 · **안 무는 것 둘**(이름⊂경로 · 말을 바꾼 되풀이)을 못 박는다 —
  이 검사는 관통에서만 불려서 무엇을 무는지 아무도 안 재고 있었다. 관통 1270 → **1275검사**.
  ⚠ 같은 갈래가 한 줄 더 남았다 → **170**.
...
- **상태**: 🟡 **절반 ✅ `e800e59`** — `owner_id` 는 살았다(화면 5 가 이름으로 그린다 · 씨앗이 값을 준다 · 게이트 셋). `tags` 는 **처음부터 살아 있었다** (화면 5 가 그리고 씨앗이 `paylab` 을 준다 — 이 항목의 「화면도 안 읽는다」는 낡은 근거였다). 남은 것은 `valid_from`·`valid_until` 둘이다.
- **남은 둘을 왜 지금 안 했나**: 원문(`fixtures/paylab-docs/*`)에 그 두 날짜의 근거가 없다. 씨앗에 넣으면 169·170 에서 막 지운 **「말만 바꾼 되풀이」**가 되고, 컴파일러로 살리는 길은 시각을 읽어야 해서 P4 가 막는다. **근거가 있는 날짜가 원문에 생기는 바퀴**가 이 항목의 주인이다.
...
- **증상**: 165(날짜 칸) · 166(목록 위 설명)과 **같은 거짓말이 한 자리 더** 있다. 제안이 하나도 없을 때 화면 6 이 그리는 문장이
  ...
  가리키는 `propose` 는 **만들기만 하는 명령**이다(163 의 `draft` 행이 그 증거 — 만들어 놓고 안 올린 것이 이 목록에 뜬다).
  즉 「올라온 것이 없다」는 화면이 그리는 목록의 조건이 아니다 — **만든 것이 없다**가 맞다.
- **근거**: `apps/web/src/lib/web/screens.ts:260` (`EMPTY_PLACES['proposals.list'].message`). 166 을 닫으며 같은 낱말을
  훑다 나왔다(`grep -rn "올라온"`). ⚠ 캡처는 아직 없다 — 데모 씨앗이 제안 다섯 장을 세워서 **이 빈 상태가 화면에 안 나온다**.
  `test/web-proposals.test.ts` 「빈 목록은 넘겨받은 빈 상태를 그대로 그린다」가 이 문장을 마크업으로 읽는다.
- **정본**: `docs/DESIGN_BRIEF.md` §4 화면 6 · `EMPTY_PLACES` (문구의 정본 · FINDINGS 133)
- **고칠 방향**: 한 낱말이다 — 「아직 **만든** 제안이 없습니다」. ⚠ 166 이 만든 `MADE` 는 `components/proposals.tsx` 안에 있고
  `screens.ts` 는 그것을 못 읽는다(의존 방향이 반대다) — 낱말을 세 자리에서 같게 하려면 **`MADE` 를 어디로 올릴지**를 먼저 정해라
  (`lib/web/` 쪽이 자연스럽다). 그냥 문자열을 하나 더 적으면 넷째 자리가 생긴다.
- **상태**: ✅ `05fd6e7` (115바퀴) — **`MADE` 를 `lib/web/screens.ts` 로 내렸다**(`EMPTY_PLACES` 바로 위). 문자열을 한 벌
  더 적지 않은 이유가 그거다: `EMPTY_PLACES` 는 `lib/web` 에 있고 **`lib/` 는 `components/` 를 못 읽는다** — 낱말이
  조각 쪽에 있는 한 이 문장만 갈라질 수밖에 없었다. 이제 표 머리(`${MADE} 날`) · 목록 위 한 줄 · 빈 목록
  (`아직 ${MADE} 제안이 없습니다`) **셋이 낱말 하나**를 읽는다. 게이트 셋(`test/web-proposals.test.ts` 「빈 목록도 같은
  낱말로 말한다」): ①빈 문장이 `아직 {MADE} 제안이 없습니다` 다 ②머리와 목록 위 한 줄도 같은 `MADE` 를 쓴다
  ③`올라온|제출된` 이 없다. 게이트가 무는 것을 봤다(문구를 「아직 올라온 제안이」로 되돌려 FAIL → 되돌림).
  ⚠ **브라우저 캡처는 없다** — 데모 씨앗이 제안 다섯 장을 세워서 이 빈 상태가 화면에 안 나온다(FINDINGS 167 의
  「근거」가 미리 적어 둔 그대로다). 대신 조각을 그려서 마크업을 눈으로 읽었다:
...
### 166. **화면 6 의 머리 설명도 「올라온」이라고 말한다** — 초안 행은 안 올라왔다   [격차]
- **증상**: 목록 위 한 줄이 `Claude Code에서 contextops propose 로 올라온 변경 제안입니다.` 인데, 163 이 세운
  `draft` 행은 **`propose` 가 만들어 놓기만 하고 아직 안 올린 것**이다. 165 가 날짜 칸에서 고친 것과 **같은 거짓말**이
  머리 설명에 한 번 더 있다. 「초안」 칩을 눌러 그 한 장만 봐도 화면은 여전히 「올라온 제안입니다」라고 적는다.
- **근거**: `apps/web/src/app/t/[team]/p/[project]/proposals/page.tsx:51` 대 씨앗의 `decision: 'draft'` 한 줄
  (`lib/demo/seed-demo.ts` · FINDINGS 163). 캡처 `docs/evidence/2026-09-07-proposal-date-column/after-screen-proposals.png`
  (113바퀴) — 첫 행이 `· 초안` 인데 그 위 설명은 「올라온 변경 제안입니다」다.
- **정본**: `docs/DESIGN_BRIEF.md` §4 화면 6 · `docs/SPEC.md` §9 6번 행
- **고칠 방향**: 한 문장이다 — 「`contextops propose` 로 만든 변경 제안입니다. 올린 것만 승인 대기로 갑니다」처럼
  **만든 것과 올린 것을 갈라 말한다.** ⚠ 165 의 게이트(`PROPOSAL_DATE_COLUMN`)가 낱말을 잠근 곳은 **표 머리뿐**이라
  이 문장은 안 물린다 — 고칠 때 이 문장도 같은 시험이 읽게 해라.
- **상태**: ✅ `0fcf8b9` (114바퀴) — 문장을 `page.tsx` 밖으로 빼 **`ProposalListIntro`** 로 만들고(시험이 그리는 것은
  조각이지 페이지가 아니라서 165 의 게이트가 표 머리만 물었다) 「만든」을 **낱말 하나(`MADE`)** 로 묶었다 —
  표 머리 `${MADE} 날` 과 이 문장이 같은 자리에서 나온다. 문장: 「…로 만든 변경 제안입니다. **올린 것만** 승인 대기로 가고,
  승인된 제안만 다음 발행에 들어갑니다.」 게이트 넷(`test/web-proposals.test.ts`): ①`contextops propose` 가 남아 있다
  ②머리의 낱말이 문장에도 있다 ③`올라온|제출된` 이 없다 ④「올린 것만」이 있다(③만 있으면 「올린」을 통째로 지워도
  초록이라 **갈라짐 자체**가 안 잠긴다). 게이트가 무는 것을 봤다(문장을 「올라온」으로 되돌려 FAIL).
  눈으로 봤다: `docs/evidence/2026-09-07-proposal-list-intro/after-screen-proposals.png`
...
- **상태**: ✅ `49ce8e4` (113바퀴) — 권한 대로 ①이다. 머리와 값을 **`PROPOSAL_DATE_COLUMN { head, of }` 한 줄**로 묶고
  (`components/proposals.tsx`) 화면은 그 표를 읽기만 한다 — 한쪽만 고치면 시험이 먼저 빨개진다. 머리는 「만든 날」.
  🔴 **게이트로 올렸다** — `web-proposals.test.ts` 가 초안 행을 그려 ① 머리가 표의 낱말이고 값이 그 표가 가리키는 칸인지
  ② 머리가 `올라온|올린|제출` 을 말하지 않는지 ③ **본체** — `getTableColumns(proposals)` 에 `submittedAt` 이 **없는지**를 잰다.
  ③이 본체인 이유: 누가 제출 시각 칸을 만드는 날 이 줄이 빨개져서 머리 낱말을 다시 고를 자리로 데려온다. 그것 없이 낱말만
  잠그면 「칸이 생겼는데 화면은 영원히 만든 날」이 된다. 머리를 「올라온 날」로 되돌려 FAIL 을 확인한 뒤 되돌렸다.
  문서도 옮겼다 — `DESIGN_BRIEF` §4 화면 6 마지막 칸 「제출 시각」 → 「만든 날(`created_at`)」 + 왜 아닌지 두 줄.
  눈으로 봤다: `docs/evidence/2026-09-07-proposal-date-column/after-screen-proposals.png` (이번 관통이 찍은 진짜 브라우저 —
  머리가 「만든 날」이고 첫 행 `· 초안` 의 그 칸이 `2026-09-01` 이다. 이제 거짓말이 아니다).
...
- **상태**: ✅ `73532f8` (112바퀴) — 씨앗뿐이다. **제품 코드 0줄.** `DemoProposal.relatesTo` 를 더하고 그 타입이
  `MILESTONES` 의 id 칸을 **직접 읽는다**(`DemoMilestoneId`) — 없는 id 는 타입이 먼저 빨개진다. 다섯 줄 중 넷에 값:
  재시도 → `PL-M1` · 웹훅 서명 → `PL-M3` · 환불 SLA → `PL-M2`+`PL-M1`(칩 **둘**) · 초안(토큰 보관) → `PL-M3`.
  성공률 목표 한 줄은 **일부러 비웠다** — 「—」도 화면의 한 갈래다. 심는 반복문이 `relates_to: p.relatesTo ?? []` 로 넘긴다.
  🔴 **게이트로 올렸다** — `demo-guest.test.ts` 가 **`MILESTONES` 를 돌며** 3종이 전부 한 번은 걸리는지 · 쓰인 id 가
  전부 심은 것인지 · **빈 행과 칩 둘인 행이 둘 다** 서는지를 잰다. 값 하나를 빼고 돌려 FAIL(`제안 어디에도 PL-M2 …`)을
  확인한 뒤 되돌렸다. ★ 「빈 배열이 아닌 행이 있다」로 끝냈으면 넷째 마일스톤이 붙는 날 그것만 영원히 안 보인다.
  눈으로 봤다: `docs/evidence/2026-09-07-proposal-relates-to/after-screen-proposals.png` (이번 관통이 찍은 진짜 브라우저).
...
- **102바퀴가 확인했다 (09:43 CI · 아홉 바퀴 만에)**: **초록이다.** `test` 층 OK · `walkthrough` 도 **처음으로 지났다**(1105검사 ·
  여덟 바퀴 동안 `api` 단계에서 막혀 있던 것이 풀렸다). 닫은 커밋은 `ea0c7e9`. 102바퀴도 README·랜딩을 한 줄도 안 건드렸다.
...
- **상태**: 대기 (배포는 2026-09-10 에 됐고 주인이던 PLAN P5 Vercel 행은 `[x]` 다 — production 에서 `queued` 로 남는 행이 생기는지는 아직 아무도 안 쟀다 · 2026-09-15 대조: `AI_JOB_RETRY_RULES.queued` 는 여전히 `mode: 'none'`(`packages/schema/src/api.ts:235`))
...
- **고친 것** (`30f0fd8` · 102바퀴): **먼저 쟀다** (`docs/evidence/2026-09-07-test-timeout/`). 그 두 시험이 기다리는 것은 파일 I/O 가
  아니라 **자식 프로세스 기동**이다 — `git init` 과 `node scripts/stop.mjs` 를 진짜로 띄운다(번들이 아니라 Claude Code 가 `node <경로>` 로
  그대로 부르는 파일이라 프로세스로 재야 한다). 한가할 때 **107~190ms**(3회) · 같은 파일 **8개 동시**에서 최악 **293ms**(그 파일 안 어떤
  시험이든 **338ms**) · 파일 전체는 2.2s → 3.1s. 8배 동시에도 **1.8배**밖에 안 는다.
  상한은 `vitest.base.ts` 의 **`TEST_TIMEOUT_MS = 30_000`** 한 자리이고 `test.testTimeout` 이 읽는다. **왜 30초인가** — 실제 실패는
  vitest 가 5초에 죽여 **진짜 값을 아무도 모른다**(잰 최악의 26배 이상이란 것만 안다). 프로세스 기동은 부하에서 wasm 기동보다 더 나쁘게
  늘어난다(136 이 잰 훅 배율 6~9배로는 5초를 못 넘기는데 넘겼다) → 관측된 실패(5초)의 **6배**. ⚠ `HOOK_TIMEOUT_MS` 와 값이 같은 것은
  **우연이다** — 재는 것이 다르니(훅=PGlite 기동) 한쪽을 고칠 때 따라 고치지 마라고 주석에 적었다.
- 🔴 **게이트로 올렸다 — 켜자마자 흩어진 상한 7개가 나왔다.** `hook-timeout.test.ts` → **`vitest-timeouts.test.ts`** (이제 상한 **둘**을
  세니 이름도 세는 것과 같게 · 100바퀴의 `import.docs`→`import.jobs` 와 같은 이유). 더한 셋: ④ 설정이 그 상수를 실제로 읽나
  ⑤ 관측된 실패·vitest 기본(둘 다 5초)보다 큰가 ⑥ **시험에 자기 상한을 준 파일이 0개인가**. ⑥ 은 훅과 인자 자리가 달라
  (훅 `(fn, ms)` · 시험 `(이름, fn, ms)`) TS 로 파싱해 **호출 사슬의 뿌리 이름**을 본다(`it.only`·`it.each(…)(…)` 도 잡는다).
  나온 7 자리: `migrate-script`(30_000·60_000·30_000·60_000) · `db-pool`(15_000 둘) · `ai-structure`(30_000) — 한가할 때 전부
  **≤ 1.33초**라 정본 하나(30초 = 22배)로 충분하다. 전부 지웠다. **136 이 훅에서 걷어낸 것과 글자 그대로 같은 갈라짐이 시험 축에 남아 있었다.**
- 🔴 **뒤집어 확인했다** (④2-B ②): `TEST_TIMEOUT_MS` 를 5_000 으로 되돌리면 ⑤ 가 빨개진다 (`expected 5000 to be greater than 5000` ·
  1 failed | 5 passed). 즉 이 상수는 살아 있다.
- **상태**: ✅ `30f0fd8` (102바퀴 · CI GREEN — apps/web **799 통과** · plugin 178 · schema 141 · compiler 185 · walkthrough 1119검사)
...
- **상태**: ✅ 5d024ed (`.claude-plugin/marketplace.json` + 이름은 저장소 URL 에서 파생 · 게이트 5검사. 🙋 새 PC 밟기는 PLAN P5 둘째 행)
...
- 🔴 **막힘의 절반이 풀렸다 (102바퀴)**: **155 가 닫혔다** (`ea0c7e9` · CI GREEN 09:43 · 09:55). 남은 전제는 **하나**다 —
  관통의 `shots` 단계가 아직 **SKIP** 이라(`apps/web/e2e` 없음 · `tools/walkthrough.ps1:104`) **쓸 진짜 캡처가 없다.**
  ⚠ 그 전제를 만드는 것이 곧 이 항목의 일이다(위 「고칠 방향」) — 순서는 ① `apps/web/e2e` 로 `shots` 를 켜고
  ② 관통에 「`apps/web/public/` 아래로 복사」 단계를 두고 ③ 랜딩이 그 파일을 읽는다. ⛔ **손으로 만든 그림으로 닫지 마라.**
  ⚠ `landing.tsx` 는 다른 세션이 여전히 만지고 있다(`f87da1b` · 09:48 은 README·SPEC 이지만 같은 사람 지시의 연속이다) —
  ①②를 먼저 하고 ③ 에서 `git status --short` 로 확인해라.
- 🔴 **①② 를 했다 (103바퀴 · 관통이 이제 진짜 캡처를 낸다)**: `apps/web/e2e/` 를 만들어 `shots` 단계를 **켰고**
  (`shots` **37검사**), 그 옆에 `shotcopy` 단계를 두어 `apps/web/public/shots/` 로 옮긴다(**10검사**). 하네스는
  playwright 가 아니라 **CDP 로 몬 헤드리스 Chrome** 이다(의존성 0 · `demo:db` + `next dev` 를 스스로 띄우고
  `/demo` 로 게스트 세션을 받는다 — 들어가는 길이 제품과 같다). 관통 표의 `count_log` 두 개는
  `apps/web/test/e2e-plan.test.ts` 가 **실제 요약 줄에 걸어 본다**(FINDINGS 95·96 이 겪은 갈라짐).
  ⚠ 첫 판은 검사가 다 초록인데 **캡처 일곱 장이 전부 skeleton** 이었다 — `.main-inner` 는 즉시 있기 때문이다.
  그래서 「로딩이 끝났나(skeleton 0)」를 검사로 넣었다. 남은 것은 **③ 랜딩이 `public/shots/manifest.json` 을
  읽는 것**뿐이다.
- 🔴 **③ 을 했다 — 닫는다 (104바퀴)**: 히어로 **바로 아래**에 B-2 「제품 화면」 절이 서고, 랜딩이
  `public/shots/manifest.json` 을 읽어 그린다. ⛔ **화면 코드에 파일 이름이 한 자도 없다** —
  목록의 정본은 `e2e/plan.ts` 의 `PUBLISHED` 하나이고, 격자도 칸 수가 아니라
  `auto-fit`/`--shot-min` 이라 계획에 한 줄을 더해도 CSS 를 안 고친다.
  **모양의 정본도 하나로 올렸다**: `packages/schema` 의 `ShotsManifest` 를 **쓰는 쪽**
  (`e2e/publish-shots.ts` · 쓰기 전에 판정)과 **읽는 쪽**(`landing.tsx` · 모듈 로드에 되판정)이
  같이 본다 — 어긋난 파일은 렌더가 아니라 **빌드**에서 죽는다 (`REPLAY_FRAMES` 와 같은 자리).
  `min(1)` 인 이유는 빈 목록을 통과시키면 **그림 0장인 랜딩이 조용히 배포되기** 때문이다.
  폭·높이는 manifest 가 주고(레이아웃이 안 튄다), 캡처마다 **그 화면의 주소**를 근거로 같이 적는다.
  🔴 **게이트** `test/web-landing-shots.test.ts` **24개**: ① `landing.tsx` 에 `.png`·`screen-` **0건** ·
  읽는 파일이 manifest 하나 ② **랜딩이 읽는 것 == 계획이 옮기는 것**(파일·alt·주소·크기 · 양방향 —
  계획에서 뺀 그림이 랜딩에 남지 않는다) ③ 그림이 실재하고 5KB 초과 ④ 근거 주소가 `PROJECT_SCREENS`
  의 실재 화면 ⑤ 그리면 `<img>` 가 장수만큼 · 순서가 히어로 뒤 「왜 git」 앞 · `btn-primary` 여전히 **1개**
  ⑥ 계약이 빈 목록·모르는 필드·빈 alt·`/shots/` 밖 경로를 거부한다.
  🔴 **눈으로 봤다** — `docs/evidence/2026-09-07-landing-shots/` (고친 뒤의 관통이 찍은 랜딩).
  1440×900 의 **접히기 전**에 캡처 세 장이 보인다 — 그게 이 항목의 증상이 사라졌다는 근거다.
  375px 에서는 한 열로 접히고 가로 스크롤 0. ⚠ 접히는 선 **아래**(대체텍스트·주소·각주)는 캡처에
  없다 — 그 셋은 ⑤ 가 마크업으로 잰다 (evidence README 의 「못 본 것」).
  `DESIGN_BRIEF` 화면 1 에 **B-2** 를 정본으로 적었다 (C-2 스텝 썸네일은 이 절이 대신한다 —
  같은 캡처를 두 곳에 흩지 않는다).
- **상태**: ✅ `8e5d269` (104바퀴 · ①② `f29133f` · ③ `8e5d269` · 주인은 PLAN **P4 둘째 행**)
...
- **상태**: 대기 (절삭 1번 — **잘랐다.** 주인이던 PLAN **P4 둘째 행은 105바퀴에 `bd6ddf3` 로 닫혔고**,
  그 행은 §7.3 질의창 **없이** GATE 3 을 통과했다. 다시 열려면 새 주인 행이 있어야 한다)
...
- **상태**: ✅ `8c3e8c5` (2026-09-06 · 57바퀴) — `…/sync` 가 생겼고 좌측 내비 표에 한 줄이다.
  ...
  ⚠ **104바퀴가 상태 줄이 둘이던 것을 하나로 고쳤다** — ✅ 아래에 옛 `대기` 줄이 남아 있어서
  이 항목이 목록에서는 **열려 있는 것처럼** 보였다 (`status-shape` 는 첫 줄을 읽어 ✅ 라 하고,
  사람이 훑으면 마지막 줄인 「대기」를 본다). **FINDINGS 102 와 같은 뿌리다 — 말하는 자리가 둘.**
  ★ 고칠 때의 규칙: **`- **상태**:` 는 항목의 마지막 줄 하나다.** 닫을 때 옛 상태 줄을 지워라.
...
- **상태**: ✅ 기록 닫음 (2026-09-15 대조) — 35 `130b5f2`(54바퀴) · 25 `a201a51`(71 과 같은 것 · 25 의 상태 줄). 아래는 그 전의 기록이다.
...
- **상태**: ✅ `7b7f521` (2026-09-06 · 아래 「닫음」 — 이 항목엔 상태 줄이 없어서 게이트가 뒤 항목의 줄을 읽고 있었다 · 2026-09-15 붙임)
...
- **상태**: ✅ 5d024ed (근거 라벨이 문서 버전 id 앞 8자를 낸다 · 게이트 「문서가 다르면 라벨도 다르다」)
...
- **상태**: ✅ 9dd032b (`judge()` 가 「캐시에 자취가 없는데 파일은 다 맞다」를 `manual` 로 · 시험 픽스처가 실제와 달랐던 것도 같이 고쳤다)
...
- **상태**: ✅ f781ae9 (①로 닫았다 — `job` → **`job_id`**. 셋째 모양이 사라졌다 · `POST /documents` 도 같이)
...
- **상태**: ✅ f781ae9 (문구를 「요청이 계약과 맞지 않는다」로 — body·query·path 셋 다에 참이다)
...
- **상태**: ✅ f781ae9 (①로 닫았다 — 「채울 수 있는 종류 0개」가 일부러임을 시험이 잠근다 · `needsB` 축은 items 쪽에서 살아 있다)
...
- **상태**: ✅ 39c9b04 (공통은 넷 다에 참인 것만 · 칸에 매인 둘은 `AI_SYSTEM_EVIDENCE_SLOTS` 로 · 게이트가 §7.2 프롬프트를 문다)
...
- **상태**: 대기 (전제였던 키는 2026-09-06 에 왔다(Gemini) · 진짜 호출로 재는 문은 `pnpm --filter web p3:measure` — 2026-09-15 대조: `packages/schema/src/json-schema.ts:90` 은 여전히 `reused: 'ref'` 뿐이고 `.meta({ id })` 는 0곳 · `plugin/contextops/schemas/*.json` 에 `__schemaN` 이름이 남았다 · 새 주인 없음)
...
- **상태**: ✅ f781ae9 (주석을 뺀 본문으로 센다 `Get-CodeOnly` · 세는 대상에 `callModel(` · 2 → **3개 호출부** · 무는 것을 봤다)
...
- **상태**: ✅ f781ae9 (**고장이었다** — 라우트가 계약으로 파싱해서 낸다 · 관통의 가짜 서버와 시험 픽스처도 같은 계약을 거친다)
...
- **상태**: ✅ 5d024ed (`# 작업 절차와 진행 보고` — 항목 유무로 제목을 가르지 않았다 · `TEMPLATE_VERSION` 1.5 · golden 3케이스)
...
- **상태**: ✅ 39c9b04 (`toJsonSchemaOf` 의 `override` 로 `anyOf` · 예외 목록이 표와 같은지를 시험이 잠근다)
...
- **고친 것** (`478ba42` + `f1a27c7` · 106바퀴): 〈고칠 방향〉 그대로다. 화면 9 머리에 **[기기 추가]** —
  이름을 적고 누르면 발급되고, 그 자리에서 **한 번만 보이는 값**과 그 아래
  `contextops setup --api-origin … --project … --token … --device-id …` **한 줄**을 통째로 복사한다.
  🔴 **줄의 정본은 `packages/schema` 의 `setupCommandLine` 하나다** — 만드는 쪽(`apps/web`)과 받는 쪽
  (plugin CLI)이 다른 패키지라, 각자 적으면 플래그 이름이 조용히 갈리고 그때 증상은 **화면이 준 줄을
  CLI 가 모른다**이다(사람은 그걸 자기 오타로 안다). 한 줄로 주는 이유는 `device_id` 다 — 발급 응답에만
  있는 값이라 사람이 토큰만 옮기면 늘 빠지고, 그러면 「이 기기만 끊기」가 영원히 안 된다.
  🔴 **게이트 둘 (④2-B 두 단계)**: `plugin/…/test/setup-command.test.ts` **5개** — ① 그 줄의 플래그가 전부
  `SETUP_FLAGS` 의 키이고 `kind: 'value'` 다 ② **그 줄을 그대로 인자로 잘라** setup 을 돌리면 두 파일이
  생기고 `device_id` 까지 따라온다. `apps/web/test/web-add-device.test.ts` **12개** — 네 모양을 전부
  그려서 읽고, **화면 소스에 setup 플래그 글자 0건**(정본이 스키마 하나임을 뒤집어 센다) ·
  `btn-primary` 최대 1 · 게스트는 `writeDoor()` 표대로 그 자리에서 이유를 듣는다.
  **눈으로 봤다**: `docs/evidence/2026-09-07-add-device/` — 캡처에서 발급 카드에 [닫기] 가 **둘**이었다
  (머리 토글 + 카드 자신). 발급된 뒤에는 토글을 안 그린다(`f1a27c7`).
  곁가지: `EMPTY_PLACES['sync.devices']` 의 「웹에 그 문이 없다」와 `setup` 의 ② 안내를 같이 고쳤다.
  ⚠ 콜백 서버는 **여전히 안 만들었다** — 한 줄 붙여넣기로 끝나는 일에 CLI 가 포트를 다룰 이유가 없다.
- **상태**: ✅ `478ba42` + `f1a27c7` (106바퀴 · 주인이던 P4 화면 9 는 이미 닫혀 있었다 · CI GREEN · 관통 1247검사)
...
- **상태**: 대기 (주인이던 P3·P4 행은 전부 `[x]` 인데 이 문은 안 생겼다 — 2026-09-15 대조: 서버에 추천 계산 0곳 · `lib/web/semver.ts` 는 여전히 사람이 고른다 · 새 주인 없음)
...
- **상태**: 대기 (2026-09-15 대조로 붙인 상태 줄 — 여전히 프로젝트 전체 수다: `roadmap/route.ts:90-93` 이 열린 충돌을 한 번 세고 `:141` 이 모든 마일스톤 행에 같은 수를 싣는다 · 붙은 것과 안 붙은 것을 가를 칸이 없다 · 같은 수를 종류로 가르는 것은 171 · ⚠ 이 항목엔 상태 줄이 없어서 게이트가 다음 항목(30)의 ✅ 를 읽고 있었다)
...
- **상태**: ✅ f736884 (②로 닫았다 — §2 에 「내용 중복 발행은 막지 않는다」와 그 이유를 적었다. 해시 규칙을 두 곳으로 만들지 않았다)
...
- **상태**: ✅ f736884 (§6.1 을 적었다 — **서버는 추천하지 않는다**. 없는 §6.5 참조를 없앴고 게이트가 절 참조를 전부 확인한다)
...
- **상태**: ✅ f736884 (§5 에 `INTERNAL` + **`AI_OUTPUT_INVALID`** — 게이트가 그 열한 번째를 찾아냈다)
...
- **상태**: ✅ f736884 (§5 의 칸을 `changes` 로 · 게이트에 예외를 파지 않았다)
...
- **상태**: ✅ f736884 (§2 에 `public_id`·revisions 네 칸·`expires_at`·`last_scan`)
...
- **상태**: 대기 (주인이던 P3 둘째 행은 `130b5f2` 로 닫혔는데 읽는 곳은 안 생겼다 — 2026-09-15 대조: `lastScan` 은 `schema.ts:243` 선언과 `batch-draft/route.ts:78` 쓰기뿐 · 새 주인 없음)

### 25. ✅ 충돌 해소가 **항목 상태를 갱신하지 않는다** — 71 과 같은 것이었다 (`a201a51`)   [구멍]
...
- **상태**: ✅ `a201a51` (71 과 **같은 것을 다른 번호로** 들고 있었다 — 107 이 지목한 그대로다).
  108바퀴가 **닫기 전에 쟀다** (재지 않고 닫는 것이 더 나쁘다 · 56 의 교훈):
  `test/api-routes.test.ts` 를 이름으로 걸어 12개를 실제로 돌렸다 —
  ① 「선택이 **항목**을 바꾼다」가 네 선택을 **라우트로** 눌러 진 쪽 항목을 다시 읽는다
  (네 줄이 **세 가지** 결과 · `both`·`dismiss` 는 항목 0건) ② 「이긴 쪽은 안 건드린다 — 개정도 안 쌓인다」
  ③ 「폐기된 항목에 개정이 쌓이고 그게 어느 충돌에서 왔는지 남는다 (P7)」
  ④ 「가리키는 것이 항목이 아닌 종류는 결정해도 바꿀 항목이 없다 (anchor)」.
  표는 `RESOLUTION_ITEM_OUTCOME` + `itemOutcomeOf()` 하나(`packages/schema`)이고 라우트·화면 4·카드가
  **읽기만** 한다. SPEC 쪽 갈림(§2 `a_ref` vs §7.2 `a_item_id`)도 이미 없다 — §2 표가 `a_item_id`·`b_item_id`
  두 칸을 적고 어느 칸이 채워지는가는 `CONFLICT_KIND_RULES` 가 정한다 (DB CHECK 5개).
...
- **상태**: ✅ f736884 (§2 를 `SYNC_STATUSES` 와 같게 · `failed` 를 되살리지 않았다 · 게이트가 대조한다)
...
- **상태**: ✅ `e5f61c8` (절반) · ✅ `7398e44` (나머지 절반 — 아래 → 줄 · 2026-09-15 대조로 닫음) — **`status` 4종은 살렸다.** `apps/web/src/lib/api/progress.ts`
  ...
  → ✅ **`source` 3종도 화면이 읽는다** (`7398e44` · 55바퀴 · 화면 8): 라우트가 `source` 를 내고(`roadmap/route.ts:79·107`)
  화면이 `PROGRESS_SOURCE_LABEL`(「Claude 가 보고」 · 「세션이 끝날 때 플러그인이 보고」 · 「사람이 직접 적음」)로 「누가 보고했나」를
  글자로 가른다(`components/roadmap.tsx:252·335` — 아이콘이 아니라 낱말). 표는 `web-tables.test.ts` 가 enum 과 대조하고
  화면 글자는 `web-roadmap.test.ts` 가 잰다.
...
- **상태**: ✅ f736884 (§4.2 세 줄 — `snapshot:` · `data.statement` · `conf:` · 왜 그런지도 같이)
...
- **상태**: ✅ f736884 (§1.1 트리를 실제 이름으로 · **게이트가 트리의 경로를 전부 확인한다**)
...
- **상태**: ✅ f736884 (§3 의 Scope·ProposalItem·ProgressEvent 를 코드의 `.refine` 모양 그대로)
...
- **고칠 방향**: 20 으로 내릴 이유가 없다 — SPEC §1.2 의 「Node 20 LTS」를
  ...
  `.nvmrc` 하나여야 한다)
- **상태**: ✅ f736884 (SPEC §1.2 를 `.nvmrc` 로 · 게이트가 Node 를 대조한다)
...
- **상태**: ✅ f736884 (§1.2 에서 버전 숫자를 뺐다 — 정본은 catalog·packageManager · 게이트가 숫자를 금지한다)
...
- **상태**: ✅ f736884 (`git checkout` 으로 재체크아웃 · 대장이 적은 둘이 아니라 **셋**이었다 · 게이트가 여덟을 바이트로 잰다)

---

## 고친 것

### 161. ✅ **관통이 찍는 캡처가 매 바퀴 바이트가 달라진다** — 원인은 **데모가 매번 다시 심겨서**였다   [격차]
- **증상**: `tools/ci.ps1` 을 두 번 돌리면 `apps/web/public/shots/*.png` 가 **다시 M** 이 된다.
  106바퀴에 세 번 돌렸는데 세 번 다 어떤 장이 바뀌었고(1·2회차는 셋 다 · 3회차는 `screen-context`·`screen-packs` 둘),
  그 바퀴는 화면 9 말고 **랜딩도 화면 5·7 도 한 줄 안 건드렸다**. 즉 바뀐 것은 코드가 아니다.
- **근거**: 106바퀴 `git status --short` 세 번(11:40 · 11:49 · 11:57) · 104바퀴부터 매 바퀴 커밋에 이 파일들이 섞여 있다
- **정본**: `apps/web/e2e/shots.ts` (찍는 곳) · `apps/web/e2e/publish-shots.ts` (옮기는 곳) · `apps/web/public/shots/manifest.json`
- **왜 고장이 아닌가**: 랜딩은 정상으로 그려지고 게이트(`web-landing-shots.test.ts` 24개)도 초록이다.
- 🔴 **쟀다 (108바퀴 · `docs/evidence/2026-09-07-shot-churn/`)** — 위에 적혀 있던 짐작(「인코딩된 바이트가
  달라진다 · 폰트 래스터·타이밍으로 보인다」)이 **틀렸다.** 107·106 두 관통의 같은 캡처를 **픽셀로** 견줬다:

  | 캡처 | 같은 바이트인가 | 달라진 픽셀 | 줄 |
  |---|---|---|---|
  | `screen-sync` | **그렇다 (byte-identical)** | **0** / 1,296,000 | — |
  | `screen-context` | 아니다 | **314** (0.024%) | 156–164 |
  | `screen-packs` | 아니다 | **1,306** (0.10%) | 243–305 |

  `screen-sync` 가 두 관통에서 **바이트까지 똑같다** — 캡처(폰트 래스터·PNG 인코더)는 **결정론적**이다.
  달라진 자리를 잘라서 눈으로 읽었더니 **`manifest_hash` 와 발행 시각 글자**였다
  (`d3585389` → `0d9a5f49` · `02:39` → `03:15`). 즉 **그림은 낡지 않았고 데모가 다시 심긴 것**이다.
  Pack 의 모든 줄이 문서 **버전 uuid** 를 태그에 싣는데(`src:doc:<uuid>#260-311`) 그 uuid 가 심을 때마다
  새로 나서 본문 바이트 → `manifest_hash` 가 달라진다. ⚠ **P4 위반이 아니다** — snapshot 자체가 다르다.
- **왜 값이 나나**: ① 매 바퀴 커밋에 **코드와 무관한 diff** 가 섞여서 「이 바퀴가 무엇을 바꿨나」가 흐려진다
  ② 남의 세션이 같이 일할 때 이 파일들이 늘 충돌 후보다 ③ `.png` 라 리뷰가 무엇이 달라졌는지 못 본다
- **고칠 방향**: 잰 뒤라 갈래가 줄었다.
  ~~② 찍는 조건을 조인다(폰트 로딩)~~ — 인코더·래스터는 이미 결정론적이라 **고칠 것이 없다.**
  ① **바뀐 장만 옮긴다** — ✅ 아래 「상태」. ③ 그림을 저장소에서 빼고 배포 때 만든다는 여전히 제일 비싸다
  (랜딩이 **읽는** 파일이라 빌드가 관통에 묶인다).
  🔴 **남은 뿌리는 하나다 — 데모 씨앗이 재현 가능하지 않다.** 고치려면 둘이 같이 와야 한다:
  ⓐ **고정 시계** (`seedDemo(now)` 는 이미 인자를 받는다 — 관통이 상수를 준다)
  ⓑ **결정론적 문서 버전 id** (지금은 DB 기본값 uuid 라 심을 때마다 새로 난다)
  ⚠ ⓑ 를 하려고 `POST /documents` 에 「id 를 받는 문」을 열지 마라 — 데모 사정으로 제품 계약을 넓히는 것이다.
  씨앗이 라우트로 만든다는 원칙(`lib/demo/seed.ts`)과 어떻게 같이 설지를 **먼저 정해라.**
  ★ 값: 「같은 씨앗 → 같은 `manifest_hash`」가 서면 P4 주장을 **제품 수준에서** 한 번 더 보인다.
- **상태**: ✅ `87eadea` (109바퀴 · 절반은 108바퀴가 `f095e66` 로) — **재는 자와 게이트는 섰다.** `apps/web/e2e/png.ts` 가 PNG 를 픽셀로 풀고,
  `shotcopy` 가 옮기기 **전에** 견줘 한 픽셀도 안 달라졌으면 **안 옮긴다.** 달라졌으면 몇 픽셀이 어느 줄에서
  달라졌는지를 관통 로그에 매 바퀴 찍는다 — 「매 바퀴 그림이 바뀐다」가 더는 짐작이 아니다.
  잠근 것: `test/e2e-png.test.ts` 5 — **같은 픽셀을 일부러 다른 바이트로 인코딩해** 놓고 0 을 요구한다
  (바이트 비교로 조용히 되돌아가면 빨개진다) · 필터 다섯 갈래를 다 풀어 본다 · 크기가 다르거나 안 읽는
  모양이면 **던진다**(조용히 「안 달라졌다」로 답하면 이 검사가 아무 말도 안 하는 검사가 된다).
  **남은 절반은 위 ⓐ+ⓑ 다** — 그때까지 두 장은 계속 달라지고, 게이트는 그걸 **옳게** 옮긴다.

  🔴 **109바퀴가 남은 절반(ⓐ+ⓑ)을 닫았다 — 제품 코드는 0줄이다.** 결정론은 **하네스가 소유한 DB**
  에 뒀다 (`apps/web/test/helpers/pin.ts`). 라우트에 「id·시각을 받는 문」은 안 열었다 — 위 ⚠ 그대로다.

  | 무엇 | 어떻게 |
  |---|---|
  | ⓑ 문서 버전 uuid | `gen_random_uuid()` 기본값을 **계수 uuid**(`pin_uuid()`)로 바꾼다 — `public` 의 15칸 |
  | ⓐ `created_at`·`updated_at` | `now()` 기본값을 **고정 기준 시각 + 1ms 계수**(`pin_now()`)로. ⚠ 한 값으로 **얼리지 않는다** — 얼리면 `order by created_at desc` 가 순서를 잃어 churn 이 다른 자리로 옮겨갈 뿐이다 |
  | ⓐ 발행 시각 | DB 기본값이 **아니다** — 라우트가 준 값이다 (`lib/api/route.ts` 의 `const now = new Date()` → `publishVersion({ now })`). 그래서 심은 **뒤에** `pinSeededClocks()` 가 `published_at = created_at` 으로 되돌린다 |
  | 어디서 켜나 | `DEV_PIN=1` — **캡처 하네스(`e2e/shots.ts`)만.** 사람이 `demo:db` 로 보는 화면은 발행 시각이 「지금」이어야 자연스럽다 |

  ⚠ **경과(`sinceText`)로 그리는 칸은 안 건드렸다.** 씨앗이 진짜 시각 기준으로 뒤로 미는 값이라
  (`hoursAgo`) 과거로 옮기면 「3시간 전」이 「N일 전」이 되어 **오히려 매일 달라진다.**
  그래서 `seedDemo(now)` 는 진짜 시각 그대로다 — `screen-sync` 는 지금도 byte-identical 이다.

  🔴 **잠근 것**: `test/demo-seed-pinned.test.ts` 2 — 「박으면 두 번 심어도 `manifest_hash`·발행 시각·
  문서 버전 id 가 같다」와 **「안 박으면 달라진다」**. 뒤엣것이 없으면 `pinDbDefaults` 가 아무 일도
  안 하게 되는 날(칸 이름·기본값 모양이 바뀌는 날)에도 이 시험은 초록이다.

  🔴 **진짜 관통에서 확인했다** — 고친 뒤 독립 실행 한 번 더: `shotcopy` 세 줄이 전부
  `그림이 그대로다 — 안 옮겼다 — 달라진 픽셀 0개` (바이트도 99916 · 40189 · 81193 로 같다) ·
  `git status --short` **비었다.** 눈으로 봤다: `docs/evidence/2026-09-07-pinned-seed/`.
