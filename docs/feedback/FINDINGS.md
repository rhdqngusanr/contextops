...
- **증상**: 화면 4 의 질문 카드에 문장을 적고 [답변 저장] 을 누르면 질문은 닫히고
  「저장됐다」가 뜨는데, `open_question` 이면 **항목이 하나도 안 생긴다.** 사람이 적은
  문장은 `conflicts.resolution.note` 에만 남고 Pack 에는 한 줄도 안 나간다.
  씨앗 질문 10장은 표가 자리를 정해 줘서 항목이 되지만(FINDINGS 67), **§7.1 이 문서를
  읽다 남긴 질문**은 그 표에 없다.
- **근거**: 이번 바퀴 직접 읽음 — 답이 항목이 되려면 `AnswerQuestions.draft` 가 와야 하는데
  (`packages/schema/src/api.ts:545`), 그것을 **보내는 제품 코드가 한 곳도 없다**:
  화면 4 `app/t/[team]/p/[project]/review/page.tsx:132` 와 화면 3 `…/import/page.tsx:210`
  이 둘 다 `{question_id, answer}` 만 싣고, 클라이언트 문(`lib/web/queries.ts:387`)의
  타입에는 `draft` 자리가 아예 없다. 보내는 자리는 **시험 하나뿐**이다
  (`test/api-routes.test.ts` — 「초안을 같이 주면 그때만 항목이 생긴다」).
  즉 계약의 그 칸은 **시험만 부르는 칸**이다 (2-B 의 그 모양).
- **정본**: `docs/SPEC.md` §5(`POST /projects/{id}/questions`) · §7.1 · §9 화면 4
- **왜 지금 드러났나**: 56 을 고치면서 그 길의 근거를 서버가 채우게 했는데, **그 길로
  들어오는 제품 트래픽이 0** 이라 이번 바퀴에 고친 것은 아직 시험에서만 도는 셈이다.
- **고칠 방향**: 셋 중 하나다 — 어중간하게 두지 마라.
  ① **화면이 타입을 고르게 한다** — 답 칸 옆에 「무엇으로 저장할까요」(policy·constraint·…)를
     두고 화면이 `draft` 를 만들어 보낸다. ⚠ 그러면 화면이 타입별 `data` 를 조립하게 되고,
     지금 `seed-questions.ts` 가 지고 있는 「어느 칸에 넣나」 표가 **두 곳**이 된다.
  ② **서버가 씨앗과 같은 방식으로 옮긴다** — 열린 질문에도 목적지 칸을 정해 준다.
     ⚠ 그 자리는 §7.1 이 질문을 만들 때만 알 수 있다 (`open_questions` 에 타입이 없다) —
     내려면 §7.1 출력 계약에 칸을 하나 더해야 한다.
  ③ **못 한다고 화면이 말한다** — 「이 답은 기록만 됩니다」를 카드에 적는다. 제일 싸고
     제일 정직하지만, 그러면 화면 4 의 질문 카드 절반이 아무것도 안 만든다.
  ★ 고르기 전에 **어느 것이 SPEC §5 의 「초안이 안 오면 답만 기록하고 질문을 닫는다」와
    맞는지** 보고, 안 맞으면 SPEC 을 같이 고쳐라.
- **상태**: 대기 (화면 3·4 가 주인 · PLAN P3 둘째 행)
