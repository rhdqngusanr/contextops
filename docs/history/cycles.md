...
### 111바퀴 — 화면 6 이 제안 상태 5종을 전부 보여 준다 (FINDINGS 163 ✅ · `98f00cb`)

**그 바퀴(111)는 대장이 빈 채로 시작해서 ④1 로 돌아갔고, 관통 캡처를 읽다가 `FINDINGS 163` 을 새로 찾아 같은 바퀴에 닫았다 — 화면 6(제안)이 제안 상태 **5종을 전부** 보여 준다.** INBOX 「할 것」비어 있음.
🟢 **CI GREEN 전 층 · 관통 1255검사 · 9단계 SKIP 0.**

🔴 **110이 남긴 방법을 그대로 썼다.** 110은 「표(enum)를 하나 골라 **데모가 그 표의 값을 전부 한 번씩 보여 주나**를 세라」고 적어 두고 안 센 표 다섯을 나열했다. 그중 `PROPOSAL_STATUSES` 를 골라 관통을 돌리고 `.ci/shots/screen-proposals.png` 를 눈으로 읽었다. 거르개 칩은 **여섯**(전체 + 5종)인데 표는 **4행**이었다. `초안`(draft) 칩은 **늘 빈 목록**이다.

🔴 **죽은 것은 코드가 아니라 씨앗이다 — 162 와 같은 모양이다.** 배선은 다 살아 있다: 만드는 라우트가 `status: 'draft'` 로 넣고(`proposals/route.ts:60`), 목록 문이 `?status=draft` 를 걸며(`:85`) 작성자 제한이 없어 팀원 누구나 본다. `DEMO_PROPOSALS` 를 심는 반복문이 **넷 다 무조건 `submit`** 했을 뿐이다.
⚠ `draft` 는 「`contextops propose` 가 만들어 놓기만 하고 아직 안 올린 것」이라, **CLI 로 만든 게 어디에 쌓이나**를 화면에서 말해 주는 유일한 상태다.

🔴 **고친 자리 — 씨앗 한 줄. 제품 코드는 0줄이다.** `DEMO_PROPOSALS` 4 → 5:

| 더한 제안 | 무엇 |
|---|---|
| 「카드 정보 제약에 `토큰 보관 기간`을 적는다」 | `decision: 'draft'` · 대상 `item_constraint_card` · 작성자 박서연. 심는 반복문이 그 값에서 `continue` 한다 — 제출도 결정도 안 한다 |

기존 4줄은 **안 건드렸다** — 랜딩의 After 문장이 그중 `published` 행을 글자 그대로 읽는다(`web-landing.test.ts`).

🔴 **게이트로 올렸다 — 핵심은 162 와 같이 「종류를 표에서 센다」이다.** `demo-guest.test.ts` 의 화면 6 시험이 셋을 잰다: ① 다섯 상태의 목록을 그대로 ② **`PROPOSAL_STATUSES` 를 돌며 종류마다 0보다 큰가** ③ `draft` 행은 `decided_by` 가 `null` 인가. 그리고 시험 하나를 더했다 — **`?status=<종류>` 를 5종 전부 눌러** 빈 목록이 없고 다른 종류가 안 섞이는지(거르는 것은 서버다).
★ **②가 본체다** — ①만 있으면 `PROPOSAL_STATUSES` 에 여섯째가 붙는 날에도 초록이라 **같은 구멍이 다시 난다.**

🔴 **잰 것** (CI 13:49 · 관통 148초):

| | 전(110바퀴) | 후 |
|---|---|---|
| 화면 6 의 행 | **4** | **5** |
| 데모에 서는 `PROPOSAL_STATUSES` | **4 / 5** (`draft` 없음) | **5 / 5** |
| `apps/web` 시험 | 862 | **863** (+1 · `?status` 5종 순회) |
| 관통 검사 | 1254 | **1255** |
| CI | GREEN | **GREEN 전 층** |

🔴 **눈으로 봤다** — `docs/evidence/2026-09-07-proposal-draft/after-screen-proposals.png` (이번 관통이 찍은 진짜 브라우저 · `.ci/shots/` 밖으로 복사했다). 표가 5행이고 첫 행이 `· 초안` 「카드 정보 제약에 「토큰 보관 기간」을 적는다」 · 박서연 · 항목 1 이다. 상태는 **아이콘 + 라벨**이라 색만으로 구분되지 않는다.

🔴 **문서도 같이 옮겼다** — SPEC §10.3 「제안 4(…)」→「**제안 5(published 1/approved 1/rejected 1/submitted 1/draft 1)**」 · `DESIGN_BRIEF` §4 화면 6 의 상태 chip 줄에 `draft` 를 더했다.

🔴 **같은 캡처에서 둘을 더 찾았다 — 이번엔 안 고쳤다 (한 바퀴에 하나).**
- **164 [구멍]** — 제안의 `relates_to`(관련 마일스톤)가 **다섯 행 전부 `—`** 다. 계약·표·라우트·화면이 다 살아 있는데(`upload.ts:111` · `db/schema.ts:435` · `proposals/route.ts:65` · `proposals.tsx:132`) `DEMO_PROPOSALS` 가 그 키를 한 줄도 안 쓴다. **화면 6 과 화면 8 을 잇는 유일한 줄**이 데모에서 한 번도 안 보인다.
- **165 [격차]** — 날짜 칸의 머리는 「올라온 날」(제출한 날)인데 값은 `created_at` 이다(`proposals.tsx:116` 대 `:142`). 넷이 다 제출된 것이던 동안엔 사실상 같았는데, 163 이 `draft` 행을 세우면서 **아직 안 올라온 제안에 「올라온 날」** 이 붙었다.

🔴 **2-B 이번 라운드 — 표 셋을 세었다.** ① `PROGRESS_STATUSES` 4종: `fixtures/seed/demo.json` 의 `progress` 가 `criterion_done 3 · in_progress 3 · done_candidate 1 · none 1` — **4/4 살아 있다.** ② `SYNC_STATUSES` 5종: 110이 닫았다(5/5). ③ `PROPOSAL_STATUSES` 5종: **4/5 였고 이번 바퀴가 닫았다.** 아직 안 센 표: `ItemType` 10종 · 에러 코드 9종 · `AI_JOB_STATUSES` 4종(데모 씨앗에 job 이 **0개**다 — 셀 때 여기부터 보면 된다).

- PLAN 의 `- [ ]` 중 남은 것 **셋**: P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(**164** · 165 · 156 · 152 · 140 · 117 · 24 · …) — **고장 0** · 이번 바퀴가 새로 찾은 것은 **구멍 둘(163·164) + 격차 하나(165)** 이고 그중 163 을 그 자리에서 닫았다. 관통은 GREEN(9단계 · 1255검사)이다.

### 110바퀴 — 데모의 화면 9 가 sync 상태 5종을 전부 보여 준다 (FINDINGS 162 ✅ · `14a0719`)
**그 바퀴(110)는 대장이 빈 채로 시작해서 ④1 로 돌아갔고, 관통·캡처를 읽다가 `FINDINGS 162` 를 새로 찾아 같은 바퀴에 닫았다 — 데모의 Sync 화면이 sync 상태 **5종을 전부** 보여 준다.** INBOX 「할 것」비어 있음.
🟢 **CI GREEN 전 층 · 관통 1254검사 · 9단계 SKIP 0.**

🔴 **어떻게 찾았나 — 목록이 아니라 캡처에서 나왔다.** 109가 「다음은 없음 · ④1 로 돌아가라」고 적어 뒀고, 그대로 관통을 돌린 뒤 `.ci/shots/screen-sync.png` 를 눈으로 읽었다. 칩 줄이 `applied 9 · outdated 2 · manual 1` **셋뿐**이었다. `SYNC_STATUSES` 는 **5종**이다.

🔴 **죽은 것은 코드가 아니라 픽스처였다.** 배선은 전부 살아 있다 — `statusOfDevice()` 가 보고 없는 기기에 `NO_REPORT_STATUS`(`unknown`)를 매기고, `DeviceTable` 이 그 행의 버전 칸에 「—」·시각 칸에 「아직 보고가 없습니다」를 그리고, `SYNC_CHIP`·`SYNC_MEANING`·`SYNC_APPLY` 가 5줄이다. **아무도 그 길로 안 들어갔을 뿐이다.** 2-B 가 말하는 「정의만 있고 아무 일도 안 하는 것」의 픽스처 판이다.
⚠ 그중 `modified`(공식 버전인데 파일이 손으로 고쳐졌다)는 **이 제품이 무엇을 잡아 주는지를 한 줄로 말하는 상태**인데, 심사위원이 보는 첫 표에 그게 없었다.

🔴 **고친 자리 — 픽스처 두 줄. 제품 코드는 0줄이다.** `fixtures/seed/demo.json` 의 `devices` 12 → 14:

| 더한 기기 | 무엇 |
|---|---|
| `linux-seoyeon-box` | `status: modified` · `version: official` — 버전은 공식인데 파일이 고쳐졌다 |
| `mac-minseo-new` | `report: null` — 토큰만 발급되고 아직 보고가 없다 → 서버가 `unknown` 을 매긴다 |

기존 12줄은 **안 건드렸다** — 수가 흔들리면 SPEC §10.3 과 갈린다.

🔴 **게이트로 올렸다 — 핵심은 「종류를 표에서 센다」이다.** `demo-guest.test.ts` 의 화면 9 시험이 넷을 잰다: ① 다섯 수(9·2·1·1·1) ② **`SYNC_STATUSES` 를 돌며 종류마다 0보다 큰가** ③ `unknown` 행은 버전·보고 시각이 **둘 다 `null`** 인가 ④ `modified` 행은 버전이 **공식**인가.
★ **②가 본체다** — ①만 있으면 `SYNC_STATUSES` 에 여섯째가 붙는 날에도 초록이라 **같은 구멍이 다시 난다.** 종류를 표에서 세야 「새 상태를 더했는데 데모에는 안 서는」 날 빨개진다.

🔴 **잰 것** (CI 13:27 · 관통 149초):

| | 전(109바퀴) | 후 |
|---|---|---|
| 화면 9 의 칩 | **3개** (applied·outdated·manual) | **5개** (+ modified · 보고 없음) |
| 데모 기기 | 12 | **14** |
| `SYNC_STATUSES` 중 데모에 서는 종류 | **3 / 5** | **5 / 5** |
| `apps/web` 시험 | 862 | **862** (같다 — `it` 을 더한 게 아니라 **기존 시험 안에 판정을 더했다**) |
| 관통 검사 | 1254 | **1254** |
| CI | GREEN | **GREEN 전 층** |

⚠ **커밋 메시지(`14a0719`)의 「시험 858 → 860」은 틀린 수다.** 실제로 세어 보니 앞뒤 모두 **862** 이고(`pnpm vitest run`), 858 은 109바퀴 STATUS 가 들고 있던 낡은 수였다. 시험 **개수**는 안 늘었다 — 늘어난 것은 기존 시험 안의 **판정**이다. 다음 바퀴는 이 표의 862 를 믿어라.

🔴 **눈으로 봤다** — `apps/web/public/shots/screen-sync.png` (이번 관통이 찍은 것 · `shotcopy` 가 옮겼고 저장소에 커밋돼 있어 `.ci/` 가 지워져도 남는다). 칩 다섯이 `applied 9 · outdated 2 · modified 1 · manual 1 · ? 보고 없음 1` 로 서고 「기기 14」다. `mac-minseo-new` 행은 버전 「—」 · 마지막 보고 「아직 보고가 없습니다」 · 적용 방식 「—」이고, `linux-seoyeon-box` 행은 `v1.1.0` · ✎ modified · 「7시간 전」 · 「플러그인」이다. 상태는 **아이콘 + 라벨**이라 색만으로 구분되지 않는다.

🔴 **문서도 같이 옮겼다** — SPEC §10.3 두 줄(`devices 12(...)` → `devices 14(...)`)과 `DESIGN_BRIEF` §4 화면 9 의 예시(12행 → 14행). 덤으로 그 예시가 `outdated` 를 **`v1.1.0`** 이라 적고 있던 것을 `v1.0.0`(앞 버전)으로 고쳤다 — `outdated` 는 정의상 앞 버전이고, 시험(`row.version === previous.semver`)과 화면이 이미 그렇게 말하고 있었다.

🔴 **2-B 이번 라운드 — 셋을 돌렸고 셋 다 살아 있다.** ① `SourceRef` 4종: `SRC_TAG` 표가 종류마다 **다른 접두사·다른 몸**을 내고 `srcKindOf()` 가 그 표를 뒤집어 되읽는다(`doc:`·`repo:`·`proposal:`·`manual:` — Pack 본문에서 `doc`·`repo`·`manual` 세 갈래를 실제로 봤다). ② `confidence` 3단계: 값이 **역추적 태그의 `conf:`** 로 나가 Pack 바이트를 바꾸고 `liveness.test.ts` 가 세 지문이 서로 다름을 잠근다. ③ `enforcement` 4종: `ENFORCEMENT_LABEL` 표가 본문 문구를 가르고 `liveness.test.ts` 가 넷의 지문이 갈리는지 잰다. **새로 적을 것 없음.**



> 🔴 **그 바퀴가 다음으로 지목한 것 — 「FINDINGS 없음 · ④1 로 돌아가라」.** 162 는 이번 바퀴에 닫혔고, 대장에 남은 대기는 109바퀴가 훑은 그대로다 — 156(배포 뒤) · 152(⛔ 다음 실패가 200자를 말할 때까지) · 140(🙋 새 PC) · 117(절삭 1번으로 잘랐다) · 33·24(「지금 열지 마라」). PLAN 의 `- [ ]` 셋도 전부 사람을 기다린다 (아래 「막힌 것」).
> **그래서 111 도 ④1 이었다** — 관통을 돌리고 **산출물을 눈으로 읽어 새로 찾아라.** 이번 바퀴가 그렇게 해서 162 를 찾았다.
> ★ 어디를 보면 나오나 (이번 바퀴가 쓴 방법): **표(enum)를 하나 골라 「데모·픽스처가 그 표의 값을 전부 한 번씩 보여 주나」를 세라.** 코드가 다 살아 있어도 **씨앗이 안 들어가면 화면에는 안 선다.** 아직 안 센 표: `ItemType` 10종 · 에러 코드 9종 · `PROPOSAL_STATUSES` · `PROGRESS_STATUSES` · `AI_JOB_STATUSES`.
> ⛔ **할 일이 없다고 멀쩡한 코드를 리팩터링하지 마라** — 없으면 「없다」고 적는 것이 그 바퀴의 성과다.

- PLAN 의 `- [ ]` 중 남은 것 **셋**: P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(156 · 152 · 140 · 117 · 24 · …) — **고장 0** · 이번 바퀴가 새로 찾은 것은 **구멍 하나(162)뿐이고 그 자리에서 닫았다.** 관통은 GREEN(9단계 · 1254검사)이다.

### 지난 바퀴 (109) — 데모 씨앗을 재현 가능하게 · 관통 캡처 세 장이 두 실행에서 byte-identical (FINDINGS 161 ✅ · `87eadea`)
**이번 바퀴(109)는 FINDINGS 161 을 닫았다 — 데모 씨앗이 재현 가능해졌고, 관통 캡처 세 장이 두 실행에서 byte-identical 이다.** INBOX 「할 것」비어 있음.
🟢 **CI GREEN 전 층 · 관통 1254검사 · SKIP 0.**

🔴 **무엇이 남아 있었나** — 108이 픽셀로 재서 원인은 확정했지만(인코더가 아니라 **데모가 매 관통마다 다시 심겨서**) 뿌리는 그대로였다: ⓐ 발행 시각과 ⓑ 문서 버전 uuid 가 심을 때마다 새로 났다. ⓑ 는 Pack 의 **모든 줄 태그**(`src:doc:<uuid>#260-311`)에 실려 `manifest_hash` 를 흔든다.

🔴 **어디에 뒀나 — 제품 코드는 0줄이다.** id 와 시각을 만드는 것은 **DB** 이므로, 결정론도 **하네스가 소유한 DB** 에 뒀다 (`apps/web/test/helpers/pin.ts`). 라우트에 「id·시각을 받는 문」은 **안 열었다** — 161 이 미리 못 박아 둔 ⚠ 그대로다 (데모 사정으로 제품 계약을 넓히는 것).

| 무엇 | 어떻게 |
|---|---|
| ⓑ uuid | `gen_random_uuid()` 기본값 → **계수 uuid** `pin_uuid()` (`public` 의 해당 칸 전부) |
| ⓐ `created_at`·`updated_at` | `now()` 기본값 → **고정 기준 시각 + 1ms 계수** `pin_now()` |
| ⓐ 발행 시각 | DB 기본값이 **아니다** — 라우트가 준 값이다 (`lib/api/route.ts` 의 `const now = new Date()`). 심은 **뒤에** `pinSeededClocks()` 가 `published_at = created_at` 으로 되돌린다 |
| 어디서 켜나 | `DEV_PIN=1` — **캡처 하네스(`e2e/shots.ts`)만.** 사람이 `demo:db` 로 보는 화면은 발행 시각이 「지금」이어야 자연스럽다 |

⚠ **시각을 한 값으로 얼리지 않았다.** 얼리면 `created_at` 이 전부 같아져 `order by created_at desc` 가 **순서를 잃는다** — churn 을 없애는 게 아니라 다른 자리로 옮기는 것이다. 그래서 1ms 씩 올라가는 계수 시계다.
⚠ **경과(`sinceText`)로 그리는 칸은 안 건드렸다.** 씨앗이 진짜 시각 기준으로 뒤로 미는 값이라(`hoursAgo`) 과거로 옮기면 「3시간 전」이 「N일 전」이 되어 **오히려 매일 달라진다.** `seedDemo(now)` 는 진짜 시각 그대로다.

🔴 **잰 것** (CI 12:59 · 관통 154초):

| | 전(108바퀴) | 후 |
|---|---|---|
| 두 실행의 캡처 | `screen-context` **307px** · `screen-packs` **1464px** 달라졌다 | **셋 다 0px** (바이트도 같다 — 99916 · 40189 · 81193) |
| 관통 뒤 워킹트리 | `.png` 두 장이 **M** | `git status --short` **비었다** |
| 같은 씨앗 → 같은 `manifest_hash` | 아니다 | **그렇다** (시험이 잠갔다) |
| `apps/web` 시험 | 856 | **858** (+2 · demo-seed-pinned) |
| 관통 검사 | 1252 | **1254** (+2) |
| CI | GREEN | **GREEN 전 층** |

🔴 **잠근 것 — 「안 박으면 달라진다」까지 잰다.** `test/demo-seed-pinned.test.ts` 2개: ① 박고 두 번 심으면 `manifest_hash`·발행 시각·문서 버전 id 가 **같다** ② **안 박으면 다르다.** ★ 왜 ②가 필요한가 — ①만 재면 `pinDbDefaults` 가 아무 일도 안 하게 되는 날(칸 이름이나 기본값 모양이 바뀌는 날)에도 초록이다. 그리고 `pinDbDefaults`·`pinSeededClocks` 는 **바꾼 칸 수·행 수를 돌려주고** 시험이 그것이 0이 아님을 요구한다.

🔴 **진짜 관통에서 확인했다** — 커밋 뒤 독립 실행 한 번 더. `shotcopy` 세 줄이 전부 `그림이 그대로다 — 안 옮겼다 — 달라진 픽셀 0개` 이고 `git status --short` 가 비었다. **104바퀴부터 여섯 바퀴 동안 매 커밋에 섞이던 `.png` diff 가 이번으로 끝났다.**
🔴 **눈으로 봤다** — `docs/evidence/2026-09-07-pinned-seed/` (`.ci/` 밖으로 복사했다). Pack Explorer 표의 발행 시각이 두 행 모두 `2026-09-01 00:00` 로 고정이고, 게스트 배너·「√ 공식 v1.1.0」·snapshot 해시 칸이 다 제자리다.

🔴 **2-B 이번 라운드 — `scope.kind` 3종** ① 소비처: `packages/compiler` 가 세 값으로 **파일을 가른다**(`domain-{slug}.md` · `scoped-{slug}.md`)고 `SCOPE_INLINE_LABEL` 이 본문 라벨을 낸다 ② 뒤집으면 갈림: `compiler/test/liveness.test.ts` 가 세 값의 파티션·정렬·라벨을 서로 다르게 잠그고, `schema/test/scope-and-enums.test.ts` 가 `domain`·`path` 는 `value` 없이는 파싱이 안 되는 것을 잰다. **살아 있다** — 새로 적을 것 없음.

**그 바퀴가 다음으로 지목한 것** — 없음. ④1 로 돌아가 관통을 돌리고 새로 찾으라고 적었다 (110바퀴가 그렇게 해서 **162** 를 찾았다).


> 🔴 **왜 「없음」인가 — 거짓말을 안 하려고 그렇다.** 대장의 대기 여섯을 다시 훑었는데
> **루프가 혼자 열 수 있는 것이 하나도 없다**: 156 은 배포 뒤 · 152 는 「⛔ 지금은 행동할 수
> 없다」(다음 실패가 200자를 말할 때까지) · 140 은 🙋 새 PC · 117 은 절삭 1번으로 잘랐다 ·
> 33·24 는 「지금 열지 마라」. PLAN 의 `- [ ]` 셋도 전부 사람을 기다린다 (아래 「막힌 것」).
> **그래서 다음 바퀴의 일은 ④1 로 돌아가 관통을 돌리고 새로 찾는 것**이다 (PROMPT ④3 의 ⛔).
> ⛔ **할 일이 없다고 멀쩡한 코드를 리팩터링하지 마라** — 없으면 「없다」고 적는 것이 그 바퀴의 성과다.

- PLAN 의 `- [ ]` 중 남은 것 **셋**: P5 첫 행(🙋 Vercel · 새 PC) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(156 · 152 · 140 · 117 · 24 · …) — **고장 0** · 이번 바퀴가 새로 찾은 고장도 **0** 이다(관통 GREEN · 9단계 · 1254검사). 161 은 닫혔다.

### 지난 바퀴 (108) — 캡처를 바이트가 아니라 **픽셀로** 견준다 · 안 달라졌으면 안 옮긴다 (FINDINGS 161 절반 · `f095e66`)
**이번 바퀴(108)는 FINDINGS 161 의 절반을 닫았다 — 캡처를 옮기기 전에 **픽셀로** 견주고, 안 달라졌으면 안 옮긴다.** 덤으로 **FINDINGS 25 를 재고 닫았다**(71 과 같은 것이었다 · 코드 0줄). INBOX 「할 것」비어 있음.
🟢 **CI GREEN 전 층 · 관통 1247 → 1252검사 · SKIP 0.**

🔴 **먼저 25 를 쟀다 — 닫기 전에.** 107 이 지목한 항목인데 「배선만 남았다」로 다섯 바퀴를 대기했고, 실제로는 **71 과 같은 것을 다른 번호로** 들고 있었다 (107 번 기록이 그렇게 적어 두었다). ★ 왜 그냥 안 닫았나 — **재지 않고 닫는 것이 더 나쁘다**(56 이 그렇게 틀린 사실을 대장에 남겼다). `api-routes.test.ts` 를 이름으로 걸어 **12개를 실제로 돌렸다**: 네 선택을 **라우트로** 눌러 진 쪽 항목을 다시 읽는 시험이 네 줄에서 **세 가지** 결과를 요구하고(`both`·`dismiss` 는 항목 0건), 이긴 쪽은 개정도 안 쌓이고, 폐기된 항목의 개정에 **어느 충돌에서 왔는지**가 남는다(P7). SPEC 쪽 갈림(§2 `a_ref` vs §7.2 `a_item_id`)도 이미 없다. **코드는 한 줄도 안 고쳤다.**

🔴 **그래서 이번 바퀴가 만든 것은 161 이다 — 그리고 161 이 적어 둔 원인 짐작이 틀렸다.**
161 은 「인코딩된 바이트가 달라진다 · 폰트 래스터·타이밍으로 보인다(**아직 안 쟀다**)」고 적혀 있었다. **쟀더니 아니다.**
...
`screen-sync` 가 두 관통에서 **바이트까지 똑같다** — 캡처(폰트 래스터·PNG 인코더)는 **결정론적**이다. 인코딩 잡음이면 이 장도 흔들렸어야 한다. 달라진 자리를 **잘라서 눈으로 읽었더니** `manifest_hash` 와 발행 시각 글자였다 (`d3585389` → `0d9a5f49` · `02:39` → `03:15`). 즉 **그림은 낡지 않았고, 데모가 매 관통마다 다시 심긴 것**이다 — Pack 의 모든 줄이 문서 **버전 uuid** 를 태그에 싣는데(`src:doc:<uuid>#260-311`) 그 uuid 가 심을 때마다 새로 난다. ⚠ **P4 위반이 아니다**(snapshot 자체가 다르다 · 컴파일러는 결정론적이다).

🔴 **만든 것 — 재는 자와 게이트.** `apps/web/e2e/png.ts` 가 PNG 를 **필터를 풀어** 픽셀로 읽고, `shotcopy` 가 옮기기 **전에** 견준다. 한 픽셀도 안 달라졌으면 **안 옮기고**, 달라졌으면 **몇 픽셀이 어느 줄에서** 달라졌는지를 관통 로그에 매 바퀴 찍는다.
★ 왜 라이브러리를 안 쓰나 — 여기 필요한 것은 Chrome 이 찍는 **한 종류**(8비트 · 인터레이스 없음 · RGB/RGBA)뿐이고, 그 밖의 모양은 **던진다.** 조용히 「안 달라졌다」로 답하면 이 검사가 아무 말도 안 하는 검사가 된다.
★ 왜 인코더는 시험 안에 뒀나 — 제품에도 관통에도 PNG 를 **쓰는** 쪽이 없다. `e2e/png.ts` 에 두면 아무도 안 부르는 함수가 하나 생긴다 (이 저장소가 매 바퀴 찾는 그 모양이다).

🔴 **잰 것** (CI 12:34 · 관통 149초):

| | 전(107바퀴) | 후 |
|---|---|---|
| churn 의 원인 | 「폰트 래스터·타이밍으로 **보인다**」 | **쟀다** — 데모 재시드(hash·시각) · 인코더는 결정론적 |
| 안 달라진 캡처를 옮기나 | **무조건 옮긴다** | **안 옮긴다** (`screen-sync` 실제로 건너뛰었다) |
| 얼마나 달라졌나 | 아무도 안 쟀다 | **매 관통 로그에 픽셀 수와 줄 번호** |
| 관통 검사 | 1247 | **1252** (+5) |
| `apps/web` 시험 | 851 | **856** (+5 · e2e-png) |
| CI | GREEN | **GREEN 전 층** |

🔴 **진짜 관통에서 확인했다** — `shotcopy` 로그 그대로: `screen-sync: 그림이 그대로다 — 안 옮겼다 — 달라진 픽셀 0개` · `screen-context: … 달라진 픽셀 399개 / 1296000 · 줄 156-164` · `screen-packs: … 1182개 · 줄 243-305`. **세 번째 독립 실행이 같은 줄 범위를 냈다** — 원인 진단이 재현된다.
🔴 **눈으로 봤다** — `docs/evidence/2026-09-07-shot-churn/` (`.ci/` 밖으로 복사했다): 두 관통의 원본 두 장 + 달라진 자리만 잘라낸 네 장. 잘라낸 그림에 해시 글자가 **읽힌다** — 이것이 「인코딩 잡음이 아니다」의 증거다.
⚠ **두 장은 이번에도 `M` 이 됐고 `git checkout` 으로 되돌렸다** — 106·107 과 같다. 다만 이제 **이유를 안다**(그림이 진짜로 달라졌다). 게이트는 그걸 **옳게** 옮긴 것이다.

🔴 **2-B 이번 라운드 — `confidence` 3단계** ① 소비처: `packages/compiler/src/tag.ts` 가 `conf:` 로 **Pack 본문 태그에 찍고**, 화면 5 의 표가 같은 값을 칸으로 그린다 ② 뒤집으면 갈림: `compiler/test/liveness.test.ts` 가 세 값의 fingerprint 를 서로 다르게 잠근다. **살아 있다** — 새로 적을 것 없음.

**그 바퀴가 다음으로 지목한 것** — FINDINGS 161 의 남은 절반 (109바퀴가 `87eadea` 로 닫았다).

### 지난 바퀴 (107) — `pack_files.source_map` 을 지웠다 · 역추적의 정본은 태그 하나다 (FINDINGS 34 · `bc8bbbb`)
...
**그 바퀴가 다음으로 지목한 것** — FINDINGS 25 (108바퀴가 **재고** 닫았다 — 71 과 같은 것이었다).
...
### 지난 바퀴 (101) — 좁은 폭에서 껍데기가 내비를 접는다 · 경계는 `--bp-narrow` 한 자리 · 375px 가로 스크롤 0 (FINDINGS 160 · `8daf8a8`+`1839969`)
**그 바퀴(101)는 FINDINGS 160 — 격차(앱 껍데기가 375px 에서 내비를 안 접어 본문이 76px 로 눌린다)를 닫았다** (`8daf8a8` + `1839969`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **여덟 바퀴째 같다**).
**이 저장소에는 breakpoint 가 0개였다** (`@media` 는 `prefers-reduced-motion` 하나뿐). 그래서 375px 에서 좌측 내비가 `--nav-w: 220px` 를 그대로 차지했고 `.main-inner` 가 **76px** 로 눌려, 안 들어가는 것이 전부 밖으로 밀렸다 — **가로 스크롤이 이미 있었다**(문서 폭 444px · 뷰포트 375).
들인 것은 **경계 한 자리**다: `--bp-narrow: 720px`. 그 아래에서 `.shell` 이 세로로 서고 `.nav` 가 가로 막대가 된다. **접히는 것은 탭 줄(`.nav-links`)뿐**이고 갈 곳은 **이미 내비 안에 있는 ⌘K 팔레트**가 받는다(같은 `PROJECT_SCREENS` 표를 읽는다 · FINDINGS 132·157). ⛔ **새 내비를 만들지 않았다** — 「어디로 갈 수 있나」를 말하는 자리가 셋이 되면 안 된다. 팀·프로젝트 이름과 팔레트 버튼은 남긴다.
🔴 **게이트로 올렸다** — `design-tokens.test.ts` ⑨ 넷: ① `--bp-narrow` 가 있다 ② 폭 질의가 **하나뿐**이고 그 수가 토큰과 같다(CSS 변수는 `@media` 안에서 못 읽어 수를 적어야 한다 — 그 짝이 갈라지는 것을 잡는다) ③ 그 블록이 `.nav .nav-links` 를 접고 뼈대의 탭 줄이 그 이름을 단다 ④ **블록이 파일 맨 끝이다**.

🔴 **잰 것** (진짜 브라우저 · CDP 375×812 · CI 08:28 · 08:41):

| | 전(99바퀴가 잼) | 후 |
|---|---|---|
| `documentElement.scrollWidth` (뷰포트 375) | **444px** | **375px** |
| 가로 스크롤 | 있다 | **없다** (`overflowPx: 0`) |
| `.main-inner` 의 `clientWidth` | **76px** | **343px** |
| `.nav` 폭 · 방향 | 220px · `column` | **375px · `row`**(가로 막대) |
| 탭 줄 `.nav-links` | 보인다(세로 7줄) | **`display: none`** — 갈 곳은 ⌘K |
| 앱 화면 **여덟 개 전부** | 안 쟀다 | **여덟 다 `overflowPx: 0` · 본문 343px** |
| 시험 | 781개 (777 통과) | **785개 (781 통과)** — +4 (design-tokens ⑨) |

🔴 **눈으로 봤다 — 실제 브라우저다** (`docs/evidence/2026-09-07-narrow-shell-collapses-nav/` · 캡처 6장 · `report.json` · `measure.mjs`). 들어간 길은 제품과 같다: `demo:db` + `next dev`(3111) → 헤드리스 Chrome(CDP) → `/demo` → 게스트 세션 → 앱 화면. 375×812 는 `Emulation.setDeviceMetricsOverride` 로 만든 **진짜 뷰포트**다. **팔레트를 실제로 눌러 열었다** — 폭 309px 로 뷰포트 안에 들어가고 줄이 8개(화면 7 + 프로젝트 1)라 접힌 뒤에도 **모든 화면에 갈 수 있다**. 1440×900 은 그대로다(내비 219 · 본문 1141 · Pack 3열 유지).

🔴 **도중에 두 번 틀렸고 둘 다 브라우저가 잡았다 — 커밋이 둘인 이유다.**
① 첫 커밋(`8daf8a8`)의 `.nav-links { display: none }` 이 **아무것도 안 접었다.** 블록을 파일 가운데(`.main-inner` 옆)에 뒀는데 아래에 있는 `.col-tight`(`display: flex`)가 **같은 특정도로 이겼다.** 규칙은 있는데 화면은 그대로였다 — 「정의만 있고 아무 일도 안 하는 코드」의 **CSS 판**이다(④2-B 가 말하는 종류인데, 시험이 글자로만 세면 초록으로 지나간다). → 블록을 **파일 맨 끝**으로 옮기고, 「`.col-tight`·`.row` 가 폭 질의보다 앞이고 질의 뒤에 아무 규칙도 없다」를 ⑨④ 로 잠갔다.
② 내비를 접어도 화면 3 이 **376px** 였다(1px). 범인은 `.drawer`(`flex: 0 0 360px`)다 — 고정 폭이라 안 줄어든다. 화면 7 의 3열(240·320)은 더 심해서 좁은 폭에서 **글자가 세로로 한 자씩** 섰다. → `.row { flex-wrap: wrap }` + 옆 칸 셋을 `flex: 1 1 100%` 로. 지금은 세로로 쌓인다(`05-packs-375.png`).

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 08:41 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 781 전부 초록** · principles OK 9 · typecheck OK · docs OK · `build` 는 따로 돌려 **OK**. README 를 마지막으로 고친 커밋은 여전히 `208bedf`(05:36) · 그 세션의 마지막 커밋도 `eceaaab`(05:46) 그대로다 — **여덟 바퀴째 안 늘었다**. 101바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **2-B 이번 라운드 — 「`--bp-narrow` 와 폭 질의가 실제로 화면을 바꾸나」** ① 소비처: 폭 질의 **하나**가 `.shell`·`.nav`·`.nav-links`·`.row`·옆 칸 셋·`.main` 을 읽는다 ② 뒤집으면 갈림: 블록을 파일 가운데로 되돌리면 브라우저에서 `.nav-links` 가 `flex` 로 돌아오고 문서 폭이 444 로 돌아간다 — 이번 바퀴가 **실제로 그 상태를 봤고**, 그래서 ⑨④ 를 만들었다. 즉 이 규칙은 **살아 있다**. ⚠ 반대로 「CSS 규칙이 있다」만 글자로 세는 시험은 **아무것도 보증하지 않는다** — 특정도·순서에 밀리면 조용히 죽는다.

그 바퀴가 **다음으로 지목한 것**: FINDINGS 153 (이번 바퀴가 닫았다 · `30f0fd8`).
🔴 PLAN 의 `- [ ]` 맨 위는 여전히 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이다. 그 몫인 격차는 **160 을 닫아 131 만 남았고**, 131 은 랜딩이라 155 가 닫힐 때까지 못 연다. ⚠ 156 은 배포 뒤에 재고(고르지 마라) · 152 는 관찰 중 · 140 은 P5 둘째 행(🙋 새 PC)과 같이 · 117 은 절삭 1번.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(155 · 156 · 153 · 152 · 140 · 117 · 131) — **고장 1**(155 · 주인은 남의 세션) · 160 ✅.

### 지난 바퀴 (100) — 빈 상태가 세는 것을 말한다 · 자리 이름도 세는 것과 같게 (FINDINGS 159 · `073308a`)
**그 바퀴(100)는 FINDINGS 159 — 격차(화면 3 「구조화 진행」의 빈 상태가 사실이 아닌 것을 말한다)를 닫았다** (`073308a`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **여덟 바퀴째 같다**).
**그 칸은 「도는 job 이 없을 때」 서는데 문구는 `아직 올린 문서가 없습니다` 였다.** 데모에는 문서가 **2건** 있다(씨앗이 `POST /documents` 를 201 로 두 번 부른다). 없는 것은 문서가 아니라 **job** 이다 — 화면이 사실이 아닌 것을 말하면 「이미 올렸는데?」 하는 사람은 그 칸을 못 믿는다.
고친 자리는 **표의 한 줄**이다. 정본은 `apps/web/src/lib/web/screens.ts` 의 `EMPTY_PLACES` 이고 화면 코드에는 문구가 0건이다(FINDINGS 133 이 그렇게 옮겨 놨다). 목적지는 그대로 같은 화면 왼쪽 칸이라 `noNext` 도 안 건드렸다.
🔴 **자리 이름도 같이 바꿨다 — `import.docs` → `import.jobs`.** 이름이 **세는 것과 다르면** 다음 사람이 그 이름을 믿고 같은 문구를 다시 쓴다. `EmptySlot` · 표 · 화면 세 곳이고 `Record` 라 하나만 고치면 타입이 먼저 막는다.
🔴 **게이트로 올렸다** — `web-empty-states.test.ts` ⑤ 둘: ① 그 칸이 실제로 `!job` 으로 갈리는지 ② 문구가 「올린 문서가 없다」고 말하지 않는지. 「화면이 사실이 아닌 것을 말한다」는 눈으로만 보면 다음에 또 새 것이 나온다.

🔴 **잰 것** (진짜 브라우저 · CI 08:06 · 08:11):

| | 전(98·99바퀴) | 후 |
|---|---|---|
| 「구조화 진행」 칸의 문장 | `아직 올린 문서가 없습니다. 왼쪽에 문서를 붙여넣어 보세요.` | **`구조화 중인 문서가 없습니다. 왼쪽에 문서를 붙여넣고 [구조화하기] 를 눌러 보세요.`** |
| 그 말이 사실인가 (문서 2건 · structure job 0개) | **아니다** | **그렇다** |
| 빈 자리 이름 | `import.docs` | **`import.jobs`** |
| 시험 | 779개 (775 통과) | **781개 (777 통과)** — +2 (web-empty-states ⑤) |

🔴 **눈으로 봤다 — 실제 브라우저다** (`docs/evidence/2026-09-07-import-empty-says-jobs/`). 들어간 길은 제품과 같다: `demo:db` + `next dev`(3111) → 헤드리스 Chrome(CDP · 1440×900) → `/demo` → 게스트 세션 → `/t/demo/p/paylab-api/import`. `01-structure-panel.png` 에서 그 칸은 **문장 하나**이고 버튼이 없다(갈 곳이 같은 화면 왼쪽이라 `noNext`), 문장이 가리키는 [구조화하기] 가 실제로 그 왼쪽 카드 아래에 있다. 콘솔로 같이 쟀다: `saysUploadedDocs:false` · `saysStructuring:true` · `GET …/jobs?feature=structure` **0개**.
⚠ **문서 건수는 `GET` 으로 못 센다** — 문서 라우트에 `GET` 이 없어서 **405** 다(`count.mjs` 가 그렇게 답했다). 그래서 「문서 2건」은 **씨앗 서버가 낸 요청 로그**(`POST /projects/{id}/documents` 201 ×2)로 셌다. 「문서 목록」이 필요해지면 그건 **새 라우트**이고 여기서 열지 않았다.
⚠ **못 본 것** — 375px 모바일(1440×900 만 · 그건 **FINDINGS 160** 이 주인이다) · 진짜로 도는 job 이 있을 때의 그 칸(키가 있는 배포 뒤 · 156 과 같은 자리).

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 08:11 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 777 전부 초록** · principles OK 9 · typecheck OK · docs OK · `build` 는 따로 돌려 **OK**. README 를 마지막으로 고친 커밋은 여전히 `208bedf`(05:36) · 그 세션의 마지막 커밋도 `eceaaab`(05:46) 그대로다 — **일곱 바퀴째 안 늘었다**. 100바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **2-B 이번 라운드 — 「빈 자리 표(`EMPTY_PLACES`)의 열 줄이 전부 뭔가를 바꾸나」** ① 소비처: `ScreenEmpty` 하나가 읽고 화면 여덟이 `slot=` 으로만 부른다 — 화면 코드에 문구가 **0건**임을 시험 ②가 글자로 센다 ② 뒤집으면 갈림: 이번 바퀴가 `import.docs` 를 지우고 `import.jobs` 로 바꾸자 **타입이 먼저 막았고**(`Record<EmptySlot, …>`), 문구를 바꾸자 브라우저의 그 칸 글자가 그대로 따라 바뀌었다(위 표). 즉 표는 **살아 있다** — 열 줄 중 이 줄은 확실히 화면을 바꾼다.

### 지난 바퀴 (99) — 쓰는 법을 말하는 placeholder 가 안 잘린다 · 폭을 먼저 재고 토큰으로 고친다 (FINDINGS 158 · `89d2a82`)
**이번 바퀴(99)는 FINDINGS 158 — 격차(화면 5 의 `scope` 거르개 placeholder 가 칸 폭에서 잘린다)를 닫았다** (`89d2a82`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **일곱 바퀴째 같다**).
**「잘린다」는 96바퀴가 캡처로 봤고, 몇 px 모자라는지는 아무도 안 쟀다.** 그래서 이번 바퀴의 첫 일은 고치는 게 아니라 **재는 것**이었다 — 진짜 브라우저(CDP · 1440×900 · `demo:db` + `next dev`)에서 계산된 스타일과 canvas 로 잰 결과 칸 안쪽이 **195px** 인데 문구(`project · domain:billing`)가 **203.91px** 이라 **8.91px 모자랐다.** 화면에는 `domain:billin` 으로 보였다 — **쓰는 법을 가르치는 자리가 틀린 문법을 가르치고 있었다.**
고친 자리는 **폭 하나**다. 정본은 `globals.css` 의 **`--filter-input-ch: 26`(글자 수) → `--filter-input-w`(+ 자기 padding·border)** 이고 `.input-filter` 가 그것을 읽는다. **px 가 아니라 `ch`** 다 — mono 라 글꼴이 폴백으로 바뀌어도 문구와 같이 늘어난다. `.input` 은 안 건드렸다(모달·설정도 쓴다).
🔴 **게이트로 올렸다** — `design-tokens.test.ts` ⑧ 이 **문구의 글자 수 + 여유 2자 ≤ `--filter-input-ch`** 를 센다. 다음 사람이 문구를 한 자 늘리면 **캡처가 아니라 시험**이 먼저 빨개진다. 「잘린 글자」는 눈으로만 보면 다음에 또 새 것이 나온다.

🔴 **잰 것** (진짜 브라우저 · CI 07:46 · 08:0x):

| | 전(98바퀴) | 후 |
|---|---|---|
| 화면에 보이는 문구 | `project · domain:billin` (**`g` 잘림**) | **`project · domain:billing`** |
| 칸 안쪽(패딩 뺀) 폭 vs 문구 폭 | 195px vs **203.91px** → **8.91px 모자람** | 221px vs 203.91px → **17.09px 남음** |
| 칸 바깥 폭 | 221px | **247px** (`calc(26 * 1ch + var(--sp-3)*2 + 2px)`) |
| 시험 | 776개 (772 통과) | **779개 (775 통과)** — +3 (design-tokens ⑧) |

🔴 **눈으로 봤다 — 실제 브라우저다** (`docs/evidence/2026-09-07-scope-placeholder/`). 들어간 길은 제품과 같다: `/demo` → 게스트 세션 → `/t/demo/p/paylab-api/context`. `01-before-1440.png` 와 `02-after-1440.png` 를 나란히 보면 바뀐 것이 **그 칸 하나**뿐이다 — 표도 머리도 그대로다. 폰트도 둘이 같다(`14.5px "JetBrains Mono", …`).

⚠ **375px 은 이 고침으로 좋아지지 않는다 — 그리고 그게 더 큰 것을 물고 나왔다.** 껍데기가 내비 `--nav-w: 220px` 를 **안 접어서** `.main-inner` 가 **76px** 이고, 이 화면은 그 전에 이미 가로로 밀려 있다(문서 폭 **444px** · 뷰포트 375). `globals.css` 에 breakpoint 는 **0개**다(`@media` 는 `prefers-reduced-motion` 하나뿐). 걸어 둔 `max-width: 100%` 는 라벨이 내용 폭 그대로 커져서 지금은 아무 일도 안 한다 — 그래서 이번 바퀴에 **breakpoint 를 새로 들이지 않았다**(한 바퀴에 하나). **FINDINGS 160**(격차)으로 적었다. ⚠ INBOX 2026-09-06 의 「모바일 375px 가로 스크롤 0」은 **랜딩**에서 잰 것이고 앱 껍데기는 안 쟀다 — 그 문장을 앱 화면의 근거로 쓰지 마라.

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 07:46 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 772 전부 초록** · principles OK 9 · typecheck OK · docs OK. README 를 마지막으로 고친 커밋은 여전히 `208bedf`(05:36) · 그 세션의 마지막 커밋도 `eceaaab`(05:46) 그대로다 — **여섯 바퀴째 안 늘었다**. 99바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **2-B 이번 라운드 — 「토큰이 정말로 폭을 바꾸나」** ① 소비처: `--filter-input-ch` → `--filter-input-w` → `.input-filter` → 화면 5 의 그 칸 하나. 표가 아니라 사슬이라 ②를 브라우저에서 직접 뒤집었다: `.input-filter{width:auto}` 를 얹으면 칸이 **245 → 219px** 로 돌아가고 `fits` 가 **true → false** 로 뒤집힌다(1440×900 · 같은 세션에서 직접 확인). 즉 이 토큰은 **살아 있다** — 값을 바꾸면 화면이 바뀐다.

**그 바퀴가 다음으로 지목한 것** — FINDINGS 159 (100바퀴가 `073308a` 로 닫았다).

### 지난 바퀴 (98) — 씨앗은 끝난 상태만 심는다 · 데모 화면 3 의 가짜 대기를 없앤다 · 수명 표의 `finished` 축을 읽는다 (FINDINGS 137 · `2604b9f`)
**이번 바퀴(98)는 FINDINGS 137 — 격차(데모의 화면 3 에 씨앗이 남긴 job 이 「⚠ 멈춘 것 같음」으로 떠 있다)를 닫았다** (`2604b9f`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **여섯 바퀴째 같다**).
**그 카드는 게스트가 만든 것이 아니라 씨앗이 남긴 것이다.** `seedPaylab` 은 제품 라우트를 그대로 부르므로 `POST /documents` 둘과 `batch-draft` 하나가 job 3개를 만들고 굴린다. 그런데 씨앗은 그 job 이 낼 것(항목·근거)을 **이미 손으로 심어 놓았고**(`paylabDrafts()` 가 정본), 키가 없는 개발용·데모 서버에서는 러너가 아무것도 못 한다 — 그래서 `queued` 가 영원히 `queued` 이고 화면은 그것을 정직하게 「멈춘 것 같음」으로 그렸다. **심사위원이 처음 여는 화면이 그것이다.**
고친 자리는 **씨앗 한 곳**이다 — `seedPaylab` 끝에서 **끝나지 않은 job 행만** 지운다. 「끝났나」를 손으로 세지 않고 수명 표 `AI_JOB_STATUS_RULES` 의 `finished` 축을 읽는다(`UNFINISHED_JOB_STATUSES`) — 상태가 하나 늘어도 그 표에 한 줄이면 따라온다.
🔴 **안 한 것 둘이 이 고침의 절반이다.** ① **끝난 job 은 안 지운다** — 키가 있는 자리에서 진짜로 돈 job 은 결과 카드로 보여야 한다 ② **「멈춘 것 같음」 chip 도 안 건드렸다** — 진짜로 멈춘 것을 숨기면 그건 고침이 아니라 은폐다. 없앤 것은 **씨앗이 만든 가짜 대기**뿐이다.

🔴 **잰 것** (시험 · CI 07:35 · 브라우저 07:4x):

| | 전(97바퀴) | 후 |
|---|---|---|
| 게스트가 보는 job (`GET /projects/{id}/jobs?limit=50` · **진짜 HTTP**) | 3 (structure 2 · conflict 1 · 전부 `queued`) | **0** |
| 화면 3 의 「멈춘 것 같음」·「차례 기다리는 중」 | 있다 | **0건** (`document.body.innerText`) |
| 씨앗 **뒤에** 올린 문서의 job | 1 | **1** — 지우는 자리가 씨앗뿐이다 (시험이 잠근다) |
| 시험 | 773개 (769 통과) | **776개 (772 통과)** — +3 (demo-guest) |

🔴 **눈으로 봤다 — 실제 브라우저다** (`docs/evidence/2026-09-07-demo-no-phantom-job/`). `pnpm --filter web demo:db` + `next dev`(3111) 위에서 헤드리스 Chrome 을 CDP 로 몰아 제품과 같은 길(`/demo` → 게스트 세션 → `/t/demo/p/paylab-api/import`)로 들어갔다. `02-structure-panel.png` 의 「구조화 진행」 칸에 카드가 **하나도 없고** 그 자리에 빈 상태가 선다.
⚠ **그 자리에서 하나 잡았다** — 빈 상태 문구가 `아직 올린 문서가 없습니다` 인데 데모에는 **문서가 2건 있다.** 없는 것은 문서가 아니라 도는 job 이다. `EMPTY_PLACES` 의 한 줄이고 **FINDINGS 159**(격차)로 적었다.
⚠ **못 본 것 둘** — 375px 모바일(1440×900 만 · 94바퀴부터 밀린 것과 같은 자리) · **키가 있는 배포에서의 모양**(진짜로 도는 job 이 남는지는 Vercel 뒤에 본다 · 156 과 같은 자리) → 「눈 판정 대기」.

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 07:35 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 772 전부 초록** · principles OK 9 · typecheck OK · docs OK · `build` 는 따로 돌려 **OK**(라우트 16개). README 를 마지막으로 고친 커밋은 여전히 `208bedf`(05:36) · 그 세션의 마지막 커밋도 `eceaaab`(05:46) 그대로다 — **다섯 바퀴째 안 늘었다**. 98바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **2-B 이번 라운드 — job 수명 4종의 `finished` 축** ① 소비처: DB CHECK 생성(`schema.ts`) · `isJobStalled()` · 그리고 이번에 **씨앗**이 넷째로 붙었다(`UNFINISHED_JOB_STATUSES`) ② 뒤집으면 갈림: `finished` 를 뒤집으면 씨앗이 지우는 행이 달라져 demo-guest 의 새 시험 셋 중 둘이 빨개진다(직접 확인) · 손으로 `['queued','running']` 을 적으면 셋째 시험이 표와의 동일성을 잡는다.

**그 바퀴가 다음으로 지목한 것** — FINDINGS 158 (99바퀴가 `89d2a82` 로 닫았다).

### 지난 바퀴 (97) — 움직임을 줄여 달라면 줄인다 · 블록 하나가 전부 끈다 · `animation: none` 이 아니라 끝난 상태로 세운다 (FINDINGS 134 · `fe7cc98`)
**이번 바퀴(97)는 FINDINGS 134 — 격차(움직임을 줄여 달라는 설정을 화면이 안 읽는다)를 닫았다** (`fe7cc98`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **다섯 바퀴째 같다**).
**css 를 통틀어 `prefers-reduced-motion` 이 1건뿐이었고 그마저 조각 하나(터미널 커서)의 것이었다.** 그래서 스켈레톤 맥박(1.4s · infinite)도 진행 막대의 전환(0.3s)도 설정과 상관없이 계속 움직였다. 이건 취향이 아니라 **증상**이다(전정 장애·편두통).
정본을 **한 곳**으로 올렸다 — `globals.css` 의 포커스 링 옆에 `@media (prefers-reduced-motion: reduce)` 블록 하나이고, 선택자가 `*, *::before, *::after` 라 **아직 없는 애니메이션까지 덮는다.** `terminal-replay.module.css` 가 갖고 있던 자기 블록은 지웠다(왜 지웠는지 그 자리에 한 줄) — 조각마다 적으면 새로 움직이는 것이 생길 때 **반드시 하나를 빠뜨린다.**
🔴 **`animation: none` 이 아니라 `0.01ms + 1회` 다.** `none` 은 재생 중이던 것을 **시작 상태로 되돌린다** — 진행 막대가 0으로 튀고 터미널이 빈다. 끝난 상태로 세워야 「움직임만 없고 내용은 그대로」가 된다. 아래 캡처 둘의 차이가 정확히 그것이다.
**CSS 로 못 끄는 것은 JS 가 같은 질의를 읽는다** — 랜딩의 터미널 재생은 `matchMedia('(prefers-reduced-motion: reduce)')` 가 참이면 재생을 아예 안 켜고 서버가 그린 **마지막 프레임**을 그대로 둔다. 이건 62바퀴부터 있던 것이고, 이번에 시험이 그 순서(`matchMedia` 가 `setPlaying(true)` 보다 앞)를 잠갔다.

🔴 **잰 것** (시험 · CI 07:19 · 브라우저 07:3x):

| | 전(96바퀴) | 후 |
|---|---|---|
| css 의 `prefers-reduced-motion` 블록 | **1개** (조각 하나의 커서만) | **1개인데 자리가 다르다** — `globals.css` 전역 · 시험이 「css 를 통틀어 하나」를 센다 |
| `.bar-fill` 의 `transition-duration` (설정 켬) | **0.3s** | **1e-05s** |
| `.skeleton` 의 `animation` (설정 켬) | **1.4s · infinite** | **1e-05s · 1회** |
| 6초 시점 터미널 (설정 켬) | — | **17/17 줄 · 커서 0** (안 켰을 때는 16/17 · 커서 1) |
| `!important` 를 쓰는 자리 | 0곳 | **1곳** (끄는 스위치라 뒤의 어떤 규칙보다 세야 한다) |
| 시험 | 769개 (765 통과) | **773개 (769 통과)** — +4 (design-tokens ⑦) |

🔴 **눈으로 봤다 — 실제 브라우저다** (`docs/evidence/2026-09-07-reduced-motion/`). 이건 **캡처만으로는 판정이 안 되는 것**이라(OS 설정을 켜야 보인다) CDP 의 `Emulation.setEmulatedMedia` 로 같은 페이지를 켠 채·안 켠 채 두 번 열고 **계산된 스타일을 직접 읽었다**. `03-control-terminal.png` 은 6초 시점에 명령이 `$ node … progre` 에서 끊겨 있고 커서가 서 있고 Roadmap 이 「근거 0 / 3」인데, `04-reduce-terminal.png` 은 같은 시각에 **17줄이 다 서 있고 커서가 없고 막대가 채워진 채 「근거 1 / 3」**이다. 화면이 **비는 것이 아니라 끝난 상태로 선다.**
⚠ **못 본 것 셋** — 진짜 OS 설정(에뮬레이션이다) · **화면에 실제로 뜬 스켈레톤**(랜딩에 `.skeleton` 이 없어 같은 스타일시트 아래 하나 붙여 쟀다) · **375px 모바일**(1440×900 만 · 94바퀴부터 밀린 것과 같은 자리) → 「눈 판정 대기」.

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 07:19 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 769 전부 초록** · principles OK 9 · typecheck OK · docs OK · `build` 는 따로 돌려 **OK**(라우트 16개). 그 세션의 마지막 커밋은 **여전히 `eceaaab`** 다 — 네 바퀴째 커밋이 안 늘었다. 97바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **2-B 이번 라운드 — 움직이는 곳(css 3 + JS 1)** ① 소비처: `globals.css` 의 블록 하나가 `.skeleton`·`.bar-fill`·`.caret` 을 전부 덮고, 타이머 재생은 `terminal-replay.tsx` 가 같은 질의를 읽는다 ② 뒤집으면 갈림: `!important` 하나를 빼면 시험 ①이 빨개지고(직접 확인), 조각에 블록을 하나 더 두면 ②가, `matchMedia` 를 `setPlaying` 뒤로 옮기면 ③이 빨개진다. 브라우저에서도 갈린다 — 위 표의 계산된 값 넷이 설정만으로 갈렸다.

**그 바퀴가 다음으로 지목한 것** — FINDINGS 137 (98바퀴가 `2604b9f` 로 닫았다).

### 지난 바퀴 (96) — 빈 상태에 다음 행동 · 문구도 목적지도 표 하나(`EMPTY_PLACES`)가 정한다 · 눈이 곁다리로 잡은 것(158) (FINDINGS 133 · `e3e48fe`)
**그 바퀴(96)는 FINDINGS 133 — 격차(빈 상태에 다음 행동이 없다)를 닫았다** (`e3e48fe`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **네 바퀴째 같다**).
**문제는 「버튼이 없다」가 아니라 「문구와 목적지가 화면마다 손으로 적혀 있다」였다.** 빈 자리 열 곳 중 **여덟 곳에 갈 곳이 없었고**, 있는 둘(정리·로드맵)은 그 화면 안에 링크를 직접 지어 두고 있었다. 그래서 「Context 화면에서 [발행하기]를 눌러보세요」처럼 **갈 곳을 글로만** 말하는 자리가 생긴다 — 화면 하나만 버튼이 없어도 그 화면은 **비어 있을 뿐 멀쩡해 보인다.** 정본을 표 하나(`lib/web/screens.ts` 의 `EMPTY_PLACES` · `EmptySlot` 10자리)로 올리고 그리는 자리는 `ScreenEmpty` 하나다. 화면은 **자리 이름만** 말한다.
🔴 **아무 데도 안 가는 버튼을 만들지 않는 것이 이 표의 절반이다** (117 과 같은 뿌리). 갈 곳이 화면 밖(CLI · 같은 화면의 다른 칸)인 셋은 `noNext` 에 **왜 없는지**를 적었고, 타입이 `next` 와 `noNext` 중 하나를 **반드시** 쓰게 한다 — 빠뜨린 것과 일부러 안 둔 것이 코드에서 구별된다.
**목록 셋이 문구를 짓지 않는다** — `ProposalTable`·`DeviceTable`·`VersionHistory` 가 `emptyMessage: string` 대신 `empty: ReactNode` 를 받는다. 같은 `state-box` 마크업 세 벌이 사라졌고, 「비었을 때 무엇을 말하나」는 이제 표 하나만 안다.

🔴 **잰 것** (시험 · CI 07:03 · 브라우저 07:1x):

| | 전(95바퀴) | 후 |
|---|---|---|
| 빈 자리 중 **갈 곳이 있는** 것 | **2/10** (정리 · 로드맵) | **7/10** + 이유가 적힌 3 (`noNext`) |
| 그 문구·목적지의 출처 | 화면 파일 8곳 | **`EMPTY_PLACES` 한 표** (화면 코드에 빈 상태 문구 **0건** — 시험이 센다) |
| 404 로 가는 버튼 | (지어낸 주소를 막는 것이 없었다) | **0** — `to` 는 `PROJECT_SCREENS` 의 `path` 뿐이고 시험이 `page.tsx` 실재를 센다 |
| 빈 상태를 그리는 자리 | `EmptyState` 직접 호출 6 + `state-box` 사본 3 | **`ScreenEmpty` 하나** |
| 시험 | — | **+12** (769개 중 765 통과 · 빨간 넷은 155) |

🔴 **눈으로 봤다 — 진짜 브라우저다** (`docs/evidence/2026-09-07-empty-next-step/`). 헤드리스 Chrome + 실제 `next dev` + 씨앗 DB 에서 진짜 로그인 경로로 들어가 1440×900 으로 찍었다. 씨앗 프로젝트는 항목 27개라 빈 화면이 안 나오므로 **제품의 문으로**(`POST /teams/{id}/projects`) `paylab/blank` 를 만들어 **정말 아무것도 없는 프로젝트**를 열었다. Context 의 버튼을 실제로 눌러 `/t/paylab/p/blank/import` 로 **갔다**. Pack Explorer 의 버튼만 accent(계산된 배경 `rgb(31,78,224)` = `--accent`)이고 Context 의 것은 outline 이다 — 그 화면의 accent 는 머리의 [발행하기] 하나다 (`.btn-primary` 1개).
⚠ **못 본 자리 셋** — 정리(새 프로젝트에도 씨앗 질문 10장이 떠서 카드 0이 안 된다) · Pack 파일 0개(발행이 없으면 그 화면을 못 연다) · **375px 모바일**(1440×900 만 · 94바퀴부터 밀린 것과 같은 자리) → 「눈 판정 대기」.

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 07:03 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 765 전부 초록** · principles OK 9 · typecheck OK · docs OK · `build` 는 따로 돌려 **OK**(라우트 16개). 그 세션의 마지막 커밋은 **여전히 `eceaaab`** 다 — 세 바퀴째 커밋이 안 늘었다. 96바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**

🔴 **곁다리로 눈에 걸린 것 → FINDINGS 158** (격차): Context 거르개의 `scope` 칸 placeholder `project · domain:billing` 이 입력칸 폭에서 **`domain:billin` 으로 잘린다.** 「쓰는 법을 보여 주는 자리」가 잘린 예시를 보여 준다. ⚠ **폭은 안 쟀다** — 잘린 것만 캡처로 봤다.

🔴 **2-B 이번 라운드 — 빈 자리 종류(`EmptySlot` 10종)** ① 소비처: `EMPTY_PLACES`(Record) · `ScreenEmpty` · 화면 8곳 ② 뒤집으면 갈림: `next` 를 지우면 그 화면에 `<a>` 가 **0개**가 되고, `to` 를 표 밖 낱말로 바꾸면 시험 둘(`PROJECT_SCREENS` 대조 · `page.tsx` 실재)이 **동시에** 빨개진다 · `tone` 을 `accent` 로 바꾸면 「한 화면에 accent 하나」가 빨개진다.


### 지난 바퀴 (95) — 팔레트의 둘째 묶음(프로젝트 전환) · 줄은 `PaletteEntry` 하나 · 목록의 출처는 `GET /teams` 뿐 · 눈이 잡은 목록 높이 (FINDINGS 157 · `d95f7ca`)
**그 바퀴(95)는 FINDINGS 157 — 구멍(팔레트에 프로젝트 전환이 없다)을 닫았다** (`d95f7ca`). INBOX 「할 것」비어 있음 · **관통은 못 돌렸다**(아래 🔴 — 155 가 `api` 단계를 막는다 · **세 바퀴째 같다**).
🔴 **대장의 근거가 반만 맞았다 — 문은 이미 있었다.** 157 은 「내가 볼 수 있는 프로젝트를 내주는 문이 0곳」이라 새 GET 을 뚫으라고 적었지만, `GET /teams` 가 **본인이 속한 팀과 그 안의 프로젝트까지** 낸다 (`api/v1/teams/route.ts` · SPEC §5 표에도 있고 · 화면은 `fetchTeams()` 로 **이미 부르고 있었다** — `ProjectGate` 가 slug→uuid 를 그것으로 바꾼다). 94바퀴가 못 본 이유는 `find … -type d` 로 **디렉터리만** 봤기 때문이다(디렉터리는 `teams/[id]/projects` 뿐이고, 목록은 `teams/route.ts` 안에 있다). **그래서 새 문을 안 뚫었다** — 뚫었으면 「내가 볼 수 있는 프로젝트」의 정본이 둘이 되고 권한이 둘로 갈린다. ★ **SPEC 도 대장도 「의도」다. 코드가 현실이다** (loop/PROMPT.md ②).
**둘째 종류가 온 지금이 묶는 자리였다** — 94바퀴는 「종류가 하나인데 `type: 'nav'|'action'` 을 미리 만들지 마라」고 남겼고 그게 맞았다. 이번에 둘째(프로젝트)가 왔으므로 줄을 `PaletteEntry` 한 모양으로 묶고, **묶음의 제목·차례는 `PALETTE_GROUP_TITLES`(Record)·`PALETTE_GROUP_ORDER` 한 표**다. 팔레트의 **그리는 부분은 줄이 어디서 왔는지 모른다** — 묶음이 셋이 되어도 안 고친다(빌더 함수 하나가 는다 · 절차는 표 옆 주석).
🔴 **지어내지 않는다** — 프로젝트 줄은 서버 응답에만 있는 것이다. 목록이 아직/못 왔으면 **빈 묶음이 아니라 한 줄 문장**이고(빈 묶음은 「내 프로젝트가 하나뿐인가」로 읽힌다), 옮길 때는 **보던 화면을 그대로 들고 간다**(Sync → 옆 프로젝트의 Sync · 표에 없는 주소면 표의 첫 화면이라 404 로 가는 줄 0).

🔴 **잰 것** (시험 · CI 06:41 · 브라우저 07:0x):

| | 전(94바퀴) | 후 |
|---|---|---|
| 프로젝트를 옮기는 자리 | **0곳** (주소를 손으로 고쳤다) | 팔레트의 둘째 묶음 — 브라우저에서 `Enter` 로 `/t/paylab/p/paylab-api/sync` → **`/t/paylab/p/ledger/sync`** 로 실제로 갔다 |
| 그 목록의 출처 | — | **`GET /teams` 하나** (새 문 0개 · 라우트 파일 0줄 변경) |
| 팔레트가 아는 줄의 모양 | `ProjectScreen`(화면 전용) | `PaletteEntry` 하나 — 묶음이 늘어도 그리는 부분은 안 고친다 |
| 목록이 못 왔을 때 | — | 「프로젝트 목록을 불러오지 못했습니다」 한 줄 (option 은 화면 7줄뿐) |
| 브라우저에 쌓는 목록 | — | **0** (시험이 `localStorage`·`sessionStorage` 를 센다) |
| 시험 | 748개 (744 통과) | **757개 (753 통과)** — +9 (web-command-palette ⑥) |
| CI | RED (155) | 🔴 **RED — 여전히 155 뿐이다** (아래) |

🔴 **눈으로 봤다 — 진짜 브라우저다** (`docs/evidence/2026-09-07-project-switch/`). 헤드리스 Chrome + 실제 `next dev` + 씨앗 DB 에서 **진짜 로그인 경로**로 들어가 1440×900 으로 찍었다. 씨앗에 프로젝트가 하나뿐이라 **제품의 문으로**(`POST /teams/{id}/projects`) `paylab/ledger` 를 하나 더 만들고 봤다 — 화면에 목록을 지어 넣은 것이 아니다. `Ctrl+K` → 묶음 **둘** · option **9줄**(화면 7 + 프로젝트 2) · 지금 화면과 지금 프로젝트 둘 다 「지금 여기」 · 「정산」 → **1줄**(「화면」 묶음은 제목째 사라진다) · `Enter` → 내비가 `paylab/ledger` 로 바뀌고 Sync 의 empty 상태 · 「zzz」 → 「그런 화면도 프로젝트도 없습니다」 · `esc` → dialog 0.
🔴 **눈이 하나를 잡았다 — 시험은 초록이었다.** 첫 판은 `--palette-list-h: 320px` 라 목록이 **화면 7줄에서 잘렸고 「프로젝트」 묶음이 접힌 자리에 있었다.** DOM 에는 9줄이 다 있고 시험도 전부 초록인데 **사람에게는 전환이 없는 것처럼 보인다.** 토큰을 420px 로 올려 다시 찍었다 (커밋 뒤의 한 줄이라 `d95f7ca` 다음 커밋에 있다).

🔴 **CI 는 아직 빨갛다 — 같은 넷이고 이 변경 때문이 아니다 (FINDINGS 155 [고장])**. 06:41 CI: `test` FAIL — `apps/web test/readme.test.ts` **넷**이고 **나머지 753 전부 초록** · principles OK 9 · typecheck OK · docs OK · `plugin` 178 초록. 그 세션의 마지막 커밋은 **여전히 `eceaaab`** 다 — 두 바퀴째 커밋이 안 늘었다. 95바퀴는 README·랜딩을 **한 줄도 안 건드렸다**. ⛔ **README·랜딩을 이 루프가 고치지 마라.**
⚠ **안 한 것** — 관통(`api` 단계 = `pnpm --filter web test` 라 155 에 그대로 막힌다) · 375px 모바일에서 팔레트를 안 봤다(1440×900 만 · 94바퀴부터 밀린 것과 같은 자리) → 「눈 판정 대기」.

🔴 **배운 것** — **「문이 없다」는 대장의 한 줄을 그대로 믿으면 문을 하나 더 뚫는다.** 근거 줄에 `find … -type d` 가 적혀 있었고 그게 디렉터리만 세는 명령이라는 것을 이번에 봤다. 라우트는 **파일 안에** 있다(`GET` 과 `POST` 가 한 파일이다). **없다는 주장을 근거로 만들 때는 「무엇으로 찾았나」를 같이 적어라** — 다음 사람이 그 명령의 사각을 볼 수 있다.

🔴 **2-B 이번 라운드 — 팔레트의 줄 종류(`PaletteGroup` 2종)** ① 소비처: 제목(`PALETTE_GROUP_TITLES`)·차례(`PALETTE_GROUP_ORDER`)·빌더 둘 ② 뒤집으면 갈림: 팀 목록을 안 주면 프로젝트 줄이 **0개**(option 7)이고, 주면 9개다 · 「정산」 검색이면 화면 묶음의 **제목까지** 사라진다 · 지금 프로젝트만 `here` 다. `keywords` 대신 프로젝트는 `hint`(`팀/프로젝트`)와 **팀 이름**으로도 걸리는지 각각 잰다.

그 바퀴가 다음으로 지목한 것은 FINDINGS 133 이었다 — 이번 바퀴가 닫았다 (`e3e48fe`).

🔴 PLAN 의 `- [ ]` 맨 위는 여전히 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이고, 그 몫인 격차 차례다: **133** → 134 → 137 → (155 가 닫힌 뒤) 131. ⚠ 156 은 배포 뒤에 재고(고르지 마라) · 153·152 는 관찰 중 · 140 은 P5 둘째 행(🙋 새 PC)과 같이 · 117 은 절삭 1번.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(155 · 156 · 153 · 152 · 140 · 117 · 131 · 133 · 134 · 137) — **고장 1**(155 · 주인은 남의 세션) · 157 ✅.

### 지난 바퀴 (94) — 명령 팔레트 ⌘K · 내비와 같은 표 `PROJECT_SCREENS` 하나 · 표를 옮기면 글자로 읽는 게이트가 따라간다 (FINDINGS 132 · `d2ac4bb`)
...
🔴 **일부러 안 만든 것 둘** — ① **명령**(발행·재시도 실행)이 없다. 종류가 하나인데 `type: 'nav' | 'action'` 을 미리 만들면 둘째가 왔을 때 그 추상이 안 맞아서 다시 짠다 ② **프로젝트 전환**이 없다. 「내가 볼 수 있는 프로젝트」를 내주는 문이 **0곳**이다(`/api/v1/teams/{id}/projects` 는 POST 뿐) — 목록을 손으로 지어내면 게스트에게 남의 팀이 보인다. **→ FINDINGS 157**(구멍)로 적었고 그 바퀴가 다음으로 지목했다.
...
**그 바퀴가 다음으로 지목한 것** — FINDINGS 157 (95바퀴가 `d95f7ca` 로 닫았다).
