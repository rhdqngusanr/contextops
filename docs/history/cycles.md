...
### 지난 바퀴 (88) — 데모 항목 15 → 27 · goals.md 가 가진 전부 · SPEC §10.3 60 → 27 · `PACK_EXCLUDED_TYPES` · 눈이 잡은 같은 목표 두 줄 (FINDINGS 119 · `70c3a73`)


**이번 바퀴(88)는 FINDINGS 119 — 격차(데모 항목 15 · SPEC §10.3 은 60)를 닫았다** (`70c3a73`). 먼저 셌다: 60 은 goals.md 에 없는 수라 지어내야 채워진다(P7) — 그래서 SPEC 쪽 수를 고쳤다. 그런데 반대 방향으로 문서에 **항목이 안 된 문장이 남아 있었다** — G2·G3 목표, M2·M3 마일스톤(§10.1 이 「M1~M3 · open_question 4」를 기대하는데 씨앗엔 M1 뿐 · 미결 0), §3 규칙 셋, §1 정산 배치 제약, §5 미결 넷. 전부 씨앗에 올려 **초안 15 → 26 · 항목 27**. INBOX 「할 것」비어 있음 · 관통 7단계 OK(1017) · 고장 0.
**씨앗** — 표 셋 `GOALS`(3) · `MILESTONES`(3 · `dependencies`) · `OPEN_QUESTIONS`(4) — `ARCHITECTURE` 와 같은 모양(줄 순서 → `priority` · quote 는 원문 그대로 · 더하는 절차는 표 옆 주석). 정책 3 · 제약 1 은 inline. `QUOTED_DATA.open_question = []`(question 은 진술).
**컴파일러** — `PACK_EXCLUDED_TYPES { open_question: 이유 }` 표 하나. partition 이 `exclude` 의 이유로 읽고, 관통의 「기대한 근거 전부 역추적」이 그 타입의 근거를 기대에서 뺀다 — 검사 쪽에 타입 이름을 안 적는다. **빨개지는 것을 봤다**: 표를 넣기 전 관통이 「기대한 근거 28개 … item_oq_* 넷」으로 FAIL.
**관통** — 제안이 더하던 G3 가 씨앗과 겹쳐 v1.1.0 에 **같은 목표가 두 줄** 섰다(눈으로 잡았다 — 시험은 못 잡는다: 다른 항목이라 메아리 검사 밖) → §2 「부딪히면 G2 가 우선」 policy 로 바꿨다(문서에 남은 마지막 근거). **demo.json** — PL-M2 보고 둘(criterion_done · in_progress) · progress 6 → 8 · 로드맵 외 1 그대로.
**문서** — SPEC §10.3 헤더 60/25 → **27/8** 과 내역(§2 표 3 · §3 규칙 7 · §4 M1~M3 · §5 미결 4 · §6 · §7 · §1 제약 2) · KNOWN_LIMITATIONS 의 「데모 항목이 15개다」 → 「27개 · 문서에서 역추적되는 전부」.

🔴 **잰 것** (`docs/evidence/2026-09-07-demo-items/` — v1.1.0 `CLAUDE.md` · `domain-refund.md` 본문):

| | 전(87바퀴) | 후(`70c3a73`) |
|---|---|---|
| 초안 · 항목 | 14 · 15 | **26 · 27** (mission 1 · goal 3 · roadmap 3 · policy 8 · constraint 2 · architecture 5 · domain 1 · open_question 4) |
| 근거 역추적 · 기대 근거 · 인용 칸 | — | **94 · 24/24 · 166** 전부 원문 그대로 (open_question 넷은 표로 기대에서 뺀다) |
| ItemType 갈래 · 메아리 | 7 · 0 | 7 · 0 (open_question 은 설계상 Pack 밖 · adr·workflow 는 문서에 없다) |
| `domain-refund.md` | 1줄 | **2줄** (에스컬레이션 규칙이 같은 도메인) |
| Roadmap 절 | PL-M1 | **PL-M1 · PL-M2 `deps: PL-M1` · PL-M3 `deps: PL-M1`** — `dependencies` 가 처음으로 값을 가진다 |
| v1.1.0 CLAUDE.md | (안 쟀다) | **5,373자** (한도 12,000 · 분할 없음) |
| 시험 · CI | — | web 54+48(landing · replay · demo-guest · reset · roadmap) · compiler 185 · **GREEN 03:32** (walkthrough 1017) |

⚠ **안 한 것** — 브라우저로 화면 5(항목 27 · 열린 질문 배지 · `conf:low` chip)·화면 8(PL-M2·M3 행)을 안 봤다 → 「눈 판정 대기」. old-roadmap.md 에서는 여전히 항목 0(설계 — stale 탐지용). `adr`·`workflow` 타입은 여전히 문서에 근거가 없다(표 10 중 7 — 더 보이려면 goals.md 를 먼저 넓혀야 한다 · FINDINGS 89 의 규칙).

🔴 **배운 것 둘** — ① **「SPEC 의 수를 못 채운다」와 「문서를 다 옮겼다」는 다른 문장이다.** 119 는 8바퀴 동안 「60 은 못 채운다」로만 읽혔는데, 셌더니 문서의 절반이 항목이 안 돼 있었다. 격차를 닫을 때는 **위(의도)만 보지 말고 아래(픽스처)를 먼저 세라.** ② **눈이 잡은 것** — v1.1.0 의 「정산 오차 0원」 두 줄은 어떤 게이트도 못 잡았다(서로 다른 항목이라 메아리 밖 · 둘 다 P7 은 멀쩡). 산출물을 **읽지** 않았으면 지나갔다.

🔴 **2-B 이번 라운드 — `RoadmapData.dependencies` · `confidence` 3단계 · `open_question` 타입** ① 소비처: `dependencies` 는 roadmap 절의 `deps:`(compiler `sections.ts`) — 정의만 있고 씨앗이 늘 `[]` 였다 → 이제 M2·M3 이 `PL-M1` 을 든다 · `confidence` 는 chip(`conf:` 태그) — 데모가 전부 `high` 였다 → 미결 넷이 `low` · `open_question` 은 화면 5 목록·chips(「열린 질문」) — 데모에 0건이었다 → 4. ② 뒤집으면 갈림: `deps:` 는 Pack 줄에 그대로 찍혔다(위 표) · `confidence`·`open_question` 의 화면 쪽은 브라우저로 안 봤다(눈 판정 대기). `ItemType` 10종 중 데모는 **8종**(open_question 포함 · Pack 은 7) — 나머지 `adr`·`workflow` 는 픽스처에 원문이 없다.

그 바퀴가 다음으로 지목한 것은 FINDINGS 118 이었다 — 89바퀴가 닫았다 (`eaaaf29` · 화면 9 요약이 「공식 vX.Y.Z 기준」 · `OFFICIAL_HINT` · `officialOf` · 시험 +4).

🔴 PLAN 의 `- [ ]` 맨 위는 **P4 둘째 행**(GATE 3 · 눈 판정)이다 — ④3 ② 로 그 행이 다음이고, **118**(화면 9 가 「무엇으로 맞춰야 하나」 — 공식 semver — 를 안 말한다 · 격차)은 그 행의 몫이라 그 행을 하는 것과 같다. 루프가 혼자 닫을 구멍은 없다 — 152 는 다음 실패가 200자를 말할 때까지 행동할 수 없고(고르지 마라), 140 은 P5 둘째 행(🙋 새 PC)과 같이, 117 은 절삭 1번. 그 뒤 116 → 112 → 59 → 100 → 131 → 132 → 133 → 134 → 137.

> **118 을 하는 법** — 대장의 「고칠 방향」: 화면 9(`…/sync`)가 `fetchVersions()`(`lib/web/queries.ts` · 문이 이미 있다)를 한 번 더 불러 `is_official` 인 행의 `semver` 를 상단 요약에 적는다. ⚠ 서버가 sync-status 응답에 넣는 쪽은 고르지 마라 — 기기 목록 라우트가 버전 표까지 알게 된다. 합격은 `dump-sync.tsx` 모양에 「공식 vX.Y.Z」가 서고 `web-sync` 시험이 그 글자를 세고 CI 초록. 화면 8 의 요약 타일이 같은 값을 어떻게 적는지(`공식 v1.0.0 기준`) 먼저 읽어 **같은 낱말**을 써라.

- PLAN 의 `- [ ]` 중 남은 것 **넷**: P4 둘째 행(GATE 3 · 눈 판정 — 다음 PLAN 행) · P5 셋째 행(🙋 Vercel) · P6 두 행(🙋 영상 · 제출서는 production URL·영상만 🙋).
- 대장의 대기(152 · 140 · 118 · 117 · 116 · 112 · 100 · 59 · 131~134 · 137) — **고장 0** · 119 ✅.

### 지난 바퀴 (87) — 인용은 한 문단 안에서만 · 재시도 불평은 오류 전부 · id 규칙은 표에서 · 진짜 Gemini 세 번(회복 16 · 실패(→152) · 16) · 152 기록 (FINDINGS 151 · 149 · `57498b7`)
...
그 바퀴가 다음으로 지목한 것은 FINDINGS 119 였다 — 88바퀴가 닫았다 (`70c3a73` · 초안 15 → 26 · 항목 27 · `PACK_EXCLUDED_TYPES` · 관통 제안을 G3 → §2 우선 규칙으로).
...
🔴 **잰 것** (`docs/evidence/2026-09-07-p3-gemini/probe.txt` 84바퀴 절 · 진짜 gemini-3.5-flash):

| | 145 만 (`9a5da46` · 두 번 + 진단) | 145+147 1회차 (`probe-84-run1.json`) | 145+147 2회차 (`probe-84-run2.json`) |
|---|---|---|---|
| goals.md | **`AI_OUTPUT_INVALID`** ×3 — 응답은 `STOP`·JSON 정상, 인용 27개 중 **5개**가 「줄 중간 줄바꿈 → 공백」 하나로 없음 · 재시도는 첫 인용만 불평 | 항목 **24** · 질문 4 · 47초 · 재시도 1 | 항목 **18** · 질문 4 · 20초 · **재시도 0** |
| 인용 | 22/27 | **28/28** · roadmap 6/6 | **22/22** · 6/6 |
| accept 거절 (145) | — (잴 수 없었다) | **0** (24/24) | **0** (18/18) |
| 탐지 후보 · 충돌 | — | 3/3 · **4/3** | 3/3 · **4/3** |
| 충돌 짝 | — | 환불 24h vs 기한 없음 · 고정 간격 금지 vs 0.5초 고정 · PII 금지 vs payload 로그 · 5회 vs 3회 — 전부 contradiction high · 질문형 | 같은 넷 |
| scope | — | goals policy 전부 `project` · 로드맵 policy 셋 `project` | 같음 (`path:src/` 는 마일스톤 셋뿐) |
| 장부 | 2행(실패 행은 추정치) | 3행 ≈ $0.042 | 3행 ≈ $0.020 |
| 시험 · CI | ai-structure 31 · ai-job +1 · **GREEN 00:52** | — | ai-structure **33** · **GREEN 01:01** (principles OK 9 · test 92초 · build 21초 · walkthrough **1003** · docs OK) |

⚠ **PLAN 행은 열어 뒀다** — 「두 번 연속 3 이상」은 봤지만 **146**(후보가 모델이 고른 scope 에 달려 있다)이 열려 있다. 이번 두 번은 policy 가 전부 `project` 라 안 갈렸을 뿐이다. 146 을 닫고 다시 두 번이면 체크한다. 행의 ⑦ 에 수치를 적었다.
⚠ **안 한 것** — 146 은 적기만 · 재시도 불평이 첫 인용 하나만 말하는 것(다섯이 틀리면 다섯을 말해야 한 번에 고친다)은 접기로 다 지나서 안 건드렸다 · 화면 3 은 여전히 브라우저로 안 봤다. 두 번 다 모델이 로드맵과 **다른** slug 를 골라 `_2` 는 실제로 안 밟혔다 — 밟히는 경우는 시험이 잠근다.

🔴 **배운 것 둘** — ① **실패 행의 토큰은 추정치다** — 장부의 「입력 1,406 · 출력 0」을 보고 모델이 빈 응답을 냈다고 읽으면 틀린다(`withBudget` 의 catch 가 글자수로 추정해 적는다). 진짜 응답은 transport 를 감싸야 보인다 — 그래서 일회용 진단을 만들고 지웠다. 144(finishReason)를 할 때 이 자리를 같이 봐라.
② **「원문 그대로」의 뜻은 글자이지 공백이 아니다.** 마크다운 문단의 하드 줄바꿈은 공백과 뜻이 같고 모델은 그렇게 읽는다. 검사를 글자 단위로 좁히되 공백은 접는 것이 「기준을 낮추지 않는」 고침이다 — 반대로 `**` 를 더한 인용은 여전히 거절돼야 하고, 시험이 그걸 잠근다.
...
### 지난 바퀴 (83) — 규칙 목록은 줄마다 항목 · 부정형도 규칙 · 진짜 Gemini 로 환불 줄이 policy · 같은 코드로 충돌 3/3 ↔ 0/3 → 146 기록 (FINDINGS 143 · `5be2611`)
...
**그 바퀴가 다음으로 지목한 것**: FINDINGS 145 → 84바퀴가 닫았다 (`9a5da46`) — 재다가 147 을 찾아 같은 바퀴에 따로 닫았다 (`e64831b`). 아래는 83 이 남긴 지목의 원문이다.
